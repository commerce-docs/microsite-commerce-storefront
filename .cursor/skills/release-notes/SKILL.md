...
| Changelog | Create `<ChangelogEntry>` blocks from change summaries **only for components with user-facing or implementer-facing changes**; omit entries for version-alignment-only or internal-only updates. **Insert each entry** at the top of its component section in `changelog.mdx` (do not add one block at the top of the file). Apply [Writing rules](#writing-rules) to all prose. |
