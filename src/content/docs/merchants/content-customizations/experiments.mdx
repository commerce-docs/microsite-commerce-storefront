...
The Experimentation Plugin is **not pre-installed** in the Commerce Boilerplate (<Link href={"https://github.com/hlxsites/aem-boilerplate-commerce"} text={"hlxsites/aem-boilerplate-commerce"} />). Instead, developers can optionally install it based on their experimentation needs.

To install and configure the plugin, follow the instructions in the <Link href={"https://github.com/adobe/aem-experimentation/"} text={"AEM Experimentation GitHub repository"} />.
...
Duplicate the original page and make the changes you need to create a new page with the new content or features you want to use in the experiment. For most content changes, you can add those changes directly to the page/document. Examples include adding any of the <Link href={"https://www.aem.live/developer/block-collection#boilerplate"} text={"AEM content blocks"} />, additional commerce blocks (such as product recommendations) and more.
