...
Both properties are required and have no default values. You must provide `recId` for all pages, and `currentSku` for non-PDP pages. The block will not function correctly if required properties are missing or invalid.
