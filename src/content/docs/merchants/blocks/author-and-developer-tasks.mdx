...
| Add a new block, or a new layout variation for an existing one (for example, an additional hero banner style) | New blocks and layout variations are built in HTML, CSS, and JavaScript, then made available for an author to select in a block or section-metadata table. A page designer builds these. A developer is needed only when the change requires drop-in code or a live catalog data feed. |
