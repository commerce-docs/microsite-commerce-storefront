...
**urlPath**: Specifies the category URL path as defined in your commerce catalog (not the CMS page path). Use this to create category-specific product listing pages. For example, `/women/dresses` shows products from the dresses category. Set this when creating a dedicated category page. Leave empty for search result pages.

This configuration is also used by the [enrichment block](/merchants/content-customizations/enrichment/) for category-based content enrichment.
