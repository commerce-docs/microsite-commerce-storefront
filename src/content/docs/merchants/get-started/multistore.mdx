...
| URL | root | placeholders | commerce-website | commerce-store | commerce-storeview |
|---|---|---|---|---|---|
| /en/** | /en/ | /en/placeholders.json | website_us | store_us | default |
| /en_ca/** | /en_ca/ | /en/placeholders.json<br/>/en_ca/placeholders-overrides.json | website_ca | store_ca | en_ca |

- **URL**: Defines the URL pattern for pages that should receive this metadata entry. Supports wildcards to match groups of pages (e.g., /en/** for all English pages). This determines which routes the metadata will apply to.
- **root**: Specifies the root folder from which all internal document links are resolved. Paths in the content will be considered relative to this directory.
- **placeholders**: Points to the `placeholders.json` file that provides key-value mappings for dynamic content replacement. You can specify multiple files—each on a new line. When multiple files are listed, the second and subsequent files act as overrides, with their values taking precedence over previous ones.
- **commerce-website**: Defines the website code configured in Adobe Commerce. A website is the top-level container in the Commerce hierarchy that can hold multiple stores. Each website has its own base configuration, including currency, customer accounts, and order processing.
- **commerce-store**: Sets the store code within the selected website. A store typically represents a logical grouping of product categories and shares catalog structure and settings with other stores under the same website.
- **commerce-storeview**: Indicates the store view code within a specific store. Store views are used to provide localized versions of the store (e.g., different languages), allowing customers to switch between translations or regional content.
...
1. Set the **URL** to the new store view. For example: `/en_ca/**`.
...
1. Set the **website** to the new store view. For example: `website_ca`.

1. Set the **store** to the new store view. For example: `store_ca`.

1. Set the **storeview** to the new store view. For example: `en_ca`.
