...
|---|---|---|---|---|---|
| /en/** | /en/ |  | website_us | store_us | default |
...
- **placeholders**: *(Optional)* Specifies the path to a `placeholders.json` file that provides key-value mappings for dynamic content replacement. By default, the storefront loads `placeholders.xlsx` from the directory defined by the **`root`** metadata. To override this behavior, you can specify multiple files—each on a new line. When multiple files are listed, the second and subsequent files act as overrides, with their values taking precedence over the previously listed files.
