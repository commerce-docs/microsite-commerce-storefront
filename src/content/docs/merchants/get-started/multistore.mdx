...
The content files provide the structure of the pages served to your shoppers. They are located in the store view directories and contain the content for each store view. These files are documents on https://da.live, SharePoint `.docx`, or Google document files used to generate the store view pages. These files should be localized or translated into the desired language.


### Placeholders

The `placeholders` sheet stores reusable variables for text and UI components. This file is used to define the text that is displayed on your storefront. The `placeholders.json` file is generated from this file and is used to replace the variables in your content files.
