...
After translating your storefront content using your chosen authoring tool ([Universal Editor](/merchants/quick-start/content-localization-universal-editor/) or [Document Authoring](/merchants/quick-start/content-localization/)), complete these Commerce-specific tasks to ensure full localization.

<Aside type="tip" title="For developers">

For technical implementation details on how the placeholder system and link localization work in the boilerplate, see:
...
1. Verify the JSON syntax remains valid after translation.
...
1. Review your [multistore setup](/setup/configuration/multistore-setup/) to confirm:
   ...
   - Folder structure on Edge Delivery Services (e.g., `/en/`, `/fr/`, `/fr-ca/`, `/en-ca/`)
...
<Steps>
1. Navigate to your storefront in each locale (e.g., `https://yoursite.com/fr/`).
...
The boilerplate automatically localizes internal links to match the current locale. This ensures users stay within their chosen language/region as they navigate.
...
This prevents the automatic localization, keeping the link pointing to `/en/` regardless of the current locale.
...
- **Placeholder files**: Always translate placeholder JSON files for drop-ins before launching a new locale. Untranslated labels create a poor customer experience.
- **Store view testing**: Test the complete customer journey (browse → cart → checkout) in each locale to verify Commerce integration.
- **Path alignment**: Ensure content paths and multistore configuration align to the same locale structure.
- **Product configuration**: Verify that products, categories, and CMS content are properly configured for each store view in Adobe Commerce Admin.
- **Translation quality**: For customer-facing Commerce content (checkout, cart messages, error messages), consider human translation over machine translation.
- **Currency and pricing**: Test that currency symbols, decimal separators, and price formatting display correctly for each locale.
- **Link localization**: Use `#nolocal` for store switcher and cross-locale links to prevent automatic localization.
