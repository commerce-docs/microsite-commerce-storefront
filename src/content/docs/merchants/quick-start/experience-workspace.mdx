...
<Aside type="note" title="Early access">
  Experience Workspace is in early access and continues to evolve. Use the linked AEM documentation
  as the source for current interface labels, configuration options, and capabilities. Try it on
  non-critical pages first, since interface labels and behavior can still change.
</Aside>
...
Experience Workspace is off by default. An administrator or developer enables it once for your organization or site — this project-level setup is separate from each author's own New Authoring toggle (see the next section), and it may require a developer to update your storefront's code before it can be turned on.
...
Open a page in your Document Authoring project in edit view, not the folder view. After your project is enabled, each author turns on **New Authoring** individually in the document editor header to open their own pages in Experience Workspace. This is a personal preference — turning it on doesn't affect other authors. To return to the document editor, open your profile menu and turn off **New Authoring**.
...
**Layout** shows the rendered page. Select a block to reveal a contextual toolbar, then use it to edit the block's properties directly on the page. Use this view for most content edits.
