...
The Single Product Detail page template comes with a `product-details` block, also known as a Product Details Page (PDP) drop-in. Like other document blocks, the PDP drop-in is a two-column table — **property names** on the left, **property values** on the right. Make the following changes to the `product-details` block.
...
<Diagram caption="Change the the Hero block">
  ...
  ![Change the Hero block](images/1_change-hero-block2.png)
</Diagram>
