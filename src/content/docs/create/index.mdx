...
From repo to render, this tutorial will guide you through the steps of creating a new Edge Delivery Services + Commerce storefront. The broad steps in this process look like this:

<Diagram caption="Broad steps to create your Commerce storefront.">
  ![Broad steps to create your Commerce storefront
  storefront.](@images/create/SimpleStorefrontSteps.svg)
</Diagram>

And the details are only slightly more involved. Let's break it down.
...
1. Return to your folder, select the `index` file, select **Sidekick**, and then select **Preview** to add your folder's content to the internal Edge Delivery CDN, where it can be downloaded and displayed on the user's browser. Without completing this step, the internal CDN remains empty, causing **404 errors** when you try to run your storefront from your local development environment (next step).

</Callouts>

You can now preview, edit, and publish content directly from your Google Drive or SharePoint folder. Proceed to the next step.
