...
1. The <Term>block decorator</Term> runs for each block on the page. A block decorator is the JavaScript file in your repository for that block, for example, `blocks/commerce-cart/commerce-cart.js`. Edge Delivery Services finds each block's `div` in the HTML, loads the matching file from your repository, and runs its default export, which imports the drop-in's <Term>initializer</Term> and then mounts the shopper-facing UI. See [How blocks connect to drop-ins](/get-started/architecture/blocks-and-repo/#how-blocks-connect-to-drop-ins) for the full three-step sequence.
