...
The following diagram shows a small branding change. When we override the default value of a single shape token, we override the default border-radius of the `Button` in the storefront's <Term>library components</Term>, which changes the look and feel of drop-ins and custom blocks that use the shared Button component.
...
These token values come from the <Term>Adobe Commerce design system</Term>. Drop-in UI components pick them up automatically, and you can reference the same tokens in your own custom block CSS to keep it visually consistent.
...
- scripts/
- **styles/** _CSS files for shared design tokens, fonts, deferred styles_
...
The process of branding your storefront is typically fast and easy. Focus on one brand category at a time and work with your designers to solve the less obvious brand-to-token overrides.
