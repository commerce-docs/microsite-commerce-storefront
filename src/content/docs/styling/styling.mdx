...
description: Learn how to customize drop-in components and custom blocks using shared design tokens and CSS classes.
slug: dropins/all/styling
sidebar:
...
Customize drop-in components and your own custom blocks using the design token system and CSS classes from the boilerplate. This guide covers the styling approach shared across drop-ins and blocks alike.
...
- These styles load immediately and affect your whole storefront, including custom blocks
...
Drop-in components and custom blocks both use the CSS custom properties (design tokens) defined in the `styles/styles.css` file from the boilerplate. These tokens ensure consistent styling across your whole storefront and make global theme changes easy to apply.
...
<Diagram caption="Find CSS classes to override.">
  ![Browser developer tools inspecting a PDP drop-in element with the Styles panel listing BEM-style CSS classes to override](@images/dropins/findstyles.webp)
</Diagram>

<Callouts>

1. **Inspect the element** you want to customize (right-click the element and select "Inspect" from the menu).
1. **Identify the CSS class(es)** for the element. We use <Link href={"https://getbem.com/naming/"} text={"BEM naming"} />, which makes components and their elements easy to identify. For example, `.pdp-product__title` indicates the title element of the product component.
