...
The main difference between the three use cases described in this section is the “default” routing logic; whether by default all paths are routed to Commerce or Edge Delivery Services. If you plan to use Edge Delivery Services for your homepage only, it makes more sense to default all paths to the Commerce origin. If you plan to migrate parts of your storefront over time with an eventual goal of being entirely served by Edge Delivery Services, then defaulting to Edge Delivery Services origins is more sensible.
...
| **Paths routed to Commerce**    | Product images, GraphQL endpoint             | Transactional pages, additional endpoints    | All paths except explicitly set ones        |
| **Paths routed to Edge**        | All other paths                              | Product catalog, content pages               | Homepage                                    |
| **Commerce Admin involvement**  | Minimal                                      | Moderate                                     | High                                        |
...
| **Common patterns**             | Use custom LS Widget for API URL             | Same as full storefront                      | Move Edge code to subfolder (e.g., aem)     |
...
* CDN must proxy requests to your Catalog Service API endpoint (https://catalog-service.adobe.io/graphql and https://catalog-service-sandbox.adobe.io/graphql).

  :::tip
  These API endpoints impact LCP, so we want to avoid an additional TLS handshake. This optimization has already been applied to the boilerplate, where we use a "custom" version of the LS Widget to change the API URL used by the widget.
  :::
...
For general information on setting up Fastly for Adobe Commerce and accessing the Admin UI, see [Fastly services overview](https://experienceleague.adobe.com/en/docs/commerce-cloud-service/user-guide/cdn/fastly).

## Backend configuration 

The first step in the configuration process is to configure a backend for each origin/service that Fastly needs to route to. Typically, this would be the Catalog Service GraphQL API endpoint, Edge Delivery Services, and default Commerce backend.

Make sure that the following is set:

* **Condition**: We will handle conditional routing in the VCL scripts. Set the condition to `false (REQUEST) false` for all backends. Do not forget to set this condition also for the default backend, otherwise this can overwrite the changes made by your VCL conditions that are configured in the snippets.

* **Shielding**: Adobe recommends keeping this off for simplicity. If you're using shielding, see the [shielding section](#shielding).

In addition to the Edge Delivery Services backend, do the same for the Catalog Service GraphQL API endpoints (https://catalog-service.adobe.io/graphql and https://catalog-service-sandbox.adobe.io/graphql), plus any other backends you may require. You can also rename the Commerce backend from the generated name to simply `commerce`.

The following screenshot shows the configuration for Edge Delivery Services.

![Backend configuration for Edge Delivery Services](@images/implementation/backend-config-edge.png)

![Backend configuration for Commerce](@images/implementation/backend-config-commerce.png)

## VCL configuration

Your routing set up routing will typically require five VCL snippets:
...
See [Getting started with custom VCL](https://experienceleague.adobe.com/en/docs/commerce-cloud-service/user-guide/cdn/custom-vcl-snippets/fastly-vcl-custom-snippets) or use the Fastly Admin module UI to apply the custom VCL snippets below to your Adobe Commerce Fastly CDN service.

These VCL snippets are starting points and can be altered or extended to fit your use cases. There are also examples of potential extended use cases listed further down on the page. The main purposes of the VCL snippets are to:
...
If you need to rename a snippet, delete it first and then recreate it with the new name: https://github.com/fastly/fastly-magento2/issues/708.
:::

![VCL configuration](@images/implementation/vcl-config.png)

See [Using VCL](https://www.fastly.com/documentation/guides/vcl/using/) in the Fastly documentation for more information.

### recv

The folloing snippet does a simple redirect if `host` does not contain `www`. It adds it and then redirects to `https` using a Fastly-specific error code.
...
The `recv` snippet has the following purpose:

* Path based routing for Edge Delivery Services, Catalog Service, or Adobe Commerce backends
...
if (req.backend == F_commerce) {
    set bereq.http.Host = "mcprod.aemshop.net";
}
...
:::note
Changes to the host header set here will require a Commerce Fastly cache purge due to how and when Fastly calculates cache `HITS`.
:::
...
Header size limit. We need to remove custom cookies. Add this in the graphql section of `recv` snippetå:
...
In the Adobe Commerce backend, assuming you are using the VCL to do the APEX redirect (`aemshop.net` → `www.aemshop.net` with 801 error), you should do two things:

1. Disable `auto-redirect`.
1. Set the `base_url` and `secure_base_url` settings to your domain, including `www`.

![Commerce configuration](@images/implementation/commerce-config.png)
...
Then, follow the instructions in [Setup push invalidation for Fastly](https://www.aem.live/docs/byo-cdn-fastly-setup#setup-push-invalidation-for-fastly). The remaining configuration described on his page is already taken care of by applying the VCL snippets above.

## Validation

To validate your CDN setup, use `curl` requests to check expected responses from the paths you have configured.

### Edge content encoding, surrogate key, and cache
...
```bash
curl -sI -H 'Fastly-Debug: 1' https://aemshop.net/scripts/aem.js | grep 'x-cache\|surrogate-key\|cache-control'
...
```bash
curl -sI -H 'accept-encoding: gzip, deflate, br, zstd' https://aemshop.net | grep content-encoding
content-encoding: gzip

curl -sI -H 'accept-encoding: gzip, deflate, br, zstd' https://aemshop.net/index.plain.html | grep content-encoding
content-encoding: gzip
```

* Validate the CDN configuration with the BYOCDN [push invalidator tool](https://tools.aem.live/).
* Additionally, preview and publish a document and validate that the changes are correctly reflected.
...
### Edge and Commerce image optimization
...
```bash
curl -sI -H "Accept: image/webp" 'https://aemshop.net/media_12ddec51160a77274a1111c5c7ec6a80ea5c5c2ef.jpeg?width=2000&format=webply&optimize=medium' | grep 'content-type'
...
```bash
curl -sI  -H "Accept: image/webp" 'https://aemshop.net/media/catalog/product/m/b/mb03-black-0.jpg?quality=80&bg-color=255%2C255%2C255&fit=cover&height=&width=300&auto=webp&crop=false' | grep 'content-type'
content-type: image/webp
```
...
```bash
curl -sI 'https://aemshop.net/graphql?query=query+STORE_CONFIG_QUERY+%7B+storeConfig+%7B+minicart_display%7D+%7D' | grep x-cache
x-cache: HIT
x-cache-hits: 1
```
