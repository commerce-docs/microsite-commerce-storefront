...
* CDN must proxy requests to your Catalog Service API endpoint (https://catalog-service.adobe.io/graphql and https://catalog-service-sandbox.adobe.io/graphql).
  ...
  :::tip
  These API endpoints impact largest contentful paint (LCP), so we want to avoid an additional TLS handshake. This optimization has already been applied to the boilerplate, where we use a "custom" version of the LS Widget to change the API URL used by the widget.
  :::
...
1. Enter the address for the Edge Delivery Services backend. For example:

   For example: `main--aem-boilerplate-commerce--hlxsites.aem.live`

1. Click **Attach condition**.

1. Select `false (REQUEST) false` from the **Condition** drop-down list.

   :::note
   Don't forget to set this condition for the default Adobe Commerce backend also, otherwise this can overwrite the changes made by the VCL conditions that you configure in the custom VCL snippets.
   :::

1. Set Shielding to **(none)**.

   Adobe recommends disabling shielding for simplicity. If you choose to use shielding, see the [shielding section](#shielding) for additional guidance.
...
:::tip
You can also rename the Adobe Commerce backend from the generated name to simply `commerce`. For example:
...
Your routing set up requires five VCL snippets:
...
See [Getting started with custom VCL](https://experienceleague.adobe.com/en/docs/commerce-cloud-service/user-guide/cdn/custom-vcl-snippets/fastly-vcl-custom-snippets) or use the Fastly Admin module UI to apply these custom VCL snippets to your Adobe Commerce Fastly CDN service.
...
The VCL snippets on this page are a starting point that you can alter or extend to fit your use cases. There are also examples of potential extended use cases listed further down on the page. The main purposes of the VCL snippets are to:
...
The following snippet does a simple redirect if `host` does not contain `www`. It adds it and then redirects to `https` using a Fastly-specific error code.
