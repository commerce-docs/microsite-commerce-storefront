...
| **Paths routed to Commerce**   | Product images, GraphQL endpoint             | Transactional pages, account page, REST endpoints, etc | All paths except those explicitly                             |
...
| **Common patterns**            | Use CDN as proxy for Catalog Service         | Same as full storefront                                | Move Edge code to subfolder (e.g., `aem`)                     |
...
* CDN must proxy requests to your Catalog Service API endpoint (https://catalog-service.adobe.io/graphql and https://catalog-service-sandbox.adobe.io/graphql).
  ...
  :::tip
  These API endpoints impact largest contentful paint (LCP), so we want to avoid an additional TLS handshake. This optimization has already been [applied](https://github.com/hlxsites/aem-boilerplate-commerce/blob/develop/blocks/product-list-page/product-list-page.js#L15) to the boilerplate, where we use a "custom" version of the LS Widget to change the API URL used by the widget.
  :::
...
:::note
The Fastly configuration instructions on this page are based on the [`fastly-magento2`](https://github.com/fastly/fastly-magento2) module, which expsoes Fastly service configurations in the  Adobe Commerce Admin.
:::
...
1. Select `false (REQUEST) false` from the **Condition** drop-down list.
   ...
   :::caution
   Don't forget to set this condition for the default Adobe Commerce backend also, otherwise this can overwrite the changes made by the VCL conditions that you configure in the custom VCL snippets. You could also lose access to the Admin, which may require an Adobe Commerce Support ticket to resolve.
   :::
...
The following `recv` snippet does a simple redirect if `host` does not contain `www`. It adds it and then redirects to `https` using a [Fastly-specific error code](https://www.fastly.com/documentation/reference/http/http-statuses/). This is done in place of the **Auto-redirect to Base URL** setting in the Commerce Admin. The steps in the [Commerce configuration](#commerce-configuration) section instruct you to set **Auto-redirect to Base URL** to **No** based on this snippet.
...
```bash
curl -Is -L https://www.aemshop.net | grep location
