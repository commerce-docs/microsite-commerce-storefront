...
### Proxy RUM through the origin to avoid a TLS to rum service
...
if (req.backend == F_edge) {
    if (req.http.Host ~ "^([^.]+)\\.([^.]+)\\.lovesac\\.com$") {
        set bereq.http.Host = re.group.1 + "--your-eds-url.hlx.page";
    } else {
        set bereq.http.Host = "your-eds-url.hlx.live";
    }
    set bereq.http.X-BYO-CDN-Type = "fastly";
    set bereq.http.X-Push-Invalidation = "enabled";
}
