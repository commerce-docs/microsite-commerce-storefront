...
When product detail pages (PDPs) have multiple URLs, set a consistent canonical URL for each product. Make sure that canonical URLs do not point to redirects (301).
...
You can use the [PDP Metadata Generator](https://github.com/hlxsites/aem-boilerplate-commerce/tree/main/tools/pdp-metadata) tool to automate the process of generating all recommended metadata. The tool is designed to fetch product data from the Catalog Service, process it, and generate a metadata spreadsheet in `.xslx` format, which can be used for the [bulk metadata](https://www.aem.live/docs/bulk-metadata) feature in Edge Delivery Services.
...
1. Fetch product data, process it, and generate a file named `metadata.xlsx` in the project directory.
