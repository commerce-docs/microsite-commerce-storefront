...
When a catalog is too large for a single index or sitemap file, you need multiple query indexes and matching sitemap files. To set this up, list all sitemap files in a `sitemap-index.xml` file and add each sitemap URL to `robots.txt`.
