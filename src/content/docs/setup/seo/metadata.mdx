...
Adobe   storefronts on Edge Delivery Services should include [schema.org](https://schema.org/) annotations to expose product data to search engines. This data should be included on all Commerce pages, especially PDPs and PLPs.
...
<Aside type="cool" title="Prerequisites">
  * Node.js installed on your machine * Access to the Catalog Service with the necessary API keys
  and configuration * Commerce boilerplate repository cloned to your local machine
</Aside>
