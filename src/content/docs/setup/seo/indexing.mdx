...
If you are migrating from Adobe Commerce on Cloud (PaaS), that platform's URL rewrites (Marketing > SEO & Search > URL Rewrites) do not transfer to your storefront. Those rewrites are a PaaS-only feature, so re-create any active ones as rows in the EDS redirect sheet instead.
