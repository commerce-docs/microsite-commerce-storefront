---
title: Plan your storefront project
description: Learn about Adobe Commerce on Edge Delivery Services project requirements, drop-in components, and storefront migration strategy.
---
...
In the [create your storefront](/get-started/) tutorial, you learned how to quickly create a new project based on the <Link href={"https://github.com/hlxsites/aem-boilerplate-commerce"} text={"Commerce boilerplate template"} />. The boilerplate provides a starter storefront that uses a pre-configured Adobe Commerce environment.
...
The discovery phase is important because Adobe Commerce is a highly customizable platform with a large third-party extension ecosystem. Migrating an Adobe Commerce storefront to Edge Delivery Services is similar to migrating to a headless storefront (like <Link href={"https://developer.adobe.com/commerce/pwa-studio"} text={"PWA Studio"} />).
...
Drop-in components are reusable components that define the storefront shopping experience. They are framework agnostic and can be used in any context (Edge Delivery Services, AEM, Luma). However, this documentation focuses on the use of drop-in components in Edge Delivery Services projects using the <Link href={"https://github.com/hlxsites/aem-boilerplate-commerce"} text={"boilerplate template"} />. The drop-in component development roadmap is synchronized with Adobe Commerce APIs, so new API features become available in drop-in components as Adobe ships them. See [Boilerplate updates](/boilerplate/updates/) for how to pull new drop-in versions into your project.

:::note
See the [drop-in components overview](/dropins/all/introduction/) for a list of all available drop-in components.
:::
...
- Reach out to the Adobe team early to share your use cases and get recommendations on how to fulfill them: <Link href={"mailto:commerce-storefront-compatibility@adobe.com"} text={"commerce-storefront-compatibility@adobe.com"} />

The Product Discovery drop-in component covers both the quick search popover and the product listing page (PLP). It installs like any other drop-in component, using scoped [`SearchResults`](/dropins/product-discovery/containers/search-results/) containers: one scoped to a popover for quick search, and one for the full PLP. Because you own the containers directly, you have full control over styling, layout, and behavior for both experiences.

See the [Product Discovery drop-in](/dropins/product-discovery/) for its containers, functions, and events.
...
- Do the extensions expose an API to access data (for example, a product labels module from <Link href={"https://commercemarketplace.adobe.com/"} text={"Adobe Commerce Marketplace"} />)? If not, create an action item to expose the required data through an API. Options include:
  ...
  - Use <Link href={"https://developer.adobe.com/graphql-mesh-gateway/"} text={"API Mesh for Adobe Developer App Builder"} />
...
| **Storefront Bridge Options**   | Plan to connect existing storefront with the new one (e.g., using [Luma Bridge](/setup/discovery/luma-bridge/)).          | Not applicable.                                                                                    |
...
- **Storefront bridge options**: Plan to connect the existing storefront with the new one. If you are using Adobe Commerce native storefront with Luma, you can use the [Luma Bridge](/setup/discovery/luma-bridge/).
