...
Turn the discovery step above into concrete tasks:

- **[Pre-onboarding checklist](/setup/pre-onboarding/)** — Confirm Experience Cloud, Commerce Cloud Manager, App Builder, and GitHub access for Adobe Commerce as a Cloud Service and Adobe Commerce Optimizer.
- **[Data export validation](/setup/discovery/data-export-validation/)** — Confirm SaaS data sync and catalog APIs before you build on storefront services.
- **[Luma Bridge](/setup/discovery/luma-bridge/)** — Use a phased approach when you keep Luma for cart, checkout, or account and use Edge Delivery Services for catalog and content.

## Project scope

The discovery phase is important because Adobe Commerce is a highly customizable platform with a large third-party extension ecosystem. Migrating an Adobe Commerce storefront to Edge Delivery Services is similar to migrating to a headless storefront (like [PWA Studio](https://developer.adobe.com/commerce/pwa-studio)).
