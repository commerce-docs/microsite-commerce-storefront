...
* [ ] Ensure that the `environment` analytics config parameter is set appropriately in your [Commerce configuration](/setup/configuration/commerce-configuration). Use the JSON strings `"Testing"` (development) and `"Production"` (production) in `config.json`. See [Storefront event collection](/setup/analytics/instrumentation/).
