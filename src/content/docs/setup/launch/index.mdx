...
* [ ] In the storefront configuration, confirm `adobe-commerce-optimizer` is `true`, `commerce-endpoint` targets the production Optimizer GraphQL endpoint (or API Mesh when Mesh fronts Optimizer), and `headers.cs.AC-view-ID` holds the production catalog view ID from Optimizer.
...
* [ ] Record the P1 support hotline number: US (+1) 800-497-0335, then press 6 for Commerce.
