...
4. **Blocks**
   ...
   - **Commerce Blocks**: Extend AEM Blocks with Commerce Drop-in Components. They use the same authoring model but dynamically render client-side UI based on configuration provided in the authored tables.

For a detailed breakdown of the file structure and where to find specific files, see the [Explore the Boilerplate](/get-started/boilerplate-project) page.
