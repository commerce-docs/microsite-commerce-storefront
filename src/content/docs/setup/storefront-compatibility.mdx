...
* Added the `guestOrder` and `guestOrderByToken` queries, which retrieve information about guest orders. `The guestOrder` query retrieves order details based on number, email and billing last name. The `guestOrderByToken` query retrieves order details based on an order token.
