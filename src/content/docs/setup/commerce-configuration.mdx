...
Within the Commerce block's `decorate` function, use the `getConfigValue` function to set the **Services endpoint** and **GraphQL headers** for the drop-in block.
...
That's all it takes to connect a drop-in Commerce block to your Commerce backend settings. You'll use the same configuration keys to connect any custom Commerce blocks or new Commerce blocks provided in later releases.
