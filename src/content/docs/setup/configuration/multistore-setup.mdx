...
**Placeholders**: JSON files that define reusable variables for text and UI components. Each drop-in component has its own placeholders file—`cart.json`, `checkout.json`, `pdp.json`. These files replace variables in your content files with text values. For translating these placeholder files and other content, see [Content localization](/merchants/multistore/content-localization/).
...
2. **Create the Canadian English folder**:
   - Select "New folder"
   - Name it `en-ca` (or your preferred naming convention like `en_ca`)
...
You can name folders anything you want, as long as they're unique and can be used in web addresses. Choose between hyphens (`en-ca`) or underscores (`en_ca`) and be consistent. Exclude the newly created store view folders when copying content to avoid nested duplicates.
</Aside>
...
**Important:** The storefront will automatically update links relative to the storefront's root path. For links you don't want to be localized (such as store switcher links), add the hash `#nolocal` to ensure these remain absolute to the site.
...
Once your infrastructure is set up, prepare content for each locale.

<Steps>
1. **Translate or localize content files** in the `/en-ca/` and `/fr-ca/` folders.

2. **Update placeholder JSON files** with correctly translated labels. Update each component's placeholders file as needed—`cart.json`, `checkout.json`, `pdp.json`.

3. **Use automated translation tools**: For guidance on using da.live's Translation app to automate content translation, see [Content localization](/merchants/multistore/content-localization/).
</Steps>
...
### Folder structure
- Use clear, consistent naming conventions (`en-ca`, `fr-ca` or `en_ca`, `fr_ca`)
...
- Test loading times from target markets

---

## Related resources

- [Content localization](/merchants/multistore/content-localization/) - Automated translation using da.live
