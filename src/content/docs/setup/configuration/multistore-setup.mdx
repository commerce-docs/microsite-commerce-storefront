...
Each store or locale is defined in configuration files, specifying which headers to send to the commerce backend, which content folders to use, and how to structure localized content. The content hierarchy follows <Link href={"https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/edge-delivery/overview"} text={"Edge Delivery Services"} /> best practices, emphasizing a single-tier structure.
