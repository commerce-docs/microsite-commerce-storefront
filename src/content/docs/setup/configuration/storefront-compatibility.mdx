...
* Fixed an Internal Server Error when selecting the second shipping method with a US address during checkout.

* Removed exposure of `Customer Groups`, `Customer Segments`, and `Cart Price Rules` information through GraphQL.
...
* Fixed issue where `customer segment` based cart price rules didn’t apply immediately. 
