...
description: Comprehensive guide to debugging and resolving CORS issues with Adobe Commerce GraphQL endpoints, including common errors, testing strategies, and production best practices.
---
...
This guide combines information from standard CORS specifications (<Link href="https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS" text="MDN Web Docs" />), Adobe Commerce best practices, and real-world troubleshooting scenarios.
</Aside>

## CORS configuration

If using a CORS module with admin UI configuration (such as [graycore/magento2-cors](https://github.com/graycoreio/magento2-cors)), navigate to its settings in the Adobe Commerce admin panel. The following configuration options are typically available:

<Aside type="note" title="Module installation issues">
For installation troubleshooting specific to a CORS module (Composer conflicts, module enable failures, version issues), consult that module's documentation.
</Aside>
...
<Aside type="tip" title="When to use a wildcard">
...
For single production domains, use specific allowed origins instead. Note that a wildcard `*` cannot be used when `Allow Credentials` is enabled - browsers will block the request.
</Aside>
...
Configure VCL to route `/graphql` requests to your Commerce backend while serving storefront assets from the CDN edge. This requires Fastly/VCL expertise and is typically handled by the DevOps teams.
