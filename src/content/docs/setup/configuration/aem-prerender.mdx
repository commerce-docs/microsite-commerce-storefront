...
1. Your `product-details` block then enhances the pre-rendered HTML with interactive features powered by the PDP drop-in.
</Steps>

<Aside type="note" title="Pre-read: Bring Your Own Markup">
For more on how overlay content works: The prerender solution uses Edge Delivery Services' <Link href="https://www.aem.live/developer/byom" text="Bring Your Own Markup (BYOM)" /> API to publish HTML content. Understanding how BYOM works helps clarify how prerendered content integrates with your storefront. The prerender app publishes HTML files that Edge Delivery Services serves as overlay content, enhancing your existing pages with prerendered product data.
</Aside>
...
4. **Obtain required credentials**:
   - **AEM Admin API token**: To obtain your token:
     1. Login to the <Link href="https://www.aem.live/docs/admin.html#setting-up-authentication-for-admin-api" text="AEM Admin API" /> (select the link from your preferred Identity Provider; the ones suffixed by "_sa" allow you to select a specific account rather than the one currently logged in)
