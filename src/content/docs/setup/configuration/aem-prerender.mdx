...
The AEM Commerce Prerender solution enables your Edge Delivery Services Commerce Storefront to serve data-hydrated product page markup to search engines and AI crawlers _before_ JavaScript executes. This can improve SEO rankings and make your product catalog accessible to LLMs.

## How it works

<Steps>
1. Deploy the App Builder app first, because your storefront integration depends on the HTML it generates. The app runs continuously in Adobe's cloud, monitoring your catalog and generating descriptive HTML for each product.
1. That HTML is published to Edge Delivery Services as overlay content and included as a portion of the initial page HTML served to visitors.
1. Your `product-details` block then enhances the prerendered HTML with interactive features powered by the PDP drop-in.
</Steps>

<Aside type="note" title="Prerequisite: Bring Your Own Markup">
The prerender solution uses Edge Delivery Services' <Link href="https://www.aem.live/developer/byom" text="Bring Your Own Markup (BYOM)" /> API to publish HTML content. Understanding BYOM helps clarify how prerendered content integrates with your storefront. The prerender app publishes HTML files that Edge Delivery Services serves as overlay content, enhancing your existing pages with prerendered product data.
</Aside>
...
6. Store customers access the prerendered product details pages.
...
The prerender solution reflects whatever Catalog Service returns. The system itself applies no filtering based on product status, visibility, or stock level — a product is prerendered if and only if Catalog Service includes it in search results and it has a valid URL key.

### Draft product preview

The prerender system has no concept of "draft" status. It renders every product that Catalog Service returns, regardless of whether that product is enabled, visible, or in stock. Whether a draft or inactive product appears in the prerendered site therefore depends entirely on your Commerce and Catalog Service configuration — not on any filtering in the prerender system.

What you can preview independently of product status is the **product template** itself: the BYOM template document that provides the page structure for all product pages. Open it in your authoring environment (DA, UE, or another supported editor) with a `defaultSku` configured to review the layout and content before any products go live.
...
- **`check-product-changes`** — runs every 5 minutes to detect changes and render and publish updated pages
...
4. **Obtain required credentials**:
   - **AEM Admin API token**: To obtain your token:
     1. Log in to the <Link href="https://admin.hlx.page/login" text="AEM Admin API" /> (select the link from your preferred Identity Provider; the ones suffixed by "_sa" allow you to select a specific account rather than the one currently logged in)
   ...
   - **App Builder credentials**: To configure the credentials:
     ...
     3. Click the "Download All" button in the top-right to download the App Builder configuration file (a `.json` file)
...
export default async function decorate(block) {
  ...
  // Step 2: Retrieve the SKU from the meta tag (with URL fallback), then initialize PDP
  ...
  // Step 3: Replace prerendered HTML with interactive composed containers
...
- **Deployment errors**: If you encounter 403 errors when running `npm run deploy`, your aio cli may be authenticated to the wrong organization. Run `aio login --force` to re-authenticate and choose the correct organization.
