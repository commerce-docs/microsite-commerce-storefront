...
If your `app.config.yaml` includes a `productDependencies` array, **each entry must include `maxVersion`** or `aio app build` will fail with a validation error.
...
The `productDependencies` field is validated by the Adobe I/O CLI schema. For publicly distributed apps, Adobe App Builder documentation uses `requiredProducts` instead. Both fields serve similar purposes but may be used in different contexts. If you're using `productDependencies`, ensure `maxVersion` is included for each entry to pass validation.
</Aside>
...
**Verify:** Use the <Link href="https://prerender.aem-storefront.com" text="Management UI" /> to view the product index and confirm the SKU is listed.
