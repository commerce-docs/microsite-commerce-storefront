...
4. **Obtain required credentials**:
   - **AEM Admin API token**: To obtain your token:
     1. Log in to the <Link href="https://www.aem.live/docs/admin.html#setting-up-authentication-for-admin-api" text="AEM Admin API" /> (select the link from your preferred Identity Provider; the ones suffixed by "_sa" allow you to select a specific account rather than the one currently logged in)
