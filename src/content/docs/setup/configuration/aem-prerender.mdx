...
See the Troubleshooting section below if you encounter this error.
</Aside>
...
<Aside type="note" title="productDependencies vs requiredProducts">
The `productDependencies` field is validated against the Adobe I/O CLI schema. For publicly distributed apps, Adobe App Builder documentation references `requiredProducts` instead. Both fields serve similar purposes but can be used in different contexts. When using `productDependencies`, ensure that each entry  includes `maxVersion` to pass validation.
</Aside>
