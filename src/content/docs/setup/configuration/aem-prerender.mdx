...
| **PLP Structured Data** | Customize the `CollectionPage`, `ItemList`, and `BreadcrumbList` JSON-LD generated for category listing pages in <Link href="https://github.com/adobe-rnd/aem-commerce-prerender/blob/main/actions/plp-renderer/ldJson.js" text="actions/plp-renderer/ldJson.js" /> |
| **Category Image Selection** | By default, the PLP renderer prefers the category image with the `BASE` role, falling back to the first available image. Customize this logic in <Link href="https://github.com/adobe-rnd/aem-commerce-prerender/blob/main/actions/plp-renderer/render.js" text="actions/plp-renderer/render.js" /> |
| **PLP Templates** | Control category listing page markup structure and styling using the Handlebars templates in <Link href="https://github.com/adobe-rnd/aem-commerce-prerender/tree/main/actions/plp-renderer/templates" text="actions/plp-renderer/templates" /> |
...
- **Long-running or stuck activations**: `pdp-renderer` and `check-product-changes` default to a 3-hour timeout and 4096 MB of memory (increased from 1 hour and 256 MB) to support larger catalogs, especially with category families configured.
  ...
  - If you customize the `timeout` for `pdp-renderer` in `app.config.yaml`, also update the matching `ACTION_TIMEOUT_MS` constant in `actions/check-product-changes/index.js` (and the equivalent value in `actions/render-all-categories/index.js` if you customize that action's timeout) — the "running" mutex that prevents overlapping executions derives its expiration from these values, so a mismatch can leave a stale lock or allow overlapping runs
