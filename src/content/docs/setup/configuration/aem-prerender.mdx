...
## PLP pre-rendering (reference example)

The steps above cover PDP pre-rendering, the solution's primary use case. A reference example that extends the same approach to product listing pages (PLPs) — pre-rendering category pages in addition to product pages — is also available.

<Aside type="note" title="Reference example, not a packaged feature">
PLP pre-rendering is provided as a reference example of how you can build PLP support into the solution if you need it — it isn't a supported, out-of-the-box drop-in. Use it as a starting point to adapt to your own storefront rather than installing it as-is.
</Aside>

See the <Link href="https://github.com/adobe-rnd/aem-commerce-prerender/pull/267" text="PLP reference example pull request" /> for the implementation, including:

- A `render-all-categories` action and poller that pre-render PLPs per category
- Handlebars templates for PLP markup, plus `CollectionPage` JSON-LD generation
- Category tree traversal shared with the ACO category-family support used for PDP pre-rendering

Review the pull request description for full implementation details before adapting it to your storefront.
