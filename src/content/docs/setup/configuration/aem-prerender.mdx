...
**How it works:** You deploy an App Builder application that runs continuously in Adobe’s cloud, monitoring your catalog and generating descriptive HTML for each product. This descriptive HTML is published to Edge Delivery Services and included as a portion of the initial HTML “source” to page visitors. Then, your `product-details` block enhances the pre-rendered HTML with interactive features powered by the PDP drop-in. You should deploy the App Builder app first because your storefront integration depends on the HTML it generates.
