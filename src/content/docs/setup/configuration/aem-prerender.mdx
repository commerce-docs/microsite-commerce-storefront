...
6. **Test the deployment**:
   - Manually invoke actions using the Management UI or via CLI commands:
     ...
     ```
   - Verify markup generation directly via the rendered page URLs:
     - Product page: `https://{namespace}.adobeioruntime.net/api/v1/web/aem-commerce-ssg/pdp-renderer/products/{urlKey}/{sku}`
...
- **Category updates aren't detected reactively.** Category entities don't expose an "updated at" timestamp through the `categories` or `categoryTree` queries, so the app can't detect category metadata changes directly. Instead, category listing pages re-render on a fixed cadence (default every 15 minutes) rather than in response to a specific change.
- **Catalog Service caps pagination per category path.** This limits pre-rendering to a maximum of approximately 10,000 products per category path.
- **No per-SKU or per-category content overrides.** The prerender solution doesn't support merging custom content into individual product or category pages (for example, showing a widget only on one category or SKU).
