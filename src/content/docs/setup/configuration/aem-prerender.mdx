...
**How they connect:** The App Builder app publishes prerendered HTML to its own storage bucket. The AEM Admin API configures the link to that bucket in your Edge Delivery Services configuration file as an overlay. The prerendered HTML is then served at URLs matching your product pages (for example, `/products/acme-widget/sku123` - note that SKUs must be lowercase in URLs). When a visitor (or crawler) requests a product page, Edge Delivery Services serves this prerendered HTML from the overlay. Your storefront JavaScript then enhances the page by replacing the static HTML with the interactive PDP drop-in.
...
2. **HTML stored in App Builder bucket** → The App Builder app stores the HTML files in its own storage bucket
3. **Overlay configured in EDS** → The AEM Admin API configures the link to the App Builder bucket in your Edge Delivery Services configuration file as an overlay
