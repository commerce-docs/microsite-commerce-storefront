...
### Working with demo-config.json

The demo-config.json file is deprecated and will be removed in a future version. Use the [config generator tool](https://da.live/app/adobe-commerce/storefront-tools/tools/config-generator/config-generator) to generate a `config.json` file for your Commerce backend. Be sure update with necessary values and validate.
