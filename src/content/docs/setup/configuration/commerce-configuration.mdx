...
- **`cs`** - Headers applied to all GraphQL requests
  - **`AC-Environment-ID`** - (Optional) The instance ID for the Adobe Commerce Optimizer Instance. This header is supported to maintain backward compatibility with earlier releases. The value is resolved automatically, so you do not need to specify it.
