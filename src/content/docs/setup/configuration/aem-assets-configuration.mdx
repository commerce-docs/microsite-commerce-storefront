...
## Boilerplate updates required

Only one update is required: set `"commerce-assets-enabled": true` in your `config.json` file, renamed from `/demo-config.json`. See [Storefront configuration](/setup/configuration/commerce-configuration/) for the full configuration model.
...
The Commerce drop-ins automatically detect the `commerce-assets-enabled` configuration and adjust image handling accordingly. See the [demo-config.json](https://github.com/hlxsites/aem-boilerplate-commerce/blob/main/demo-config.json) file in the boilerplate repository. 
...
<Aside type="tip" title="Compatibility notes">

- Commerce-hosted images continue to work when AEM Assets is enabled.
- If you're not using the boilerplate, add AEM support in the drop-in image slots with `tryRenderAemAssetsImage`.
...
If you use the boilerplate, AEM Assets works out of the box. Without the boilerplate, implement the minimal config and slot usage below. See the [Commerce drop-ins overview](/dropins/all/introduction/) for how slots and initializers work.

<Aside type="caution" title="Standalone drop-ins without the boilerplate">
When you use drop-ins without the boilerplate, there is no built-in configuration loader. You must provide the `commerce-assets-enabled: true` flag via your own public config (or config service) and render images with AEM support in each drop-in image slot using `tryRenderAemAssetsImage`. See [Minimal configuration](#minimal-configuration) and [Minimal integration steps](#minimal-integration-steps) below.
</Aside>
