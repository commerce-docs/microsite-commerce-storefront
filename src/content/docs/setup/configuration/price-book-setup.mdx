...
This guide is specifically for **Adobe Commerce Optimizer (ACO)** implementations. If you are using Adobe Commerce as a Cloud Service (ACCS) or Adobe Commerce PaaS without Optimizer, this feature does not apply to your setup. ACCS uses customer groups and shared catalogs for pricing instead of Price Book IDs. See the <Link href="/setup/configuration/commerce-configuration/" text="Storefront configuration" /> documentation for configuration options for your Commerce environment.
</Aside>
