...
For example, if your storefront is running on `http://localhost:3000` and your Adobe Commerce backend is running on `https://commerce.example.com`, you will need to configure CORS to allow requests from the storefront to the backend. This is a common scenario when using Edge Delivery Services (EDS) to serve the storefront and Adobe Commerce backend from different domains.

:::caution
**Production recommendation**: CORS should be a last resort for production. Prefer same-origin delivery of the storefront and API (for example, via Fastly VCL) to avoid CORS entirely.
