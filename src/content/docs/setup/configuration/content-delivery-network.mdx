...
We also re-apply non-normalized `Accept-Encoding` headers. For details on why, see `miss` and `pass` snippets [here](https://www.aem.live/docs/byo-cdn-fastly-setup#create-vcl-snippets).
...
if (req.backend == F_edge_delivery) {
    ...
    if (bereq.http.Fastly-Orig-Accept-Encoding) {
      set bereq.http.Accept-Encoding = bereq.http.Fastly-Orig-Accept-Encoding;
    }
}
