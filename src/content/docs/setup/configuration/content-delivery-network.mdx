...
Some Adobe Commerce on Cloud projects are entitled to only a single Fastly service but need to serve two or more independent domains — for example, when each domain is migrating to Edge Delivery Services as its own site (separate GitHub repository and content source) rather than as a single [multistore](/setup/configuration/multistore-setup/) codebase. You can serve multiple Edge Delivery Services sites from the same shared Fastly service.

:::note
This differs from [multistore](/setup/configuration/multistore-setup/), which uses a single Edge Delivery Services site (one codebase and content repository) to serve multiple domains. Use the guidance below when each domain is backed by a **separate** Edge Delivery Services site.
:::
