...
Some Adobe Commerce on Cloud projects are entitled to only a single Fastly service but need to serve two or more independent domains. An Edge Delivery Services site is a configuration entry — a production host plus a push invalidation configuration — so each domain needs its own Edge Delivery Services site, whether or not those sites point to the same code and content. You can serve multiple Edge Delivery Services sites from the same shared Fastly service.

:::note
This differs from [multistore](/setup/configuration/multistore-setup/), which serves multiple stores or locales from a single Edge Delivery Services site (one codebase and content repository) — typically via subfolders on one domain. Use the guidance below when each domain needs its own Edge Delivery Services site, regardless of whether those sites share the same code and content.
:::
...
As before, attach a condition set to `false` to each new backend, and confirm that same `false` condition is also applied to the default Adobe Commerce backend and to every other Edge Delivery Services backend you add. Skipping this on any backend risks one backend's condition silently overriding another's routing (including the Admin, as noted above).
...
Update the `pass` and `miss` snippets so each backend is given its own origin `Host` header:
...
Each Edge Delivery Services site registers its own push invalidation configuration (see [Edge Delivery Service configuration](#edge-delivery-service-configuration)). That configuration is stored per site, not per Fastly service, so you can register the **same** Fastly `serviceId` and purge `authToken` for each site — as long as the `host` value in each site's configuration matches that site's own production domain:
...
:::caution
Both sites share the same Fastly purge API rate limit, since purge requests for both sites go through the same `serviceId`/`authToken`. If either site publishes very frequently, monitor for throttling on the shared token.
:::
...
### Multiple Edge Delivery Services sites (cross-site isolation)

If you are running [multiple Edge Delivery Services sites on one Fastly service](#multiple-edge-delivery-services-sites-on-one-fastly-service), confirm that publishing on one site does not purge or otherwise affect the other site's cache:
