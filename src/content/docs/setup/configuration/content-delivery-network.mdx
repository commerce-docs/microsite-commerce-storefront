...
Drop-in components automatically append image optimization parameters to image URLs (for example, width, height, format). If necessary, you can override these parameters with the [`setImageParamKeys`](/sdk/reference/initializer/#setimageparamkeysparams) in the initializer configuration.
</Aside>
