...
To enable proper event collection, you need to configure your store's analytics settings. This configuration tells the instrumentation system about your store's identity and structure. Refer to the <Link href="https://experienceleague.adobe.com/developer/commerce/storefront/setup/configuration/commerce-configuration/" text="Commerce Configuration documentation" /> for more information.
...
- **Shopping events**: Cart updates and views (`addToCart`, `removeFromCart`, `shoppingCartView`), page views (`pageView`, `productPageView`), checkout (`startCheckout`, `completeCheckout`), purchase (`purchase`) and more.
- **Customer profile events**: Customer login (`signIn`), customer logout (`signOut`), create account (`createAccount`), edit account (`editAccount`) and more.
- **Search events**: Search queries and results: Search query (`searchRequestSent`), search results (`searchResponseReceived`).
