...
To configure variables for your Commerce backend, update the `analytics` object located in either your site config or a `config.json` in your repository. It should resemble the `analytics` property on https://aemshop.net/config.json.
...
For performance considerations, Adobe highly recommends writing events to the ACDL directly without using the Storefront Events SDK. Drop-in components will send events to ACDL out of the box, but you should double-check the correctness of all events.
</Aside>
