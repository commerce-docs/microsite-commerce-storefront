...
If existing drop-ins don't cover a feature but you don't want to own a whole new drop-in package, see [Composing blocks](/dropins/all/compose-a-block/) — a lighter-weight pattern that combines an existing drop-in's API function, shared SDK components, and the event bus inside a custom commerce block.
</Aside>
