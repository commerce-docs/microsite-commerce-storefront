...
Next, we need to render the `GiftOptionsField` component into the checkout page by creating the `gift-options-field` <Link href={"https://developer.mozilla.org/en-US/docs/Web/API/Web_components/Using_custom_elements"} text={"custom element"} />.
...
Get the data from the cart event and use it to populate the gift message fields on the checkout page. See [How drop-ins coordinate](/get-started/architecture/drop-ins-on-a-page/) if you're not yet familiar with how the event bus works. Here's an example of how you might do this:
...
- Familiarity with <Link href={"https://www.aem.live/docs/configuration"} text={"project configurations"} />.

### What you'll learn

- How to configure third-party API keys for use in drop-ins.
- How to use the [event bus](/get-started/architecture/drop-ins-on-a-page/) to emit events and listen for events from the third-party component.
...
The steps above are a plan an AI coding agent can build from: which drop-in and fragment to extend, what UI to add, and how to wire the new field into an initializer's model transformer. Install [Boilerplate skills](/ai/boilerplate-skills/) so your agent follows this project's real conventions — actual fragment names, model transformers, and event payloads — instead of inventing its own.
