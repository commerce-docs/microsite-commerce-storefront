...
- New **`autoSync` property added** for all containers: This property allows the container automatically synchronizing its state changes with the backend.
...
- The `ShippingMethods` container configuration structure has been modified. The `ShippingMethodsProps` interface has been changed:
  ...
  Implemented an error handling for the `ShippingMethods` container based on the _"Optimistic" UI updates with rollback pattern_ consisting on the following:

    - It provides a callback function **`onSelectionError()`** to be used in the integration layer in case the merchant wanted to perform some actions like showing an inline error messages etc.
