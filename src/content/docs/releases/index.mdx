...
Visit the [Title and header customization](/dropins/checkout/containers/overview/#title-and-header-customization) documentation for more information.

New `Newsletter` container added.

Visit the [Newsletter](/dropins/checkout/containers/newsletter) documentation for more information.
...
- **`PaymentMethodsProps.UIComponentType` property added**: This property allows using a different UI component type as selector for each payment method; UI components available are: 'ToggleButton' and 'RadioButton'.
...
The `ShippingMethods` container configuration structure has been modified. `ShippingMethodsProps` interface is extending from `TitleProps` interface inheriting the following properties:
