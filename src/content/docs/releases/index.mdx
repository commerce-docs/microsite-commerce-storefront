...
This suite ships Elsie 1.8 and `@dropins/tools` 1.8 across B2C and B2B boilerplate merges, adds native product video on the Product Details Page block, extends checkout shipping customization, and brings B2B Quick Order plus Storefront Compatibility Package updates. For per-component version details, see the [changelog](/releases/changelog/).

<Aside type="caution" title="Breaking changes">

- **Product Recommendations (B2C)**: The B2C boilerplate moves to **v4.0.1**. Treat this as a **major** upgrade from the 3.x line. Review the [Product Recommendations](/dropins/recommendations/) documentation and the [v4.0.0](https://github.com/adobe-commerce/storefront-recommendations/releases/tag/v4.0.0) and [v4.0.1](https://github.com/adobe-commerce/storefront-recommendations/releases/tag/v4.0.1) release notes before upgrading.
- **Product Discovery and URLs**: Search and category URL handling now lives primarily in the boilerplate Product List Page block, and filter changes reset pagination in the drop-in ([#1119](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1119)). Retest any custom URL or routing logic that assumed the previous drop-in-only behavior.
- **Requisition List (B2B)**: Version **1.2.0** replaces built-in catalog API helpers with **props your integration must inject**. Plan updates to any custom Requisition List wiring before upgrading.

</Aside>

### Highlights

#### Native product video on the Product Details Page

Merchants can show product video in the gallery without building a custom media pipeline. The boilerplate Product Details block wires the new Product Gallery behavior from the Product Details Page drop-in ([#1100](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1100)). Developers keep full control through existing slots and configuration.

#### Checkout shipping you can own end to end

Checkout adds a **ShippingMethodItem** slot so you can replace how each shipping method renders while still driving selection through the drop-in APIs. GraphQL extensibility for **estimateShippingMethods** includes **EstimateShippingModel** and **ESTIMATE_SHIPPING_METHOD_FRAGMENT**, plus a **checkout/shipping-methods-render** hook for extension samples shipped in the boilerplate. Teams that customize shipping UX get clearer extension points without forking core UI.

#### Multi-store wishlists and cleaner catalog URLs

Wishlist storage keys and cookies can scope per store view when you pass **storeCode**, so multi-store deployments stop leaking guest wishlists across stores. Product Discovery improves pathname handling when the store code appears in the path and resets listing pagination when filters change, which reduces dead ends on localized URLs ([#86](https://github.com/adobe-commerce/storefront-search-dropin/pull/86), [#88](https://github.com/adobe-commerce/storefront-search-dropin/pull/88), [#91](https://github.com/adobe-commerce/storefront-search-dropin/pull/91)). Together with boilerplate URL handling ([#1119](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1119)), shoppers land on the right list and page after search and filter actions.

#### B2B Quick Order and compatibility hardening

The B2B boilerplate adds **@dropins/storefront-quick-order@~1.0.0** so buyers can use the [Quick Order](/dropins-b2b/quick-order/) drop-in on Edge Delivery storefronts. Storefront Compatibility B2B **1.0.19** adds requisition list sharing and PHP 8.5 support paths, while Storefront Compatibility **4.8.18** fixes guest order cancel responses when order comments are present.

### Updated boilerplate

#### Infrastructure

- **Drop-in SDK and build tools**: `@dropins/tools` moves to **~1.8.0** and `@dropins/build-tools` to **1.1.0** on both boilerplate branches merged for this suite ([#1152](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1152), [#1156](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1156)).
- **Site configuration cleanup**: Removed legacy folder-mapping references from default site metadata ([#1148](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1148)).
- **Demo defaults**: Demo configs default assets on and set the Adobe Commerce Optimizer flag for ACO-focused demos ([#1114](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1114), [#1124](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1124)).
...
- **Search and category URLs in the boilerplate**: Product List Page block now owns URL construction and sync with the Product Discovery drop-in, including default sort and visibility handling ([#1119](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1119)).
- **Layout columns**: Full-width sections and padding-based gutters improve responsive column layouts ([#1133](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1133)).
- **Product Details block**: Example integration for native PDP video and tighter validation when product payloads are incomplete ([#1100](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1100)).
- **B2B wishlist deep links (B2B)**: Wishlist alert links use `rootLink` so store codes in the path resolve correctly ([#1130](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1130)).
- **B2B website switches (B2B)**: Cart state clears when shoppers change websites so stale carts cannot trigger 403 responses ([#1137](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1137)).
...
**Elsie 1.8.0** (published as `@adobe-commerce/elsie@1.8.0`) is the runtime behind `@dropins/tools@~1.8.0` in this suite. Drop-in packages pick up coordinated fixes and extension patterns, including the checkout shipping hooks and fragments noted above. See the [@adobe-commerce/elsie@1.8.0](https://github.com/adobe-commerce/StorefrontSDK/releases/tag/%40adobe-commerce%2Felsie%401.8.0) release on GitHub for the full change log.

### Updated B2C drop-in components

| Component | Improvements |
|-----------|--------------|
| [**Cart**](/dropins/cart/) v3.2.0 | `includeOutOfStockItems` on **CartSummaryList**; Elsie 1.8 alignment |
| [**Checkout**](/dropins/checkout/) v3.2.0 | **ShippingMethodItem** slot; **EstimateShippingModel** and **ESTIMATE_SHIPPING_METHOD_FRAGMENT** for `estimateShippingMethods`; **checkout/shipping-methods-render** hook and extension sample |
| [**Order**](/dropins/order/) v3.2.0 | Exported **GUEST_ORDER_FRAGMENT** and related fragments for GraphQL extensions; cancellation flow distinguishes guest versus authenticated paths |
| [**Product Details Page**](/dropins/product-details/) v3.0.2 | Native **ProductGallery** video support; **`getProductsData`** API for catalog lookups; price display fix for Adobe Commerce Optimizer and PaaS ([#56](https://github.com/adobe-commerce/storefront-pdp/pull/56), [#58](https://github.com/adobe-commerce/storefront-pdp/pull/58)) |
| [**User Account**](/dropins/user-account/) v3.2.0 | **`orderTime`** on **OrderModel** with **OrdersListOrderTime** slot; CustomerOrder fragment extensibility |
| [**User Auth**](/dropins/user-auth/) v3.2.0 | Title, Form, and Buttons slots across containers; missing Reset Password slot props restored |
| [**Wishlist**](/dropins/wishlist/) v3.2.0 | Optional **`storeCode`** with per-store storage keys and cookies for multi-store isolation |
| [**Personalization**](/dropins/personalization/) v3.1.1 | Provider merges custom **`langDefinitions`** with bundled defaults |
| [**Product Discovery**](/dropins/product-discovery/) v3.1.0 | Filter changes reset to page 1; pathname fixes when store codes appear in URLs ([#86](https://github.com/adobe-commerce/storefront-search-dropin/pull/86), [#88](https://github.com/adobe-commerce/storefront-search-dropin/pull/88), [#91](https://github.com/adobe-commerce/storefront-search-dropin/pull/91)) |
| [**Product Recommendations**](/dropins/recommendations/) v4.0.1 | **Major** 4.x line for B2C; title rendering fix ([#36](https://github.com/adobe-commerce/storefront-recommendations/pull/36)) |
| [**Payment Services**](/dropins/payment-services/) v3.0.x | B2C boilerplate pins `@dropins/storefront-payment-services@~3.0.0` on the **3.0.x** line with Elsie 1.8 |

### Updated B2B drop-in components

| Component | Improvements |
|-----------|--------------|
| [**Company Management**](/dropins-b2b/company-management/) v1.2.0 | Initializer matches other drop-ins; Provider merges **`langDefinitions`**; Elsie 1.8 |
| [**Company Switcher**](/dropins-b2b/company-switcher/) v1.1.1 | Elsie 1.8; Provider merges **`langDefinitions`** |
| [**Purchase Order**](/dropins-b2b/purchase-order/) v1.1.1 | Elsie 1.8; Provider merges **`langDefinitions`** |
| [**Quote Management**](/dropins-b2b/quote-management/) v1.1.2 | Public package metadata on npm; carries 1.1.1 **`langDefinitions`** merge and Elsie 1.8 work |
| [**Requisition List**](/dropins-b2b/requisition-list/) v1.2.0 | Catalog integration moves to **injected props** instead of built-in catalog helpers (**breaking** for custom callers) |
| [**Quick Order**](/dropins-b2b/quick-order/) v1.0.0 | First **1.0.0** integration on the B2B boilerplate for CSV, multi-SKU, and variant-grid flows |
