...
This suite includes stored payment methods in My Account, order comments for B2B, Product Recommendations v4 with catalog view support, and improvements to User Account, Checkout, Payment Services, Requisition List, and Quick Order. For per-component version details, see the [changelog](/releases/changelog/).

<Aside type="caution" title="Breaking changes">
The Product Recommendations drop-in has been updated to a new major version (v4). The drop-in now uses the `getRecommendationsUnitsByIds` GraphQL query instead of the legacy `getRecommendations` query. If you call `getRecommendations` directly, update your integration to use the new API.
</Aside>
...
Shoppers can now view and remove saved payment methods directly from their account page. The new [`PaymentMethods`](/dropins/user-account/) container in the User Account drop-in lists stored cards with brand name, masked card number, and expiry status. Removing a card opens a confirmation modal that shows a card preview and prevents accidental deletion ([#57](https://github.com/adobe-commerce/storefront-account/pull/57), [#58](https://github.com/adobe-commerce/storefront-account/pull/58)).

#### Order Comments for B2B

Store administrators can add order-level comments in Commerce Admin, and B2B shoppers now see those comments — timestamp and message — directly in the order details view. The new [`OrderComments`](/dropins/order/) container in the Order drop-in fetches comments for both guest and authenticated users. Add the new `commerce-order-comments` boilerplate block to any order details page to enable this view ([#68](https://github.com/adobe-commerce/storefront-order/pull/68), [#87](https://github.com/adobe-commerce/storefront-order/pull/87), [#1210](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1210)).

#### Product Recommendations v4 — catalog view support

The [Product Recommendations](/dropins/recommendations/) drop-in has been updated to a new major version. It now uses the `getRecommendationsUnitsByIds` GraphQL query, which correctly respects catalog view scoping. This means recommendation units configured globally work as expected in multi-catalog and multi-store setups. See the [Recommendations API](/dropins/recommendations/functions/) for the updated function ([#34](https://github.com/adobe-commerce/storefront-recommendations/pull/34), [#1183](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1183)).

#### Requisition List enhancements

B2B users can now move or copy items between requisition lists without leaving the page, making it faster to reorganize products across procurement lists. The Add to List button on the product details page also shows an active state when the current product is already in the selected list ([#116](https://github.com/adobe-commerce/storefront-requisition-list/pull/116), [#127](https://github.com/adobe-commerce/storefront-requisition-list/pull/127)).

#### Checkout vault payment method support

The [Checkout](/dropins/checkout/) drop-in now passes the full `additionalData` payload when a payment method is selected, enabling Payment Services vault and other payment methods that require extra mutation fields to work correctly during checkout ([#311](https://github.com/adobe-commerce/storefront-checkout/pull/311)).
...
- **Product Recommendations block**: Block updated to use the Recommendations drop-in v4 ([#1183](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1183)).
- **B2B (Order Comments)**: New `commerce-order-comments` block for displaying order-level comments on B2B order details pages.
- **B2B (Quick Order)**: Quick Order block updated with improved configurable product Add to Cart handling and standardized event payloads.

### Updated Drop-in SDK

The Drop-in SDK (`@dropins/tools`) was upgraded to ~1.8.1 in this release. The upgrade adds reCAPTCHA Enterprise support for storefront forms and fixes incorrect `srcset` attribute generation on product images.
...
| [**Product Details Page**](/dropins/product-details/) v3.0.3 | New `formatValue` prop on [`ProductAttributes`](/dropins/product-details/containers/product-attributes/) for custom numeric attribute formatting ([#65](https://github.com/adobe-commerce/storefront-pdp/pull/65)). |
| [**User Account**](/dropins/user-account/) v3.3.0 | New `PaymentMethods` container with stored card listing and delete-with-confirmation modal. New `PaymentCard` component. `fieldIdPrefix` on `Addresses` now correctly forwarded. Customer dropdown custom attributes prefill correctly from the API ([#51](https://github.com/adobe-commerce/storefront-account/pull/51), [#55](https://github.com/adobe-commerce/storefront-account/pull/55), [#56](https://github.com/adobe-commerce/storefront-account/pull/56), [#57](https://github.com/adobe-commerce/storefront-account/pull/57), [#58](https://github.com/adobe-commerce/storefront-account/pull/58)). |
...
| [**Quick Order**](/dropins-b2b/quick-order/) v1.0.1 | Consistent Add to Cart for configurable products. Standardized event payloads across Quick Order and Grid Ordering ([#25](https://github.com/adobe-commerce/storefront-quick-order/pull/25)). |
</TableWrapper>
