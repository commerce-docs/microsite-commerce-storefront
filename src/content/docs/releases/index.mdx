...
This suite delivers B2C and B2B boilerplate updates through the Suite7 and B2B Suite3 release merges ([#1152](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1152), [#1156](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1156)). Developers gain deeper checkout extensibility, clearer multi-store routing and wishlist behavior, optional PDP video in the boilerplate, and a first-class Quick Order path on the B2B branch. The Drop-in SDK (Elsie) moves to 1.8.0 together with `@dropins/build-tools` 1.1.0 so generated projects pick up safer GraphQL override tooling. For per-component version details, see the [changelog](/releases/changelog/).

<Aside type="caution" title="No breaking changes">
No breaking API or configuration changes from the February 2026 suite.
</Aside>

### Highlights

#### Checkout extensions you can wire without forking the drop-in

Checkout now ships extension demos and hooks that mirror how merchants extend Luma. Address autocomplete and buy-online-pick-up-in-store extensions land in the `commerce-checkout-with-extensions` sample, `getShippingMethodExtras` and a `ShippingMethodItem` slot let you badge or fully replace shipping rows, and GraphQL authors can extend `estimateShippingMethods` through a dedicated fragment plus shipping-method checkout hooks ([#299](https://github.com/adobe-commerce/storefront-checkout/pull/299), [#300](https://github.com/adobe-commerce/storefront-checkout/pull/300), [#307](https://github.com/adobe-commerce/storefront-checkout/pull/307), [#305](https://github.com/adobe-commerce/storefront-checkout/pull/305), [#303](https://github.com/adobe-commerce/storefront-checkout/pull/303)). The checkout-with-extensions block in the boilerplate shows how to register those extensions with the SDK extension manager.

#### Multi-store routes, wishlists, and carts that stay in the right store context

URL handling for search and category pages now lives in the boilerplate so store codes stay correct when shoppers filter, sort, or paginate ([#1119](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1119)). Product Discovery fixes category routing when a store code appears in the path ([#88](https://github.com/adobe-commerce/storefront-search-dropin/pull/88), [#91](https://github.com/adobe-commerce/storefront-search-dropin/pull/91)). Wishlist storage and cookies can scope per store view, and the boilerplate passes that context through ([#73](https://github.com/adobe-commerce/storefront-wishlist/pull/73), [#1139](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1139)). B2B shoppers see wishlist links with the right store prefix, and cart state clears when switching websites so authorization errors do not linger ([#1130](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1130), [#1137](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1137)). Non-existent storefront URLs now return a real 404 response instead of a client-side dead end ([#1134](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1134)).

#### PDP video, configurable PLP page sizes, and Quick Order data from the PDP drop-in

The boilerplate PDP block enables carousel video alongside imagery without custom scripts ([#1100](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1100)). The Product Details Page drop-in adds option transformers plus a `getProductsData` helper so Quick Order and other bulk flows can hydrate rows from GraphQL ([#51](https://github.com/adobe-commerce/storefront-pdp/pull/51), [#56](https://github.com/adobe-commerce/storefront-pdp/pull/56)). The product listing block now reads page size from configuration so merchants can tune how many cards load per page ([#1143](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1143)). On the B2B branch, the new Quick Order block delivers parity with Luma Quick Order plus grid ordering for configurable products ([#1125](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1125)).

#### Language overrides and invoices that respect your GraphQL extensions

Several drop-ins now merge initializer `langDefinitions` with bundled defaults so placeholder and label overrides actually render ([#27](https://github.com/adobe-commerce/storefront-personalization/pull/27), [#79](https://github.com/adobe-commerce/storefront-wishlist/pull/79), [#124](https://github.com/adobe-commerce/storefront-requisition-list/pull/124), [#89](https://github.com/adobe-commerce/storefront-quote-management/pull/89), [#52](https://github.com/adobe-commerce/storefront-purchase-order/pull/52)). The User Account drop-in exports `CUSTOMER_ORDER_FRAGMENT` so you can extend customer order GraphQL through `overrideGQLOperations` (for example invoice totals on order detail and order list views) ([#46](https://github.com/adobe-commerce/storefront-account/pull/46)).

### Updated boilerplate

#### Infrastructure

- **Build tooling**: `@dropins/build-tools` is pinned to 1.1.0 and `@dropins/tools` moves to ~1.8.0 so local builds match the suite ([#1152](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1152)).
- **Default assets**: Demo configuration now defaults `assets-enabled` to true so fresh sandboxes load media helpers without extra toggles ([#1114](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1114)).
- **Site configuration**: `default-site.json` drops the unused `folders` reference to reduce noise for implementers ([#1148](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1148)).
- **Adobe Commerce Optimizer demo flag**: `demo-config-aco.json` now opts into the optimizer flag by default for teams validating ACO scenarios ([#1124](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1124)).
...
- **PDP block**: Product carousels can include video assets with the same JSON configuration workflow as images ([#1100](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1100)).
- **PLP block**: Merchants set how many products appear per page through block configuration (default remains nine) ([#1143](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1143)).
- **404 handling**: Missing pages return proper HTTP 404 responses for crawlers and monitoring tools ([#1134](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1134)).
- **Layout**: Column layouts support full-width sections and padding-based gutters so edge-to-edge sections align predictably ([#1133](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1133)).
- **URL handling**: Search and PLP URLs centralize logic in the boilerplate so filter changes reset pagination and store codes stay in sync with Document Authoring paths ([#1119](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1119)).
- **Wishlist wiring**: Boilerplate initialization passes store view context into the Wishlist drop-in for multi-website isolation ([#1139](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1139)).
- **B2B navigation**: Wishlist links from PDP and Cart include the active store code, and cart state clears when shoppers switch websites ([#1130](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1130), [#1137](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1137)).
- **Quick Order (B2B)**: The B2B branch adds the `commerce-b2b-quick-order` block, Document Authoring models, and Cypress coverage for CSV, SKU entry, and configurable grid flows ([#1125](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1125)).
...
The Drop-in SDK (Elsie) 1.8.0 release focuses on predictable GraphQL tooling for generated drop-ins:

#### New features

No new shopper-facing UI primitives ship beyond what individual drop-ins document for this suite.

#### Improvements and fixes

- **GraphQL override builds**: `skipFragments` and `operations` can target the same fragment name without producing an empty fragment parse error, which unlocks safer field replacement when you use `overrideGQLOperations` ([#178](https://github.com/adobe-commerce/StorefrontSDK/pull/178)).

### Updated B2C drop-in components

| Component | Improvements |
|-----------|--------------|
| [**Cart**](/dropins/cart/) v3.2.0 | `includeOutOfStockItems` on `CartSummaryList` keeps low-stock rows in the main list while preserving the warning banner actions ([#102](https://github.com/adobe-commerce/storefront-cart/pull/102)). |
| [**Checkout**](/dropins/checkout/) v3.2.0 | Address autocomplete and BOPIS extension samples, shipping row extras, `ShippingMethodItem` slot, `estimateShippingMethods` fragment extension, shipping checkout hooks, and checkout-with-extensions wiring ([#299](https://github.com/adobe-commerce/storefront-checkout/pull/299), [#300](https://github.com/adobe-commerce/storefront-checkout/pull/300), [#307](https://github.com/adobe-commerce/storefront-checkout/pull/307), [#305](https://github.com/adobe-commerce/storefront-checkout/pull/305), [#303](https://github.com/adobe-commerce/storefront-checkout/pull/303)). |
| [**Order**](/dropins/order/) v3.2.0 | Suite alignment on Elsie 1.8.0 with no new shopper-facing APIs beyond dependency updates. |
| [**Payment Services**](/dropins/payment-services/) v3.0.1 | Suite alignment on Elsie 1.8.0 with no new shopper-facing APIs beyond dependency updates. |
| [**Personalization**](/dropins/personalization/) v3.1.1 | Language packs passed through initialization now merge with bundled strings ([#27](https://github.com/adobe-commerce/storefront-personalization/pull/27)). |
| [**Product Details Page**](/dropins/product-details/) v3.0.2 | Option `transformer` hook, `getProductsData` bulk helper, and wishlist price handling fixes for ACO plus PaaS ([#51](https://github.com/adobe-commerce/storefront-pdp/pull/51), [#56](https://github.com/adobe-commerce/storefront-pdp/pull/56), [#58](https://github.com/adobe-commerce/storefront-pdp/pull/58)). |
| [**Product Discovery**](/dropins/product-discovery/) v3.1.0 | Category routing respects store codes in the pathname ([#88](https://github.com/adobe-commerce/storefront-search-dropin/pull/88), [#91](https://github.com/adobe-commerce/storefront-search-dropin/pull/91)). |
| [**Product Recommendations**](/dropins/recommendations/) v4.0.1 | Moves to the GraphQL `getRecommendationsUnitsByIds` query for loading recommendation units ([#34](https://github.com/adobe-commerce/storefront-recommendations/pull/34), [#35](https://github.com/adobe-commerce/storefront-recommendations/pull/35)). |
| [**User Account**](/dropins/user-account/) v3.2.1 | `CUSTOMER_ORDER_FRAGMENT` export for extending customer order queries (for example invoice totals) through `overrideGQLOperations` ([#46](https://github.com/adobe-commerce/storefront-account/pull/46)). |
| [**User Auth**](/dropins/user-auth/) v3.2.0 | Title, form, and button slots across auth forms plus missing slots on reset-password flows ([#58](https://github.com/adobe-commerce/storefront-auth/pull/58), [#64](https://github.com/adobe-commerce/storefront-auth/pull/64)). |
| [**Wishlist**](/dropins/wishlist/) v3.2.0 | Optional `storeCode` scoping for storage keys and cookies, plus merged language definitions ([#73](https://github.com/adobe-commerce/storefront-wishlist/pull/73), [#79](https://github.com/adobe-commerce/storefront-wishlist/pull/79)). |

### Updated B2B drop-in components

| Component | Improvements |
|-----------|--------------|
| [**Company Management**](/dropins-b2b/company-management/) v1.2.0 | Suite alignment on Elsie 1.8.0 with no new shopper-facing APIs beyond dependency updates. |
| [**Company Switcher**](/dropins-b2b/company-switcher/) v1.1.1 | Patch release for workflow alignment without shopper-visible changes. |
| [**Purchase Order**](/dropins-b2b/purchase-order/) v1.1.1 | Customer language definitions now merge with bundled defaults ([#52](https://github.com/adobe-commerce/storefront-purchase-order/pull/52)). |
| [**Quick Order**](/dropins-b2b/quick-order/) v1.0.0 | Initial release with CSV upload, multi-SKU entry, and configurable grid ordering integrated through the boilerplate ([#1125](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1125)). |
| [**Quote Management**](/dropins-b2b/quote-management/) v1.1.2 | Language definition merges in the provider ([#89](https://github.com/adobe-commerce/storefront-quote-management/pull/89)). |
| [**Requisition List**](/dropins-b2b/requisition-list/) v1.2.0 | Disabled selector states behave correctly in Storybook-driven flows, and initializer language overrides merge as expected ([#120](https://github.com/adobe-commerce/storefront-requisition-list/pull/120), [#124](https://github.com/adobe-commerce/storefront-requisition-list/pull/124)). |
...
#### Note on SCP versions

The package numbers above are the Composer releases Adobe tests against for each Commerce version. They come from versioned SCP branches, not from the default `main` branch of the SCP repository.
