...
- **Global Price component localization**: Implemented global internationalization (i18n) locale support for the Price component, enabling proper currency and number formatting across different regions and languages.

### Updated boilerplate

The Commerce boilerplate has been significantly enhanced with new carousel and cards-list components from Adobe Store, improved Storefront Builder compatibility, and comprehensive architecture improvements. Key highlights include major scripts.js refactoring for better maintainability, enhanced drop-in functionality across Cart, Checkout, and Wishlist components, optimized header performance, and improved accessibility with global placeholders and modal localization. Additional updates include reCAPTCHA enhancements, Product Discovery search fixes, comprehensive documentation improvements, and removal of deprecated widgets for cleaner implementation.
...
| [**Wishlist**](/dropins/wishlist/) | • Fixed accessibility issues with proper aria-label attributes<br/>• Enhanced Wishlist functionality with better ACCS support and move-to-cart flows<br/>• Added image slots for customizable image rendering<br/>• Fixed alert messaging to show accurate success/error status |
