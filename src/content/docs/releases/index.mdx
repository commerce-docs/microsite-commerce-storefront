...
- **Search functionality improvements**: Enhanced search functionality with better result handling, improved styling, and updated component architecture
- **Header optimizations**: Refined header search panel styling and improved product discovery integration
- **Code cleanup**: Removed outdated picker tool and cleaned up documentation for better maintainability
- **Architecture improvements**: Enhanced scoped functionality across components for better isolation and performance
...
- **Enhanced UI components**: Added new Table component with flexible column configuration and comprehensive test coverage. Introduced Choose file UI component for better file upload experiences with proper validation and accessibility features.
- **Button component enhancements**: Extended Button component with href and type properties, enabling better link functionality and form integration. Added support for external links and improved semantic HTML output.
- **ToggleButton improvements**: Added disabled state support to ToggleButton component, providing better user experience and form control capabilities with proper accessibility attributes.
- **Quantity Stepper refinements**: Updated Quantity Stepper component by removing increase/decrease buttons for cleaner interface design while maintaining full keyboard accessibility and screen reader support.
- **GraphQL validation tools**: Enhanced GraphQL validation CLI with exclude option, allowing developers to skip specific files or patterns during validation for more flexible development workflows.
- **Lazy slots functionality**: Implemented support for lazy slots, enabling performance optimizations by deferring slot rendering until needed, reducing initial page load times.
- **Cross-platform compatibility**: Fixed compilation issues on Windows systems, ensuring consistent development experience across all operating systems with proper path handling and dependency resolution.
- **API method additions**: Added getFetchGraphQlHeader method for better header management and inspection capabilities. Enhanced AEM Configs API flexibility for improved configuration handling.
- **Slot API enhancements**: Extended Slot API with remove method, providing developers with more granular control over slot lifecycle management and dynamic content updates.

### Updated drop-in components

| Component | Improvements |
|-----------|--------------|
| [**Product details page**](/dropins/product-details/) v1.3.4 | • Added comprehensive gift card product support with new GiftCardOptions component<br/>• Implemented scope support for event bus isolation enabling multiple PDP instances<br/>• Enhanced global locale integration for improved multi-language support<br/>• Updated GraphQL integration with enhanced header management capabilities<br/>• Enhanced attribute processing with proper JSON parsing and error handling<br/>• Fixed carousel keyboard navigation for improved accessibility<br/>• Added comprehensive E2E tests for gift card products |
