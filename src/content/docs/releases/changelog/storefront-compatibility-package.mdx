...
## Storefront Compatibility Package v4.9.6, v4.8.31, v4.7.19
...
- **Product Ratings and Reviews**: Added composite database indexes beginning with `entity_pk_value` to the `rating_option_vote`, `rating_option_vote_aggregated`, and `review` tables to improve query performance without core overrides or query changes.
- **Product Reviews**: Added a dedicated `reviews(sku)` GraphQL query for storefronts, with SKU-based product lookup and pagination support.
