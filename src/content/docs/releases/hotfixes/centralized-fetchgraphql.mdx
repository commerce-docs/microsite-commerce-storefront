...
This hot fix centralizes header management. Instead of configuring request headers separately for each drop-in, you can now configure them once at the endpoint service level. FetchGraphQL instances (Catalog Services and Core) are created and exported from `scripts/commerce.js`.
...
4. **Bug fix**: Resolves issues with Customer Group ID header handling
