...
<div class="changelog-page">
  ...
  <div class="changelog-entries">
    ...
    >
      The Commerce boilerplate has been updated for the March 2026 suite merge on **main** (B2C) and **b2b** (B2B), including Drop-in SDK **1.8**, native product video on the Product Details block, Product List Page URL handling, layout column improvements, and B2B stability fixes.

      **Infrastructure and dependencies**

      - **Drop-in SDK 1.8** - `@dropins/tools` updated to **~1.8.0** and `@dropins/build-tools` to **1.1.0** ([#1152](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1152), [#1156](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1156)).
      - **Site metadata** - Removed legacy folder-mapping references from default site configuration ([#1148](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1148)).
      - **Demo configuration** - Default demo configs enable assets and set the Adobe Commerce Optimizer flag for ACO-focused environments ([#1114](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1114), [#1124](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1124)).

      **Product List Page and search URLs**

      - **URL handling in boilerplate** - Search and category URL construction, default sort, and visibility filters now coordinate between the Product List Page block and the Product Discovery drop-in ([#1119](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1119)).

      **Product Details and media**

      - **Native PDP video** - Example integration enables product video in the Product Gallery using the updated Product Details Page drop-in ([#1100](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1100)).

      **Layout**

      - **Column layout** - Full-width sections and padding-based gutters refine responsive layouts ([#1133](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1133)).

      **B2B-only fixes (b2b branch)**

      - **Wishlist store prefix** - Wishlist alert links use `rootLink` so store codes in the path resolve correctly ([#1130](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1130)).
      - **Website-scoped carts** - Cart state clears when switching websites to avoid stale carts and 403 responses ([#1137](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1137)).
    ...
    >
      ...
      **Elsie 1.8.0** is the Drop-in SDK runtime behind `@dropins/tools@~1.8.0` in the March 2026 suite. Drop-in packages in the suite pick up this alignment together with their own feature and patch releases.

      - **Coordinated upgrade** - Consumer drop-ins move from Elsie **1.7.x** to **1.8.0** as part of the suite consolidation ([#1152](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1152), [#1156](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1156)).
      - **Release notes** - See [@adobe-commerce/elsie@1.8.0](https://github.com/adobe-commerce/StorefrontSDK/releases/tag/%40adobe-commerce%2Felsie%401.8.0) on GitHub for the authoritative change list.
    ...
    >
      ...
      - **`includeOutOfStockItems` on CartSummaryList** - You can include out-of-stock line items in the cart summary when your experience needs them visible for merchandising or compliance.
      - **Elsie 1.8 alignment** - Runtime bump to `@adobe-commerce/elsie@1.8.0` for compatibility with the March 2026 suite.
    ...
    >
      ...
      - **ShippingMethodItem slot** - Replace the default shipping method row UI while keeping selection and API wiring inside the drop-in.
      - **GraphQL extensibility for estimates** - Extend `estimateShippingMethods` with **EstimateShippingModel** and **ESTIMATE_SHIPPING_METHOD_FRAGMENT**.
      - **checkout/shipping-methods-render hook** - Hook and sample extension show how to plug custom shipping UI into the commerce checkout block.
      - **Elsie 1.8 alignment** - Runtime bump to `@adobe-commerce/elsie@1.8.0`.
    ...
    >
      ...
      - **Guest order cancel** - Fixed **RequestGuestOrderCancel** handling when order comments are present in the response, avoiding errors during guest cancellation flows ([#268](https://github.com/magento-commerce/storefront-compatibility/pull/268)).
    ...
    >
      ...
      - **Requisition list sharing** - Adds GraphQL support for sharing requisition lists between users ([#97](https://github.com/magento-commerce/storefront-compatibility-b2b/pull/97)).
    ...
    >
      ...
      - **GraphQL fragment extensions** - **GUEST_ORDER_FRAGMENT** and related exports let you extend **CustomerOrder** queries with custom fields through **overrideGQLOperations**.
      - **Cancellation flow** - Authenticated customers no longer follow the guest cancellation path when tokens are absent from the active fragment, while guest cancellation keeps working.
      - **Elsie 1.8 alignment** - Runtime bump to `@adobe-commerce/elsie@1.8.0`.
    ...
    >
      ...
      - **`langDefinitions` merge** - The Provider now deep-merges initializer **`langDefinitions`** with bundled defaults so overrides apply reliably.
      - **Elsie 1.8 alignment** - Runtime bump to `@adobe-commerce/elsie@1.8.0`.
    ...
    >
      ...
      - **Product video in ProductGallery** - Native gallery support for product video alongside images, integrated in the Commerce boilerplate Product Details block ([#1100](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1100)).
      - **`getProductsData` API** - Batch-oriented product data retrieval supports Quick Order and other catalog workflows ([#56](https://github.com/adobe-commerce/storefront-pdp/pull/56)).
      - **Wishlist and pricing on ACO and PaaS** - Resolves wishlist issues tied to price handling in Adobe Commerce Optimizer and PaaS contexts ([#58](https://github.com/adobe-commerce/storefront-pdp/pull/58)).
      - **ProductOptions transformer** - Adds the **`transformer`** prop for custom option type conversion ([#51](https://github.com/adobe-commerce/storefront-pdp/pull/51)).
    ...
    >
      ...
      - **Filter pagination reset** - Changing filters returns shoppers to page **1** of results so stale page numbers cannot return empty lists ([#86](https://github.com/adobe-commerce/storefront-search-dropin/pull/86)).
      - **Store code in paths** - Category and search path parsing handles store codes in the URL and quick-search edge cases ([#88](https://github.com/adobe-commerce/storefront-search-dropin/pull/88), [#91](https://github.com/adobe-commerce/storefront-search-dropin/pull/91)).
      - **Boilerplate coordination** - Works with the Product List Page URL handling merged in the Commerce boilerplate ([#1119](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1119)).
    ...
    >
      **Major version upgrade** on the B2C boilerplate line. The Product Recommendations drop-in has been updated to include the following changes:

      - **Title rendering** - Fixes product title rendering in recommendation units ([#36](https://github.com/adobe-commerce/storefront-recommendations/pull/36)).
      - **4.0 foundation** - Delivers the **v4.0.x** architecture and API updates described in the [v4.0.0](https://github.com/adobe-commerce/storefront-recommendations/releases/tag/v4.0.0) release before this patch.
    ...
    >
      ...
      - **Multi-store isolation** - Optional **`storeCode`** scopes localStorage, sessionStorage, and wishlist cookies per store view while keeping single-store keys backward compatible.
      - **`langDefinitions` merge** - Provider merges initializer language overrides with bundled defaults.
      - **Elsie 1.8 alignment** - Runtime bump to `@adobe-commerce/elsie@1.8.0`.
    ...
    <ChangelogEntry
      date="2026-03-24"
      title="User Account v3.2.0"
      components={['User Account']}
    >
      The User Account drop-in has been updated with the following changes:

      - **`orderTime` on OrderModel** - Order cards can show time-of-day when you implement the **OrdersListOrderTime** slot in your block.
      - **CustomerOrder fragment extension** - GraphQL fragments can extend customer order queries for custom fields.
      - **Elsie 1.8 alignment** - Runtime bump to `@adobe-commerce/elsie@1.8.0`.
    ...
    <ChangelogEntry
      date="2026-03-24"
      title="User Auth v3.2.0"
      components={['User Auth']}
    >
      The User Auth drop-in has been updated with the following changes:

      - **Container slots** - Title, Form, and Buttons slots are available across sign-in, sign-up, reset password, and update password flows for consistent customization.
      - **Reset Password slot props** - Restores missing slot props on the Reset Password container so overrides receive full context.
      - **Elsie 1.8 alignment** - Runtime bump to `@adobe-commerce/elsie@1.8.0`.
    ...
    >
      ...
      - **Initializer parity** - `initialize()` matches the configuration pattern used by other storefront drop-ins.
      - **`langDefinitions` merge** - Provider merges initializer overrides with bundled defaults.
      - **Elsie 1.8 alignment** - Runtime bump to `@adobe-commerce/elsie@1.8.0`.
    ...
    >
      **Initial stable integration** on the B2B Commerce boilerplate at **@dropins/storefront-quick-order@~1.0.0** ([#1156](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1156)). The Quick Order drop-in adds CSV upload, multi-SKU entry, and variant grid flows for logged-in B2B buyers. See the [Quick Order documentation](/dropins-b2b/quick-order/) for containers, events, and initialization.
    ...
    >
      ...
      - **Public package metadata** - npm package visibility updates for downstream installs.
      - **Inherited fixes from 1.1.1** - Provider **`langDefinitions`** merge and Elsie **1.8** alignment ship with this tag.
    ...
    >
      **Breaking change for custom integrations.** The Requisition List drop-in has been updated with the following changes:

      - **Injected catalog dependencies** - Built-in catalog API helpers are removed. Your integration must inject the catalog props expected by list and selector experiences.
      - **`langDefinitions` merge** - Provider merges initializer overrides with bundled defaults.
      - **Elsie 1.8 alignment** - Runtime bump to `@adobe-commerce/elsie@1.8.0`.
