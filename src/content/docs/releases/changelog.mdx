...
***************/}
    ...
    >
      ...
      - **Metadata and schema improvements** - Added support for lowercase product SKUs in URLs and fixed metadata handling for multistore configurations with proper JSON-LD path resolution
      - **Mini-PDP component reorganization** - Moved mini-pdp to the components folder for better code organization and reusability
      ...
      - **PDP state management** - Fixed a critical bug where the PDP state was overwritten when multiple PDPs were loaded on the same page, ensuring proper product data isolation
...
********/}
    ...
    >
      ...
      - **Added tiered pricing support**: Introduced comprehensive tiered pricing functionality that automatically applies quantity-based discounts to cart items. The feature includes GraphQL schema integration for price tier data, intelligent tier matching that finds the highest applicable discount based on current quantity, and automatic discount percentage calculations. Supports both simple and configurable products with proper tier display in the cart UI.
      - **Enhanced configurable product merging**: Improved cart item merging logic to intelligently combine configurable products when option changes result in matching existing cart items, consolidating quantities and removing duplicates for a cleaner cart experience. Fixed an issue where updating configurable cart items with the same options would incorrectly add to quantity instead of maintaining current quantity.
      - **Added company context synchronization**: Implemented automatic cart refresh when company context changes, ensuring cart contents and pricing remain synchronized with the current B2B company context for multi-company scenarios.
      - **Fixed cross-tab logout synchronization**: Resolved an issue where cart state was not properly synchronized across browser tabs after user logout, ensuring consistent cart behavior across all active sessions.
      - **Enhanced checkout tax display**: Improved tax display logic to show "TBD" for virtual carts without applied taxes until checkout initialization, then display actual tax values, providing clearer tax information during the checkout flow.
      - **Fixed quantity input validation**: Corrected quantity field length validation to properly handle maximum quantity values, ensuring accurate input constraints.
      - **Improved accessibility and performance**: Enhanced Lighthouse accessibility scores with proper aria-labels for quantity input fields and improved keyboard navigation support for better screen reader compatibility.
...
************/}
    ...
    >
      ...
      - **Enhanced checkout architecture with reusable utilities**: Refactored checkout implementation with a new utility library including address transformation, form validation, cart state management, and shipping cost estimation functions, thereby improving code reusability across checkout implementations.
      - **Improved zero subtotal checkout payment method handling**: Added proper support for zero subtotal checkouts (free orders) with dedicated payment method display and handling when cart total is fully discounted through coupons or promotions.
      - **Enhanced gift options integration with AEM Assets**: Improved gift options functionality with AEM Assets image support for gift wrapping swatches, enabling dynamic image loading, and better visual presentation of gift options.
...
*********/}
    ...
    >
      ...
      - **Enhanced order placement response structure**: Refactored PlaceOrderFragment to use shared GUEST_ORDER_FRAGMENT pattern, improving consistency across order placement operations and providing standardized error handling.
      - **Added personal email context to ACDL events**: Enhanced Adobe Client Data Layer integration by adding the personalEmail context to place order events for complete customer identification data.
      - **Improved ACDL order context**: Extended OrderContext model with paymentAmount and priceTotal fields, and enhanced shopping cart item tracking to include discountAmount for more accurate financial reporting.
      - **Enhanced returns list pagination**: Improved scroll behavior in returns list pagination to only scroll when the user is not at the top of the page, providing better user experience.
      - **Added multi-store GraphQL support**: Enhanced configuration to support Magento store code headers when MAGENTO_STORE_CODE environment variable is set, enabling multi-store GraphQL operations.
...
********************/}
    ...
    >
      The Payment Services drop-in has been updated to include the following changes:

      - **Migrated to public npm registry**: Transitioned package dependencies from Adobe internal Artifactory to public npm registry (@adobe-commerce scope), improving accessibility and simplifying installation.
      - **Added Fastlane payment method support**: Introduced PaymentMethodCode.FASTLANE to enable PayPal Fastlane accelerated checkout experience, providing streamlined one-click payments for returning customers.
      - **Enhanced guest cart creation**: Updated cart creation logic to use the createGuestCart mutation instead of createEmptyCart, ensuring proper guest session handling and improved error messaging.
...
************************/}
    ...
    >
      ...
      - **Added comprehensive gift card product support**: Introduced complete gift card functionality with the new GiftCardOptions component and ProductGiftCardOptions container, supporting sender/recipient information, custom messages, amount selection (both preset and custom amounts), and comprehensive validation rules with full internationalization support for all form fields and error messages.
      - **Added global locale integration**: Introduced the globalLocale configuration option to the initialize function, enabling consistent multi-language support across product attributes and pricing displays, with proper locale handling for date formatting and currency display.
      - **Improved swatch accessibility**: Updated swatch and picker components to use unique IDs combining attribute and option identifiers, preventing duplicate name attributes and ensuring proper form behavior with enhanced aria-label support for screen readers.
      - **Fixed dynamic bundle pricing**: Corrected bundle product price calculations to display accurate totals when selecting different bundle options, ensuring accurate price display for both fixed and dynamic bundles.
      - **Updated GraphQL integration**: Upgraded @adobe-commerce/fetch-graphql dependency to v1.2.0 and added the getFetchGraphQlHeader method for enhanced header management and inspection capabilities.
      - **Enhanced attribute processing**: Improved handling of special attributes like ac_giftcard with proper JSON parsing, snake_case to camelCase transformation for validation rules, and graceful error handling for malformed data.
      - **Fixed carousel keyboard navigation**: Removed preventDefault() calls from arrow key handlers in the Carousel component, improving accessibility and allowing proper keyboard navigation without interfering with native browser behavior.
      - **Enhanced Cypress test coverage**: Added comprehensive end-to-end tests for gift card products and configurable products using both standalone and container-based implementations.
...
*********************/}
    ...
    >
      ...
      - **Added getFetchGraphQlHeader method**: Exposed the getFetchGraphQlHeader method for retrieving individual GraphQL request headers, enabling developers to inspect and debug header values programmatically.
      - **Enhanced search filter support**: Extended the SearchFilter model to support additional filter operators including contains, startsWith, and eq for more flexible product search and filtering capabilities.
      - **Improved facet checkbox accessibility**: Fixed checkbox naming to use unique IDs combining attribute and bucket title, preventing duplicate name attributes and ensuring proper form behavior and accessibility compliance.
...
***************************/}
    ...
    >
      ...
      - **Enhanced ProductList Price Display for complex products**: Updated the ProductList container to properly display price ranges for complex products (configurable, grouped, bundled) using the PriceRange component, while simple products continue to display standard pricing. The enhancement includes proper type checking based on itemType and comprehensive test coverage to verify correct component rendering.
      - **Improved mobile responsiveness**: Enhanced mobile view with improved carousel scrolling behavior, proper viewport-based card sizing (80vw with 300px max-width), and automatic left margin adjustment for edge-to-edge scrolling. The responsive layout ensures optimal product browsing experience on smaller screens.
      - **Fixed global CSS styling issues**: Resolved global CSS selector conflicts by renaming `.recommendations-product-list__content` to `.recommendations-carousel__content`, preventing unintended style leakage to other parts of the application and ensuring proper encapsulation of component styles.
...
****************/}
    ...
    >
      ...
      - **Added address validation support**: Introduced address validation functionality with a new AddressValidation component and container, enabling merchants to validate addresses and present suggested corrections to customers.
      - **Enhanced form submission control**: Added the onSubmit callback prop to Addresses and AddressForm containers, enabling custom form submission workflows such as address validation before final submission.
      - **Added UID support for address selection**: Implemented uid field support throughout address data models and GraphQL queries, allowing address selection and identification using either numeric IDs or string UIDs for improved flexibility.
      - **Improved order history data structure**: Refactored order item data structure to include sku and urlKey at both the item level and nested product object level, ensuring consistent data access patterns.
...
***********************/}
    ...
    >
      ...
      - **Enhanced cookie security with Secure flag**: Added Secure flag to authentication cookies (`auth_dropin_firstname` and `auth_dropin_user_token`), ensuring cookies are only transmitted over HTTPS connections for improved security and protection against man-in-the-middle attacks. Note that this requires your application to run over HTTPS in production environments.
...
************/}
    ...
    >
      ...
      - **Fixed visual flickering on wishlist open**: Resolved CSS layout issue where wishlist content would flicker when opening by adding consistent minimum height constraints to the wishlist content container across all responsive breakpoints, ensuring smooth visual transitions.
      - **Added bundle product support**: Implemented comprehensive support for bundle products in wishlists, including proper handling of `bundle_options` and `bundleOptionsUIDs` for adding, displaying, and managing bundle products with their selected options in the wishlist.
      - **Improved getWishlistById API**: Cleaned up the `getWishlistById` function by removing unused `currentPage` and `pageSize` parameters, simplifying the function signature and improving code maintainability. The API now only requires the `wishlistId` parameter.
