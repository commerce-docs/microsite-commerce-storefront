...
***************/}
    ...
    >
      ...
      **Latest Updates:**
      ...
      - **Storefront Builder compatibility improvements** - Enhanced compatibility with storefront-builder for improved UE integration
      ...
      **July-June 2025:**
      ...
      - **Package updates** - Updated package versions and dependencies
      ...
      - **Product Discovery Beta** - Integrated Product Discovery drop-in in beta version
      - **PREX Drop-in events** - PREX Drop-in now sends Adobe Client Data Layer events
...
********/}
    ...
    <ChangelogEntry
      date="2025-08-12"
      title="Cart v1.5.0-beta2"
      components={['Cart']}
    >
      The Cart drop-in component has been updated to include the following changes:

      - Fixed an issue where the channel attribute was missing from all Adobe Experience Platform (AEP) events on the Product Details Page. The channel field is now properly populated as part of the global storefront context for all PDP events, ensuring complete event data collection.
...
*******************/}
    ...
    <ChangelogEntry
      date="2025-08-12"
      title="Personalization v1.0.1-beta1"
...
************************/}
    ...
    <ChangelogEntry
      date="2025-08-12"
      title="Product details page v1.3.0-beta1"
...
*********************/}
    ...
    <ChangelogEntry
      date="2025-08-12"
      title="Product Discovery v1.1.0-beta2"
...
***************************/}
    ...
    <ChangelogEntry
      date="2025-08-12"
      title="Product Recommendations v1.1.0-beta1"
      components={['Product Discovery']}
    >
      The Personalization drop-in has been updated to include the following changes:
...
****************/}
    ...
    <ChangelogEntry
      date="2025-08-12"
      title="User account v1.0.9-beta2"
      components={['User account']}
    >
      The User account drop-in has been updated to include the following changes:

      - Fixed an issue where the entire order list would appear empty when a customer had an order containing out-of-stock products, even if other valid orders existed.
      - Added support for the `grandTotalExclTax` field and updated the Order confirmation to use it instead of computing `order?.grandTotal?.value - order?.totalTax?.value` in the frontend. When the order total is FREE, the "Total excluding taxes" line is now omitted, preventing negative excluding-tax totals on zero-total orders.
...
***********************/}
    ...
    <ChangelogEntry
      date="2025-08-12"
      title="User authentication v2.1.0-beta1"
