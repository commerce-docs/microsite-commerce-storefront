...
*********************/}
    ...
    >
      **Major Version Release**: Product Discovery has been completely redesigned with a new event-driven architecture and breaking changes that require implementation updates.
      ...
      - **Refactored styling system**: Complete overhaul of the styling system with new CSS files for better customization capabilities:
        - `alert-message.css` - Alert message styling
        - `facet-list.css` - Facet list component styling  
        - `facet.css` - Individual facet styling
        - `product-item.css` - Product item styling
        - `product-list.css` - Product list container styling
        - `results-info.css` - Search results info styling
        - `search-bar-input.css` - Search bar input styling
        - `search-bar-results.css` - Search bar results styling
      - **Updated containers**: All containers have been updated with the new architecture including SearchBarInput, SearchBarResults, ProductList, Facets, ResultsInfo, SortBy, and Pagination containers.
