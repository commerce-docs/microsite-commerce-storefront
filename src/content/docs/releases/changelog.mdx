...
********/}
    ...
    >
      ...
      - **Added tiered pricing support**: Introduced comprehensive tiered pricing functionality that automatically applies quantity-based discounts to cart items. The feature includes GraphQL schema integration for price tier data, intelligent tier matching that finds the highest applicable discount based on current quantity, and automatic discount percentage calculations. Supports both simple and configurable products with proper tier display in cart UI.
      - **Enhanced configurable product merging**: Improved cart item merging logic to intelligently combine configurable products when option changes result in matching existing cart items, consolidating quantities and removing duplicates for a cleaner cart experience. Fixed issue where updating configurable cart items with the same options would incorrectly add to quantity instead of maintaining current quantity.
...
************/}
    ...
    >
      ...
      - **Enhanced checkout architecture with reusable utilities**: Refactored checkout implementation with new utility library including address transformation, form validation, cart state management, and shipping cost estimation functions, improving code reusability across checkout implementations.
      - **Improved zero payment method handling**: Added proper support for zero payment method (free orders) with dedicated payment method display and handling when cart total is fully discounted through coupons or promotions.
...
*********/}
    ...
    >
      ...
      - **Added personal email context to ACDL events**: Enhanced Adobe Client Data Layer integration by adding personalEmail context to place order events for complete customer identification data.
      ...
      - **Enhanced returns list pagination**: Improved scroll behavior in returns list pagination to only scroll when user is not at the top of the page, providing better user experience.
...
********************/}
    ...
    >
      ...
      - **Enhanced guest cart creation**: Updated cart creation logic to use createGuestCart mutation instead of createEmptyCart, ensuring proper guest session handling and improved error messaging.
...
************************/}
    ...
    >
      ...
      - **Added comprehensive gift card product support**: Introduced complete gift card functionality with new GiftCardOptions component and ProductGiftCardOptions container, supporting sender/recipient information, custom messages, amount selection (both preset and custom amounts), and comprehensive validation rules with full internationalization support for all form fields and error messages.
      - **Added global locale integration**: Introduced globalLocale configuration option to the initialize function, enabling consistent multi-language support across product attributes and pricing displays, with proper locale handling for date formatting and currency display.
      ...
      - **Updated GraphQL integration**: Upgraded @adobe-commerce/fetch-graphql dependency to v1.2.0 and added getFetchGraphQlHeader method for enhanced header management and inspection capabilities.
...
*********************/}
    ...
    >
      ...
      - **Added getFetchGraphQlHeader method**: Exposed getFetchGraphQlHeader method for retrieving individual GraphQL request headers, enabling developers to inspect and debug header values programmatically.
      - **Enhanced search filter support**: Extended SearchFilter model to support additional filter operators including contains, startsWith, and eq for more flexible product search and filtering capabilities.
...
****************/}
    ...
    >
      ...
      - **Added address validation support**: Introduced address validation functionality with new AddressValidation component and container, enabling merchants to validate addresses and present suggested corrections to customers.
      - **Enhanced form submission control**: Added onSubmit callback prop to Addresses and AddressForm containers, enabling custom form submission workflows such as address validation before final submission.
...
************/}
    ...
    >
      ...
      - **Added bundle product support**: Implemented comprehensive support for bundle products in wishlist, including proper handling of `bundle_options` and `bundleOptionsUIDs` for adding, displaying, and managing bundle products with their selected options in the wishlist.
      - **Improved getWishlistById API**: Cleaned up the `getWishlistById` API by removing unused `currentPage` and `pageSize` parameters, simplifying the function signature and improving code maintainability. The API now only requires the `wishlistId` parameter.
