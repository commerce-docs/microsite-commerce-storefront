...
<div class="changelog-page">
  ...
  <div class="changelog-entries">
    ...
    >
      The Commerce boilerplate ships the Suite7 (B2C) and Suite3 (B2B) merges ([#1152](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1152), [#1156](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1156)) with the following implementer-facing updates:

      **Infrastructure**

      - **Dependency alignment** - `@dropins/tools` moves to ~1.8.0 and `@dropins/build-tools` moves to 1.1.0 so local installs match the suite ([#1152](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1152)).
      - **Default assets** - Demo configuration now defaults `assets-enabled` to true ([#1114](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1114)).
      - **Site metadata** - `default-site.json` removes the unused `folders` reference ([#1148](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1148)).
      - **Adobe Commerce Optimizer demo** - `demo-config-aco.json` sets the optimizer flag to true for ACO validation flows ([#1124](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1124)).

      **Authoring and merchandising**

      - **PDP video** - Product detail blocks support video in the carousel using the same JSON authoring path as images ([#1100](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1100)).
      - **PLP page size** - Product list blocks read `pageSize` from configuration (default nine) ([#1143](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1143)).

      **Routing, layout, and reliability**

      - **URL handling** - Search and PLP URL updates now live in dedicated modules so filters reset pagination and store codes stay synchronized with authored paths ([#1119](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1119)).
      - **Column layout** - Full-width sections and padding-based gutters improve edge-to-edge layouts ([#1133](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1133)).
      - **HTTP 404 behavior** - Missing pages return real 404 responses instead of silent client-side failures ([#1134](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1134)).
      - **Wishlist context** - Boilerplate initialization forwards store view codes into the Wishlist drop-in for per-store isolation ([#1139](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1139)).

      **B2B**

      - **Wishlist and cart fixes** - Wishlist links include the active store code on PDP and Cart, and cart state clears when switching websites to avoid stale authorization ([#1130](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1130), [#1137](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1137)).
      - **Quick Order** - Adds the `commerce-b2b-quick-order` block, Document Authoring models, fixtures, and Cypress flows for CSV, multi-SKU, and configurable grid ordering ([#1125](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1125)).
    ...
    >
      ...
      The Drop-in SDK (Elsie) 1.8.0 release improves build-time GraphQL authoring:

      **Improvements and fixes**

      - **GraphQL overrides** - `skipFragments` and `operations` can target the same fragment without leaving an empty fragment body, so `overrideGQLOperations` can replace fields safely ([#178](https://github.com/adobe-commerce/StorefrontSDK/pull/178)).
    ...
    >
      ...
      - **Out-of-stock visibility** - `CartSummaryList` adds an `includeOutOfStockItems` flag (default `false`) so merchants can keep low-stock rows in the main list while preserving the alert banner actions ([#102](https://github.com/adobe-commerce/storefront-cart/pull/102)).
    ...
    >
      ...
      - **Extension demos** - Address autocomplete and buy-online-pick-up-in-store extensions ship as reference implementations for the checkout-with-extensions block ([#299](https://github.com/adobe-commerce/storefront-checkout/pull/299)).
      - **Shipping presentation** - `getShippingMethodExtras` decorates shipping methods with icons, descriptions, and badges, while `ShippingMethodItem` exposes a slot for fully custom rows ([#300](https://github.com/adobe-commerce/storefront-checkout/pull/300)).
      - **GraphQL extensibility** - `ESTIMATE_SHIPPING_METHOD_FRAGMENT` extends the `estimateShippingMethods` mutation, and shipping checkout extension hooks cover the rest of the lifecycle ([#307](https://github.com/adobe-commerce/storefront-checkout/pull/307), [#305](https://github.com/adobe-commerce/storefront-checkout/pull/305)).
      - **SDK extension manager** - Checkout containers align with the SDK extension manager so extensions register consistently ([#303](https://github.com/adobe-commerce/storefront-checkout/pull/303)).
    ...
    >
      ...
      - **Guest cart after logout** - Guest carts stay usable after logout so shoppers can keep browsing without stale session errors ([#245](https://github.com/magento-commerce/storefront-compatibility/pull/245)).
      - **PHP 8.5** - Package metadata declares PHP 8.5 support for teams upgrading application runtimes ([#257](https://github.com/magento-commerce/storefront-compatibility/pull/257)).
      - **Guest order cancel comments** - Request guest order cancel responses handle order comments without throwing ([#268](https://github.com/magento-commerce/storefront-compatibility/pull/268)).
    ...
    >
      ...
      - **Requisition List GraphQL** - Product SKU is available alongside the nested product object for simpler integrations ([#87](https://github.com/magento-commerce/storefront-compatibility-b2b/pull/87)).
      - **Requisition List sharing** - Store compatibility updates cover the new list sharing workflows ([#97](https://github.com/magento-commerce/storefront-compatibility-b2b/pull/97)).
      - **PHP 8.5** - Package metadata declares PHP 8.5 support ([#95](https://github.com/magento-commerce/storefront-compatibility-b2b/pull/95)).
    ...
    <ChangelogEntry
      date="2026-03-24"
      title="Order v3.2.0 (suite alignment)"
      components={['Order']}
    >
      This suite pins the Order drop-in to v3.2.0 for Elsie 1.8.0 alignment. No new shopper-facing Order APIs ship beyond the version bump.
    ...
    <ChangelogEntry
      date="2026-03-24"
      title="Payment Services v3.0.0 (suite alignment)"
      components={['Payment Services']}
    >
      This suite keeps Payment Services on the `~3.0.0` line without new drop-in behavior beyond dependency alignment.
    ...
    >
      ...
      - **Language overrides** - User-provided `langDefinitions` merge with bundled defaults so initializer text overrides render as expected ([#27](https://github.com/adobe-commerce/storefront-personalization/pull/27)).
    ...
    >
      ...
      - **Configurable option transforms** - `ProductOptions` accepts a `transformer` prop so teams can convert option payloads before render ([#51](https://github.com/adobe-commerce/storefront-pdp/pull/51)).
      - **Bulk product fetch** - `getProductsData` loads multiple SKUs in one call for Quick Order and other high-volume UIs ([#56](https://github.com/adobe-commerce/storefront-pdp/pull/56)).
      - **Wishlist pricing** - Wishlist interactions now price correctly for Adobe Commerce Optimizer plus PaaS combinations ([#58](https://github.com/adobe-commerce/storefront-pdp/pull/58)).
    ...
    >
      ...
      - **Store code aware routing** - Category detection handles pathnames when the active store code is part of the URL ([#88](https://github.com/adobe-commerce/storefront-search-dropin/pull/88), [#91](https://github.com/adobe-commerce/storefront-search-dropin/pull/91)).
    ...
    >
      **Major version upgrade** for catalog-aware recommendations and suite pins on the `~4.0.1` line. The Product Recommendations drop-in has been updated to include the following changes:

      - **Catalog-aware unit loading** - Recommendation units load through the GraphQL `getRecommendationsUnitsByIds` query so results follow catalog view rules. The older `getRecommendations` path does not apply catalog view the same way for globally scoped units ([#34](https://github.com/adobe-commerce/storefront-recommendations/pull/34)).
      - **v4.0.0 release packaging** - Publishes compatibility for recommendations-by-unit-id GraphQL and Commerce headers for Adobe Commerce Optimizer (ACO) merchants ([#35](https://github.com/adobe-commerce/storefront-recommendations/pull/35)).
    ...
    <ChangelogEntry
      date="2026-03-24"
      title="User account v3.2.1"
      components={['User account']}
    >
      The User Account drop-in has been updated with the following changes:

      - **Customer order GraphQL extensions** - The package exports `CUSTOMER_ORDER_FRAGMENT` so you can append fields (for example invoice totals) to customer order queries through `overrideGQLOperations` on order detail and order list views ([#46](https://github.com/adobe-commerce/storefront-account/pull/46)).
    ...
    <ChangelogEntry
      date="2026-03-24"
      title="User authentication v3.2.0"
      components={['User authentication']}
    >
      The User Auth drop-in has been updated with the following changes:

      - **Composable auth forms** - Sign-in, sign-up, reset-password, and update-password containers expose `Title`, `Form`, and `Buttons` slots so teams can rearrange or replace sections without forking ([#58](https://github.com/adobe-commerce/storefront-auth/pull/58)).
      - **Reset password parity** - Missing slots on reset-password flows now match the rest of the auth containers ([#64](https://github.com/adobe-commerce/storefront-auth/pull/64)).
    ...
    >
      ...
      - **Multi-store isolation** - Optional `storeCode` configuration scopes cookies and storage keys per store view while single-store setups keep legacy keys ([#73](https://github.com/adobe-commerce/storefront-wishlist/pull/73)).
      - **Language overrides** - Initializer `langDefinitions` merge with bundled defaults so placeholder overrides apply ([#79](https://github.com/adobe-commerce/storefront-wishlist/pull/79)).
    ...
    <ChangelogEntry
      date="2026-03-24"
      title="Company Management v1.2.0 (suite alignment)"
      components={['Company Management']}
    >
      This suite bumps Company Management to v1.2.0 for Elsie 1.8.0 alignment without new shopper-facing APIs in this drop-in.
    ...
    <ChangelogEntry
      date="2026-03-24"
      title="Company Switcher v1.1.1 (suite alignment)"
      components={['Company Switcher']}
    >
      This suite bumps Company Switcher to v1.1.1 for workflow alignment without new shopper-facing changes.
    ...
    >
      ...
      - **Language overrides** - Customer-provided `langDefinitions` merge with bundled defaults so initializer text overrides render ([#52](https://github.com/adobe-commerce/storefront-purchase-order/pull/52)).
    ...
    >
      **Initial release** of the Quick Order drop-in for B2B bulk ordering. Shoppers can add many SKUs by CSV, enter multiple SKUs manually, and use the configurable variants grid. The B2B boilerplate adds the `commerce-b2b-quick-order` block, Document Authoring models, fixtures, and Cypress coverage ([#1125](https://github.com/hlxsites/aem-boilerplate-commerce/pull/1125)).
    ...
    >
      ...
      - **Language overrides** - User-provided `langDefinitions` merge with bundled defaults so initializer text overrides render ([#89](https://github.com/adobe-commerce/storefront-quote-management/pull/89)).
    ...
    >
      ...
      - **Disabled selector state** - Storybook and UI flows now honor the disabled state for the requisition list selector ([#120](https://github.com/adobe-commerce/storefront-requisition-list/pull/120)).
      - **Language overrides** - Initializer `langDefinitions` merge with bundled defaults so custom labels apply ([#124](https://github.com/adobe-commerce/storefront-requisition-list/pull/124)).
