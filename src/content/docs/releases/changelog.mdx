...
************************************/}
      ...
      >
          ...
          **v4.8.19**
          ...
          - **Wishlist count sync**: Fixed a sync issue between `items_v2` and `items_count` in the Wishlist GraphQL API ([#274](https://github.com/magento-commerce/storefront-compatibility/pull/274)).
          ...
          **v4.7.12**
          - **Guest order cancel with comments**: Resolved an internal server error when calling `requestGuestOrderCancel` on orders that include comments ([#275](https://github.com/magento-commerce/storefront-compatibility/pull/275)).
      </ChangelogEntry>
...
*********/}
    ...
    >
      ...
      - **Order Comments container**: Merchants and administrators can add order-level comments in Commerce Admin, and shoppers now see those comments (timestamp and message) in the order details view. The new `OrderComments` container displays comments for both guest and authenticated users. Use the new `commerce-order-comments` boilerplate block to render comments on order details pages ([#68](https://github.com/adobe-commerce/storefront-order/pull/68), [#87](https://github.com/adobe-commerce/storefront-order/pull/87)).
...
************************/}
    ...
    >
      ...
      - **Number attribute formatting**: The `ProductAttributes` container accepts a new `formatValue` prop for custom formatting of numeric attribute values, enabling store-specific number display logic ([#65](https://github.com/adobe-commerce/storefront-pdp/pull/65)).
    </ChangelogEntry>
...
***************************/}
    ...
    >
      **Major version upgrade.** The Product Recommendations drop-in has been updated to the following changes:
      ...
      - **Catalog view support**: The drop-in now uses the `getRecommendationsUnitsByIds` GraphQL query instead of the legacy `getRecommendations` query. This ensures recommendation units respect catalog view scoping, so globally scoped units work correctly in multi-catalog setups ([#34](https://github.com/adobe-commerce/storefront-recommendations/pull/34)).
...
*********************/}
    ...
    >
      ...
      - **Configurable product Add to Cart**: Add to Cart behavior for configurable products is now consistent and reliable, with improved parentSku handling and variant data collection for cart operations ([#25](https://github.com/adobe-commerce/storefront-quick-order/pull/25)).
      - **Standardized event payloads**: Event payload structures have been standardized across Quick Order and Grid Ordering for consistent event handling in integrations.
    </ChangelogEntry>
    ...
    >
      ...
      - **Move and copy items between lists**: B2B users can now move or copy items from one requisition list to another without leaving the page. Both move and copy operations are supported, so users can reorganize products across lists efficiently ([#127](https://github.com/adobe-commerce/storefront-requisition-list/pull/127)).
      - **Active state on PDP Add to List button**: The `RequisitionListSelector` button on the product details page now shows an active state (class `requisition-list-selector--active` and optional `activeIcon` prop) when the current product is already saved in the selected list ([#116](https://github.com/adobe-commerce/storefront-requisition-list/pull/116)).
    </ChangelogEntry>
