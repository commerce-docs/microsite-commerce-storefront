...
********/}
    ...
    >
      ...
      - **Added tiered pricing support**: Introduced tiered pricing functionality that automatically applies quantity-based discounts to cart items, displaying applicable discount percentages based on quantity thresholds for both simple and configurable products.
      - **Enhanced configurable product merging**: Improved cart item merging logic to intelligently combine configurable products when option changes result in matching existing cart items, consolidating quantities and removing duplicates for a cleaner cart experience.
      ...
      - **Improved accessibility**: Added aria-label to quantity input fields and enhanced keyboard navigation support for better screen reader compatibility.
...
*********/}
    ...
    >
      ...
      - **Added B2B negotiable quote order placement**: Introduced placeNegotiableQuoteOrder API and mutation to enable placing orders from negotiable quotes in B2B scenarios.
...
************************/}
    ...
    >
      ...
      - **Fixed dynamic bundle pricing**: Corrected bundle product price calculations to display accurate totals when selecting different bundle options.

    </ChangelogEntry>

    <ChangelogEntry
      date="2025-10-14"
      title="Product details page v1.3.4"
      components={['Product details page']}
    >
      The Product Details Page drop-in has been updated to include the following changes:

      - **Added comprehensive gift card product support**: Introduced complete gift card functionality with new GiftCardOptions component and ProductGiftCardOptions container, supporting sender/recipient information, custom messages, amount selection, and validation rules with full internationalization support.
      - **Added scope support for event bus isolation**: Implemented optional scope parameter across product data functions enabling multiple independent PDP instances on the same page without event conflicts.
      - **Enhanced global locale integration**: Added globalLocale configuration option to the initialize function, improving multi-language support across product attributes and pricing displays.
      - **Updated GraphQL integration**: Upgraded @adobe-commerce/fetch-graphql dependency to v1.2.0 and added getFetchGraphQlHeader method for enhanced header management.
      - **Enhanced attribute processing**: Improved handling of special attributes like ac_giftcard with proper JSON parsing, snake_case to camelCase transformation, and graceful error handling for malformed data.
      - **Fixed carousel keyboard navigation**: Removed preventDefault() calls from arrow key handlers to improve accessibility and allow proper keyboard navigation.
...
***********************/}
    ...
    >
      ...
      - **Enhanced cookie security**: Added Secure flag to authentication cookies (`auth_dropin_firstname` and `auth_dropin_user_token`), ensuring cookies are only transmitted over HTTPS connections for improved security and protection against man-in-the-middle attacks.
