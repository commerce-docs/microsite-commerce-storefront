...
************************/}
    ...
    >
      ...
      - **Added scope support for event bus isolation**: Implemented optional scope parameter across product data functions enabling multiple independent PDP instances on the same page without event conflicts, enhancing multi-instance deployment scenarios
      - **Enhanced global locale integration**: Added globalLocale configuration option to the initialize function, improving multi-language support across product attributes and pricing displays with proper locale inheritance
      - **Updated GraphQL integration**: Upgraded @adobe-commerce/fetch-graphql dependency to v1.2.0-alpha001 and added getFetchGraphQlHeader method for enhanced header management and improved GraphQL communication
      ...
      - **Fixed carousel keyboard navigation**: Removed preventDefault() calls from arrow key handlers to improve accessibility and allow proper keyboard navigation without interfering with other page interactions
      - **Enhanced test coverage**: Added comprehensive E2E tests for gift card products in both standalone and container-based implementations, ensuring proper validation and user interaction flows
