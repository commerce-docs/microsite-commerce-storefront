...
Analytics runs from `scripts/consented.js`, which loads only after the shopper grants consent. By default the boilerplate declines consent, so no events are sent until you wire up a consent management platform. For testing, append `?consent=accept` to the URL to load consented scripts.
</Aside>
