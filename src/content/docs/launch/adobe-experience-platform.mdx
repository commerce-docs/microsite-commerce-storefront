...
slug: setup/analytics/adobe-experience-platform
---
...
<Aside type="caution" title="Consent gating">
Analytics runs from `scripts/consented.js`, which loads only after the shopper grants consent. By default the boilerplate declines consent, so no events are sent until you integrate a consent management platform. For testing, append `?consent=accept` to the URL to load consented scripts.
</Aside>

### Method 2: Direct Script Configuration (Alternative)

Alternatively, you can configure AEP directly in your `scripts/consented.js` file:
...
Once configured, your storefront automatically sends the following types of events to Adobe Experience Platform:
