...
slug: setup/analytics/instrumentation
---
...
The <Link href={"https://github.com/adobe/adobe-client-data-layer"} text={"Adobe Client Data Layer (ACDL)"} /> is a standardized JavaScript framework that simplifies data collection on your storefront. It provides a unified approach to capturing, storing, and transmitting user interaction data.
...
Refer to the <Link href="/setup/configuration/commerce-configuration/" text="Commerce Configuration documentation" /> for complete details on how to configure your store.
...
For the instrumentation to work with Adobe Commerce's data services, you require additional configuration parameters. The easiest way to obtain these parameters is through the `magento/module-data-services-graphql` module, which exposes the necessary GraphQL endpoints.
...
The Commerce boilerplate includes the <Link href={"https://github.com/adobe/commerce-events/tree/main/packages/storefront-events-collector"} text={"Storefront Events Collector"} />, which automatically:
...
Your instrumentation automatically tracks:
...
To debug events in your storefront, use the AEP Debugger Events view. See the <Link href="https://experienceleague.adobe.com/docs/experience-platform/debugger/home.html" text="AEP Debugger video tutorial" /> for instructions.
</Aside>

### Event Schema Compliance

All events must comply with the schema defined by the <Link href={"https://github.com/adobe/commerce-events/tree/main/packages/storefront-events-sdk"} text={"Storefront Event SDK"} />. This ensures compatibility with Adobe Commerce services and analytics tools.
...
You can validate your event implementation using the <Link href={"https://github.com/adobe/adobe-client-data-layer/pull/156"} text={"ACDL Event Validator"} />. This tool checks:
...
<Aside type="note" title="Debugging events">
The AEP Debugger provides an Events view that you can use to examine the events being sent from your Commerce site. See the <Link href="https://experienceleague.adobe.com/docs/experience-platform/debugger/home.html" text="AEP Debugger video tutorial" /> for instructions.
</Aside>
...
1. Confirm the shopper has granted consent. The boilerplate loads the collector from `scripts/consented.js` only after consent is granted, and it declines consent by default until you wire up a consent management platform. To test, append `?consent=accept` to the URL.
</Steps>
...
For additional troubleshooting, refer to the <Link href={"https://experienceleague.adobe.com/en/docs/commerce/product-recommendations/admin/workspace#data-collection"} text={"Live Search Data Collection documentation"} />.
