...
Adding the MCP server does not automatically integrate it with the skill workflow — an agent can call it directly and skip the `dropin-developer` skill entirely. The same applies if you also add <Link href="/ai/wayfinder/" text="Wayfinder" />: without an explicit rule, your agent has no way to know which of the three tools should answer a given question. See <Link href="/ai/#install-the-integrations" text="Install the integrations" /> for a rule to add to `AGENTS.md` that keeps all three working together.
</Aside>
