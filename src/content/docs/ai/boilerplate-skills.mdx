...
Four of these skills—**researcher**, **drop-in developer**, **project manager**, and **tester**—use drop-in helper tools to verify slots, events, API functions, and configuration against your project's source code rather than published documentation. Without the [drop-in helper](/ai/dropins-helper/) installed, most verification falls back to local TypeScript definitions. For details about the underlying APIs, see the [Blocks reference](/boilerplate/blocks-reference/) and [Drop-in components](/dropins/all/introduction/).

Adding the drop-in helper or [Wayfinder](/ai/wayfinder/) does not automatically integrate them into this workflow. To learn how these integrations interact with Boilerplate skills, see **Add coordination instructions** in [Install the integrations](/ai/#installation-steps), which explains the precedence rules.
...
| Tool server config file     | Connects your agent to the `commerce-extensibility:search-commerce-docs` tool (separate from the drop-in helper) for live documentation search. |

</TableWrapper>

This tool server is separate from the [drop-in helper](/ai/dropins-helper/). It provides your agent with access to Adobe Commerce documentation for integration patterns, architecture, and security guidance. For slot, event, and API lookups, use the drop-in helper. See **Drop-in API accuracy** above.
