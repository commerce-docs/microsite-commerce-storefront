...
The Dropins MCP is a free <Link href="https://modelcontextprotocol.io" text="Model Context Protocol" /> (MCP) server for Cursor, VS Code, and Claude. It reads an index built from drop-in source code, so your assistant can look up current slots, events, containers, API functions, and design tokens instead of guessing from outdated training data.
...
1. **Add the server to your editor configuration.** Open the tab for your editor and paste the snippet into the config file shown. If you use the Claude Code extension in VS Code, use the **Claude Code** tab, not the **VS Code (Copilot)** tab — that one is for VS Code's built-in MCP support through GitHub Copilot.
