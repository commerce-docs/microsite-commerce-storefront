---
slug: dropins/checkout/tutorials/address-integration
title: Integrate address verification
...
- Create the autocomplete script from the sample — `blocks/commerce-checkout/address-autocomplete.js`
- Edit the checkout block — `blocks/commerce-checkout/commerce-checkout.js`
- Edit the `initializeCheckout()` function

## What you'll build

By the end of this tutorial, you'll have:

- A copy of the `address-autocomplete.js` sample script (from `/public/samples/address-autocomplete.js`) added to your project and configured with your Google API key.
- Google Places autocomplete wired into the shipping form in `commerce-checkout.js`, triggering suggestions as the shopper types a street address.
- Selected suggestions that automatically fill in the street, city, country, and postal code fields.

You might want to enhance the shopper experience by streamlining the process of populating and verifying the shipping address, thereby reducing the risk of user error. You can achieve this by implementing a third-party address lookup and autocomplete APIs, such as those provided by <Link href={"https://mapsplatform.google.com/maps-products/#places-section"} text={"Google Places"} />.
...
For this tutorial, you must have a valid Google API key. <Link href={"https://developers.google.com/maps/documentation/javascript/get-api-key"} text={"Use API keys"} /> describes the process to obtain and set up this key.
...
- [AddressValidation container](/dropins/checkout/containers/address-validation/) — reference for the validation UI.
- [Validate a shipping address](/dropins/checkout/tutorials/validate-shipping-address/) — a related address task.
