...
When a shopper types an exact match for a configured term, the storefront can send them directly to a destination URL instead of showing search results. This tutorial shows you how to configure these search redirects, so your merchandising team can manage them without code changes.

## What you'll edit or create

- Create the redirects spreadsheet — `/search-redirects`
- Create the utility module — `scripts/search-redirects.js`
- Edit the product list page block — `blocks/product-list-page/product-list-page.js`
- Edit the header block (optional) — `blocks/header/header.js`
...
- An integration in `blocks/product-list-page/product-list-page.js` that intercepts matching queries before any Live Search API call is made.
- An optional integration in `blocks/header/header.js` that redirects from the header search input without navigating to `/search` first.
