---
slug: dropins/order/tutorials/order-cancellation
...
The order drop-in lets both logged-in shoppers and guests cancel an order, but guests must locate their order through a search form instead of order history. This tutorial shows you how to implement that cancellation workflow, including the guest confirmation email step.

## What you'll edit or create

- Edit the `OrderCancelForm` container
- Edit the `OrderSearch` container
- Edit the `OrderStatus` container

## What you'll build

By the end of this tutorial, you'll have:

- A cancellation reason picker and submit button rendered by the `OrderCancelForm` container.
- An order search form built on the `OrderSearchForm` component, so guest shoppers without order history access can locate an order to cancel.
- An order status page that updates through the `OrderStatusContent` component, showing `Canceled` immediately for logged-in shoppers or `Cancellation requested` until a guest confirms by email.
...
1. If the shopper is a logged-in customer, the order is `Canceled` immediately. Otherwise, the order status shows `Cancellation requested` until the guest clicks a link in a confirmation email.
...
The order status shows `Cancellation requested` until the shopper clicks the link in an email similar to the following to confirm the cancellation.

<Aside type="choose" title="Confirm Your Main Website Store Order Cancellation">
It seems that you'd like to cancel your order #000000001. If this is correct, please <em>click here</em> to confirm your cancellation request.

If you have questions about your order, you can email us at <em>support@example.com</em>.
...
- [Order overview](/dropins/order/) — the order drop-in.
- [OrderCancelForm container](/dropins/order/containers/order-cancel-form/) — reference for the cancel UI.
