---
slug: dropins/cart/tutorials/enable-product-variation-updates-in-cart
title: Enable product variation updates in cart
...
The cart and mini cart drop-ins already include an **Edit** button that lets shoppers change a product's size, color, or other variation options without leaving the cart. This tutorial shows you how to turn that feature on or off through AEM block configuration.

## What you'll edit or create

- Edit the `enable-updating-product` block property
- Edit the `commerce-cart` block
- Edit the `commerce-mini-cart` block

## What you'll build

By the end of this tutorial, you'll have:

- An `enable-updating-product` toggle set on the `commerce-cart` and `commerce-mini-cart` AEM blocks that turns the Edit button on cart items on or off.
- An Edit button that opens a mini Product Detail Page modal, letting shoppers change size, color, or other variation options without leaving the cart.
...
The `enable-updating-product` property is *set to `false` by default* in the cart block, the same as in the mini-cart block. If you want to enable the **Edit** button:
...
The `enable-updating-product` property is *also set to `false` by default* in the mini-cart block. If you want to enable it:
...
- [Cart overview](/dropins/cart/) — other cart configuration options.
- [Product Details Page](/dropins/product-details/) — the drop-in behind the in-cart edit modal.
