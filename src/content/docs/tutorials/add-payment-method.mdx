---
slug: dropins/checkout/tutorials/add-payment-method
title: Add a custom payment method
...
- Edit the `Methods` slot on the `PaymentMethods` container
- Edit the `handlePlaceOrder` callback on the `PlaceOrder` container

## What you'll build

By the end of this tutorial, you'll have:

- A Braintree handler added to the `Methods` slot of the [`PaymentMethods`](/dropins/checkout/containers/payment-methods/) container in `commerce-checkout.js`, with `autoSync` set to `false`.
- The Braintree drop-in UI rendered inline when a shopper selects Braintree during checkout.
- Braintree payment processing wired into the `handlePlaceOrder` callback of the [`PlaceOrder`](/dropins/checkout/containers/place-order/) container, so orders complete using the Braintree payment nonce.
...
<Aside type="tip" title="Supported payment providers">
  The Checkout drop-in supports Adyen payment methods (including Bancontact) and payment extensions in addition to the Braintree example below. See the [release notes](/releases/) for the latest supported providers.
</Aside>
...
1. Update the [`PaymentMethods`](/dropins/checkout/containers/payment-methods/) container to include a custom handler for the Braintree payment method. Set `autoSync` to `false` to prevent automatic calls to the [`setPaymentMethod`](/dropins/checkout/functions/#setpaymentmethod) function when the payment method changes.
...
Implement the Braintree payment logic within the `handlePlaceOrder` handler of the [`PlaceOrder`](/dropins/checkout/containers/place-order/) container. This involves processing the payment using the Braintree <Link href={"https://developer.paypal.com/braintree/docs/guides/payment-method-nonces"} text={"nonce"} />.
...
- [Checkout overview](/dropins/checkout/) — where payment methods fit in the flow.
- [Add a stored payment method](/dropins/user-account/tutorials/payment-services-my-account/) — a related payment task in the account area.
