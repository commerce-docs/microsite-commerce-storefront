---
slug: dropins/checkout/tutorials/multi-step
...
The Commerce boilerplate ships a single-page checkout, but some storefronts need a step-by-step flow instead, with separate steps for shipping, payment, and billing. This tutorial shows you how to build a multi-step checkout that supports guest and logged-in shoppers, as well as virtual-only carts.

## What you'll edit or create

- Create the block entry point — `blocks/commerce-checkout-multi-step/commerce-checkout-multi-step.js`
- Create the step coordinator — `blocks/commerce-checkout-multi-step/steps.js`
- Create the per-step modules — `steps/`
- Create the fragment builder — `blocks/commerce-checkout-multi-step/fragments.js`
- Edit the `LoginForm`, `ShippingMethods`, `PaymentMethods` containers

## What you'll build

By the end of this tutorial, you'll have:

- A `commerce-checkout-multi-step.js` block entry point that replaces the boilerplate's single-page checkout with a step-by-step flow, coordinated by `steps.js`.
- Individual step modules in `steps/shipping.js`, `steps/shipping-methods.js`, `steps/payment-methods.js`, and `steps/billing-address.js`, each rendering its own container and validating before advancing.
- The `LoginForm`, `ShippingMethods`, `PaymentMethods`, and `BillToShippingAddress` containers rendered with `autoSync` set to `false`, so each step saves to the cart only when the shopper continues.
- Virtual-only carts that skip the shipping step automatically.
...
Step modules rely on the checkout drop-in's API functions for cart management. The complete API reference is available in the [Checkout functions](/dropins/checkout/functions/) documentation.
...
| `setShippingMethods()` | Set shipping methods on cart | Shipping Methods |
...
Implement each step module following the same interface pattern described in the [Step Modules architecture section](#step-modules) above, with proper error handling, user flow logic, and integration with containers and APIs.
</Task>
...
2. **Step progression styling** - Add complete visual styling (borders, colors, animations, and so on) to `commerce-checkout-multi-step.css`, following the visibility rules above as the foundation.
</Steps>
...
- [Checkout overview](/dropins/checkout/) — the default single-page flow you restructured.
- [Slots](/dropins/checkout/slots/) — reorganize checkout UI across steps.
