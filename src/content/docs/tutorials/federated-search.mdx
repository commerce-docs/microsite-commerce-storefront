...
- Create the content index config — `helix-query.yaml`
- Create the combined-search block — `blocks/combined-search/combined-search.js`
- Create the combined-search styles — `blocks/combined-search/combined-search.css`
...
- A `helix-query.yaml` content index configuration that indexes your site's blog posts and pages for search.
- A `combined-search` block (`blocks/combined-search/combined-search.js`) that runs parallel content and product searches using the Product Discovery drop-in's `search()` function, and displays the results in a tabbed interface.
- A `/search` page that hosts the `combined-search` block.
...
- Access to configure content indexing for your site (similar to [enrichment configuration](/setup/configuration/enrichment/))
...
This configuration tells Edge Delivery Services to index your site's content pages automatically. When you publish pages, the system:
...
- `value`: Extraction method (for example, `attribute()`, `textContent()`)
...
If you have already configured [enrichment](/setup/configuration/enrichment/), add the `content` index to your existing `query.yaml` file.
</Aside>
...
2. **Client-side content search** (`searchContent`): Because the content index is a JSON file, it can be filtered client-side using the JavaScript `filter()` method. This search is fast and doesn't require additional API calls.
...
To host the combined search block, create a dedicated page in your project. Using your content authoring tool (for example, [DA.live](/merchants/quick-start/document-authoring/), Google Docs, SharePoint), add a new page at `/search`.
...
- Review the [Blocks Reference](/blocks/) for block development patterns
