---
slug: how-tos/instrument-product-page-view-event
title: Publish a product page view event
...
sidebar:
  label: Publish a product page view event
prerequisites:
  html: true
  js: true
  commerce: true
...
A `product-page-view` event lets your analytics tools track when a shopper views a product from a custom product detail page that doesn't use the PDP drop-in. This tutorial shows you how to publish that event, building on the shared helper script from [Instrument analytics events without drop-ins](/how-tos/instrument-analytics-events/).
...
- Edit the PDP initializer — `scripts/initializers/pdp.js`
- Create the `setProductContext()` function
- Create the `publishProductPageViewEvent()` function
- Edit the `product-page-view` event

## What you'll build

By the end of this tutorial, you'll have:

- A `product-page-view` event pushed to `window.adobeDataLayer` by a `publishProductPageViewEvent` function you add to `scripts/initializers/pdp.js`.
- A `productContext` entry set by `setProductContext` with the current product's name, SKU, and pricing, built on the shared `setContext` and `pushEvent` primitives from `scripts/acdlHelper.js`.
...
- A custom product detail page that doesn't use the PDP drop-in
