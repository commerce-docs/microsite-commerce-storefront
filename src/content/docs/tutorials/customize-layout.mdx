---
slug: dropins/user-account/tutorials/customize-layout
title: Customize AddressForm layout
...
- Edit the address styles — `blocks/commerce-addresses/commerce-addresses.css`
- Edit the `.account-address-form` grid rules
- Edit the `grid-template-columns` property

### Layout Customization Example

The following examples show how to adjust the <Link href={"https://github.com/hlxsites/aem-boilerplate-commerce/blob/main/blocks/commerce-addresses/commerce-addresses.css"} text={"storefront-addresses"} /> block CSS in a boilerplate environment to customize the address form layout.
...
To create a three-column layout with a full-size text area and button area, set the grid template columns to `repeat(3,1fr)`:
...
- A one-column AddressForm layout for the fields that default to 50% width, using a CSS override you add to the `commerce-addresses` block.
- A two-column AddressForm layout with the text area and button row spanning the full width, using `grid-template-columns` and the `account-address-form-wrapper__buttons` class.
- A three-column AddressForm layout using the same technique, for storefronts that want more fields per row.
...
- [AddressForm container](/dropins/user-account/containers/address-form/) — reference for the form you customized.
- [Slots](/dropins/user-account/slots/) — the slot system used for layout changes.
