---
slug: dropins/cart/tutorials/gift-options
title: Add gift options to PDP
...
The [`GiftOptions` container](/dropins/cart/containers/gift-options/) lets shoppers add gift wrapping or a personalized message from the product detail page. This tutorial shows you how to render the container on a product details page (PDP) and carry the shopper's selections into the cart.

## What you'll edit or create

- Edit the `GiftOptions` container
- Edit the `onGiftOptionsChange` callback
- Edit the `addProductsToCart()` and `updateProductsFromCart()` calls

## What you'll build

By the end of this tutorial, you'll have:

- The `GiftOptions` container rendered on the product detail page, using either cart item data or a manually composed configuration.
- Gift wrapping and message selections captured through the `onGiftOptionsChange` callback and stored until the shopper adds the item to the cart.
- The **Add to Cart** button extended to call `addProductsToCart` and then `updateProductsFromCart`, applying the saved gift options to the new cart item.
...
- [Product Details slots](/dropins/product-details/slots/) — where the gift options mount on the PDP.
- [Cart overview](/dropins/cart/) — how gift selections flow into the cart.
