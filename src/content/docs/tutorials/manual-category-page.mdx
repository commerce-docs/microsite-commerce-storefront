---
slug: how-tos/manual-category-page
...
The [AEM Commerce Prerender](/setup/configuration/aem-prerender/) solution generates category pages automatically, but it has [documented limits](/setup/configuration/aem-prerender/#limitations) that some catalogs hit. This tutorial shows you how to author a category page by hand instead, with SEO metadata and a redirect for when the page's URL changes.

## What you'll edit or create

- Create a category document in Document Authoring
- Create the `product-list-page` block with `urlPath`
- Create the `metadata` block with SEO fields
- Create the `redirects` sheet entry
...
- A redirect entry in your `redirects` sheet that keeps existing links working after you change a category page's URL
...
<Aside type="tip" title="Add structured data, and breadcrumbs if you need them">
  ...
  the [Add breadcrumbs for PLP and PDP](/how-tos/breadcrumbs-plp-pdp/) block.
</Aside>
