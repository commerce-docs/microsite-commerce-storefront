...
If your storefront uses [AEM Commerce Prerender](/setup/configuration/aem-prerender/) to generate product page HTML, you still need a source for the PDP ancestor trail. The default product query does not return a product's category hierarchy from Catalog Service, so it cannot build PDP breadcrumbs from product data alone. PLP rendering is different: the prerender can use category data to build the category ancestor trail.
