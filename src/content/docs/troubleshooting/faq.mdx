...
The [storefront configuration](/setup/configuration/commerce-configuration/#configuration-caching) is cached by the Edge CDN with a `max-age=7200` (2 hours) cache control header and is also stored in [session storage](https://developer.mozilla.org/en-US/docs/Web/API/Window/sessionStorage).

When the storefront configuration is loaded in the browser, it is stored in [session storage](https://developer.mozilla.org/en-US/docs/Web/API/Window/sessionStorage). This means the configuration persists until the browser session is cleared or the page is refreshed after the expiration of the CDN cache. You can verify the current configuration by accessing `<site.com>/config.json` directly in your browser. This will show you the current configuration being served by the CDN.

**For developers:** If you make changes to your configuration and don't see them reflected in the browser, first check if the CDN cache has expired by accessing `<site.com>/config.json` directly in your browser. If the configuration has updated on the CDN, then you need to manually clear the session storage. You can do this by:
- Opening browser developer tools
- Going to the Application/Storage tab
- Clearing session storage for your domain
- Refreshing the page

**For end users:** They would need to clear their browser session (close and reopen the browser, or clear browsing data) to see configuration changes.
