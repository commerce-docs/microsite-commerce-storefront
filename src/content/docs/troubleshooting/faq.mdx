...
- <Link href="https://bit.ly/L445-workbook" text="[Summit 2024] Commerce Lab Workbook" />.
...
If a route returns an error or blank content, check whether Edge Delivery Services could load the underlying document from the content pipeline.

<Steps>
1. Open your browser developer tools and go to the **Network** tab.
1. Reload the page and select the **document** request for the URL you are troubleshooting (usually the first HTML request).
1. Open the response **Headers** and look for **`x-error`**. The header value may provide an important signal about missing content.
</Steps>

When **`x-error`** reports a failure to load a path ending in **`.md`** from **content-bus** (for example, a `404` for a file under `/products/...`), the page is not available in the Edge Delivery content pipeline. Typical causes include a missing or unpublished document, a wrong path, or content that has not been synced yet.
...
**What to do:** Add or publish the corresponding document in your content space (da.live, for example), or use an **overlay** so EDS can serve HTML for that route. See [BYOM](https://www.aem.live/developer/byom) for more information.

## My product pages return 404

Product detail pages (PDPs) can return **404** for several reasons. Most likely, the document does not exist in the content pipeline. Check the following:

- **Overlay / BYOM not set up:** If your project relies on prerender or bring-your-own-markup (BYOM)patterns so PDPs exist at the edge, confirm that configuration is complete. See [AEM Commerce prerender](/setup/configuration/aem-prerender/).
- **Folder mapping and uppercase SKUs:** When you use the **deprecated** [folder mapping](https://www.aem.live/developer/folder-mapping) approach for PDPs, Edge Delivery Services expects **lowercase** paths. If your catalog uses **uppercase characters in SKUs**, you must supply **metadata** for those URLs (for example via [bulk metadata](https://www.aem.live/docs/bulk-metadata)) so the correct document and metadata are associated with each path. See [SEO metadata](/setup/seo/metadata/#generate-metadata), including the **SKU casing** and **bulk metadata** sections.
