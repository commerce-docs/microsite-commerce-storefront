...
<Aside type="tip">
**New to event-driven architecture?** Learn about the [event system architecture and patterns](/dropins/all/events/) including the publish-subscribe pattern, event declarations, and best practices before diving into the API.
</Aside>
...
**Note:** The `cart/data` event is commonly used across drop-ins but may not be defined in all event type definitions. Always check the specific drop-in's event types for available events.
