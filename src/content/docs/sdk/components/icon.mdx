...
Use `Icon` to render an SVG symbol that communicates or reinforces an action. The `source` prop accepts three kinds of value: a built-in icon name, your own SVG component, or a same-domain URL to an SVG file. External URLs are rejected for security.
...
await UI.render(Icon, { source: '/icons/custom.svg', title: 'Custom' })(document.querySelector('.custom-icon'));
```
