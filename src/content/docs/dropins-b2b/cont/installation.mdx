---
title: Cont installation
description: Learn how to install the Cont drop-in component into your site.
sidebar:
  label: Installation
  order: 2
---
...
The Cont drop-in component, like our other drop-in components, are designed for the browser's JavaScript runtime without the need for a bundler. You can also install and execute drop-in components in a build-time environment with bundlers like Vite. The installation steps for both runtime and build-time environments are the same after the initial drop-in component package imports.

## Step-by-step

Use the following steps to install the Cont component:
...
For NPM (recommended for performance), install the drop-in component tools (`@dropins/tools`) and the Cont package name (`@dropins/storefront-cont`).


    ```bash frame="none"
    npm install @dropins/tools @dropins/storefront-cont
    ```
...
With the `importmap` defined, you can now **import the required files** from these packages into your Cont block as described in the next step.
...
Import the required files from the drop-in components tools (`@dropins/tools/fetch-graphql.js'`, `@dropin/tools/initializers.js`) and the Cont (`@dropins/storefront-cont`) into a JavaScript file for your Cont block. The example here shows the imports added to a `Cont.js` file. These imports constitute the minimum imports you need to create a fully functioning Cont for your site:
...
import * as pkg from '@dropins/storefront-cont/api.js';

// Drop-in component provider
import { render as provider } from '@dropins/storefront-cont/render.js';

// Drop-in component containers
import Container from '@dropins/storefront-cont/containers/Container.js';
```
...
Enables the Cont API functions and more.
...
Structural elements that are responsible for managing and displaying the UI for the Cont.
...
Connect your Cont component to the API endpoint and set the required headers as shown in the example below. Replace the endpoint URL and header placeholder values with the actual values from your Commerce backend services:
...
The code below shows how to register the Cont, load it (mount), and enable the logger for debugging purposes. You can add these functions within a `<script>` tag in your Cont HTML page as shown here:
...
1. This function registers the Cont to be loaded on the page by the `initializers.mount` function.
1. This event handler triggers the initializers.mount function to load the Cont after the page has loaded.
...
Render the Cont on the page. The example below provides the minimal configuration options required to render the default Cont drop-in component.

```js title="Cont.js"
// Render Cont
provider.render(Container, {
  // Add configuration options here
})(document.getElementById('your-Cont-element'));
```

Test the Cont by viewing your Cont page in a browser, or running your local dev or build server. If you see the Cont rendered in the browser, congrats!, you have a successful installation. If not, check the console for any errors and verify that you have followed all the steps correctly.
...
The installation of all drop-in components follow the same pattern demonstrated by installing the Cont: Install, Map, Import, Connect, Register, and Render.
