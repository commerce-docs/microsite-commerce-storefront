...
The `QuickOrderCsvUpload` container provides CSV file upload for bulk quick order. It is particularly useful for repeat orders or bulk purchases where manual SKU entry would be inefficient. The file must include "SKU" and "QTY" columns (max 200 rows). The container manages the full workflow: file selection, validation, and error handling. It verifies the uploaded file structure, validates required fields, and provides clear feedback when issues are detected. On success, it parses the content and emits `quick-order/add-items`; QuickOrderItems fetches product data and updates the list. When the Quick Order feature is disabled, the container displays an overlay indicating that Quick Order functionality is currently disabled.
...
<Diagram caption="QuickOrderCsvUpload container with file upload and sample CSV download">
  ![QuickOrderCsvUpload container](@images/quick-order-csv-upload.png)
</Diagram>
