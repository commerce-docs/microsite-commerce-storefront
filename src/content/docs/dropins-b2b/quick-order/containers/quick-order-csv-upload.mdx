...
The `QuickOrderCsvUpload` container provides CSV file upload for bulk quick order. It is useful for repeat orders or bulk purchases where manual SKU entry would be inefficient. The file must include `SKU` and `QTY` columns (max 200 rows).
