...
| `routeSharedRequisitionList` | `(relativeUrl: string) => string` | No | Receives the relative share URL and returns the final URL to display in the share modal's copy-link field. Use this to produce an absolute URL. Example: `(relativeUrl) => window.location.origin + relativeUrl`. Falls back to the relative URL when not provided. |
