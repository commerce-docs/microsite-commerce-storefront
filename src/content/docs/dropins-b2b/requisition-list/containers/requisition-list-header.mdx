...
| `onShareSuccess` | `(result: { emails: string[] }) => void` | No | Called after a successful email share with the resolved list of recipient email addresses. Forwarded to the embedded `ShareRequisitionListContent` modal. Use to show a confirmation message or refresh list state after sharing. |
