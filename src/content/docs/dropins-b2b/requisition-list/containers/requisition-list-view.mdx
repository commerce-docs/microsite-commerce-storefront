...
| `getProductData` | `(skus: string[]) => Promise<Product[] \| null>` | Yes | Function that given an array of SKUs, returns an array of products with all their data from the catalog service. |
| `enrichConfigurableProducts` | `(items: Item[]) => Promise<Item[]>` | Yes | Function that given an array of requisition list items, enriches the configurable products contained in it. |
