...
| `listItemsPageSize` | `number` | No | The number of line items to load per list per GraphQL request. Defaults to `100`. When a list has more items than fit in one page, additional requests are made automatically so consumers (for example PDP “already on list”) see the full item set. |
