...
Triggered when an alert or notification occurs. It emits and listens for alerts related to requisition list actions. The `requisitionList/alert` event provides a standardized way to display feedback to users. Consider implementing a global alert handler rather than handling alerts in individual components.
...
Triggered when a redirect or navigation is suggested after a requisition list operation. The `requisitionList/redirect` event is advisory - your application controls whether to honor the redirect request. This allows you to implement custom routing logic or prevent navigation in certain scenarios.
...
sequenceDiagram
    ...
    User->>App: Click "Create List"
    App->>DropIn: createRequisitionList()
    DropIn->>EventBus: emit('requisitionList/alert', success)
    EventBus->>App: notify success
    App->>User: Show success message
    DropIn->>EventBus: emit('requisitionLists/data', updated lists)
    EventBus->>App: update lists display
```
...
sequenceDiagram
    ...
    User->>App: Click "Add all to cart"
    App->>DropIn: addRequisitionListItemsToCart()
    DropIn->>Cart: Add items
    DropIn->>EventBus: emit('requisitionList/alert', success)
    EventBus->>App: show success notification
    DropIn->>EventBus: emit('requisitionList/redirect', {url: '/cart'})
    EventBus->>App: navigate to cart
    App->>User: Show cart page
```
