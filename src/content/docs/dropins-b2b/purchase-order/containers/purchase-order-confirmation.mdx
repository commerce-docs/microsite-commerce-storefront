---
title: PurchaseOrderConfirmation Container
description: Learn about the PurchaseOrderConfirmation container in the Purchase Order drop-in.
sidebar:
  label: PurchaseOrderConfirmation
---

import { Aside } from '@astrojs/starlight/components';
import TableWrapper from '@components/TableWrapper.astro';

The `PurchaseOrderConfirmation` container displays a confirmation message after successful purchase order creation with order details and next steps.

<div style="background-color: var(--sl-color-blue-low); border-left: 4px solid var(--sl-color-blue); padding: 0.75rem 1rem; border-radius: 0.25rem; margin: 1rem 0;">
<strong>Version: 1.0.0-beta2</strong>
</div>

## Configuration

The `PurchaseOrderConfirmation` container provides the following configuration options:

<TableWrapper nowrap={[0,1]}>

| Parameter | Type | Req? | Description |
|---|---|---|---|
| `purchaseOrderNumber` | `string \| number` | Yes | Configuration for purchase order number |
| `routePurchaseOrderDetails` | `function` | Yes | Callback function for route purchase order details |

</TableWrapper>

## Slots

This container does not expose any customizable slots.

## Usage

The following example demonstrates how to use the `PurchaseOrderConfirmation` container:

```js
import { render as provider } from '@dropins/storefront-purchase-order/render.js';
import { PurchaseOrderConfirmation } from '@dropins/storefront-purchase-order/containers/PurchaseOrderConfirmation.js';

await provider.render(PurchaseOrderConfirmation, {
  purchaseOrderNumber: "example",
  routePurchaseOrderDetails: routePurchaseOrderDetails,
})(block);
```



{/* This documentation is auto-generated from: https://github.com/adobe-commerce/storefront-purchase-order */}
