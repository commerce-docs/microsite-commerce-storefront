---
title: Company Management installation
description: Learn how to install the Company Management drop-in component into your site.
sidebar:
  label: Installation
  order: 2
---
...
The Company Management drop-in component, like our other drop-in components, are designed for the browser's JavaScript runtime without the need for a bundler. You can also install and execute drop-in components in a build-time environment with bundlers like Vite. The installation steps for both runtime and build-time environments are the same after the initial drop-in component package imports.

## Step-by-step

Use the following steps to install the Company Management component:
...
For NPM (recommended for performance), install the drop-in component tools (`@dropins/tools`) and the Company Management package name (`@dropins/storefront-company-management`).
...
With the `importmap` defined, you can now **import the required files** from these packages into your Company Management block as described in the next step.
...
Import the required files from the drop-in components tools (`@dropins/tools/fetch-graphql.js'`, `@dropin/tools/initializers.js`) and the Company Management (`@dropins/storefront-company-management`) into a JavaScript file for your Company Management block. The example here shows the imports added to a `Company Management.js` file. These imports constitute the minimum imports you need to create a fully functioning Company Management for your site:

```js title="Company Management.js"
// GraphQL Client
...
import * as pkg from '@dropins/storefront-company-management/api.js';
...
import Container from '@dropins/storefront-company-management/containers/Container.js';
```
...
Enables the Company Management API functions and more.

### Drop-in component provider

Renders the Company Management UI.

### Drop-in component containers

Structural elements that are responsible for managing and displaying the UI for the Company Management.
...
Connect your Company Management component to the API endpoint and set the required headers as shown in the example below. Replace the endpoint URL and header placeholder values with the actual values from your Commerce backend services:
...
The code below shows how to register the Company Management, load it (mount), and enable the logger for debugging purposes. You can add these functions within a `<script>` tag in your Company Management HTML page as shown here:
...
1. This function registers the Company Management to be loaded on the page by the `initializers.mount` function.
1. This event handler triggers the initializers.mount function to load the Company Management after the page has loaded.
...
Render the Company Management on the page. The example below provides the minimal configuration options required to render the default Company Management drop-in component.

```js title="Company Management.js"
// Render Company Management
provider.render(Container, {
  // Add configuration options here
})(document.getElementById('your-Company Management-element'));
```

Test the Company Management by viewing your Company Management page in a browser, or running your local dev or build server. If you see the Company Management rendered in the browser, congrats!, you have a successful installation. If not, check the console for any errors and verify that you have followed all the steps correctly.
...
The installation of all drop-in components follow the same pattern demonstrated by installing the Company Management: Install, Map, Import, Connect, Register, and Render.
