...
<Aside type="note">
Company Hierarchy supports only a single-level structure — parent companies and their direct children. Child companies cannot have their own child companies. Nested hierarchies are not supported.
</Aside>

<Aside type="caution">
Only users with the Company Administrator role can view and interact with this container. Users without that role see an access-denied message. The following Admin settings must also be enabled: **Admin** > **Stores** > **Settings** > **Configuration** > **General** > **B2B Features** > **Company** set to Yes, and the parent company must exist with Active status before you can assign child companies.
</Aside>
