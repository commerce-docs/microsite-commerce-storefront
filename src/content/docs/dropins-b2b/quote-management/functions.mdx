...
Emits the [`quote-management/quote-duplicated`](/dropins-b2b/quote-management/events/#quote-managementquote-duplicated-emits-and-listens) event.
...
Sets the expiration date for a negotiable quote template. This allows buyers or sellers to define when a quote template expires, after which it can no longer be used to generate new quotes.
...
| Parameter | Type | Req? | Description |
|---|---|---|---|
| `params` | `SetQuoteTemplateExpirationDateParams` | Yes | An object containing the template UID and the expiration date to set. |
