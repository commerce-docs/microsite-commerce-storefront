...
description: Learn how to install the Quote Management drop-in component into your site.
sidebar:
  label: Installation
  order: 2
---
...
The Quote Management drop-in component, like our other drop-in components, are designed for the browser's JavaScript runtime without the need for a bundler. You can also install and execute drop-in components in a build-time environment with bundlers like Vite. The installation steps for both runtime and build-time environments are the same after the initial drop-in component package imports.
...
For NPM (recommended for performance), install the drop-in component tools (`@dropins/tools`) and the Quote Management package name (`@dropins/storefront-quote-management`).
...
With the `importmap` defined, you can now **import the required files** from these packages into your Quote Management block as described in the next step.
...
import Container from '@dropins/storefront-quote-management/containers/Container.js';
```
...
Enables the Quote Management API functions and more.

### Drop-in component provider

Renders the Quote Management UI.

### Drop-in component containers

Structural elements that are responsible for managing and displaying the UI for the Quote Management.
...
Connect your Quote Management component to the API endpoint and set the required headers as shown in the example below. Replace the endpoint URL and header placeholder values with the actual values from your Commerce backend services:
...
The code below shows how to register the Quote Management, load it (mount), and enable the logger for debugging purposes. You can add these functions within a `<script>` tag in your Quote Management HTML page as shown here:
...
1. This function registers the Quote Management to be loaded on the page by the `initializers.mount` function.
1. This event handler triggers the initializers.mount function to load the Quote Management after the page has loaded.
...
Render the Quote Management on the page. The example below provides the minimal configuration options required to render the default Quote Management drop-in component.

```js title="Quote Management.js"
// Render Quote Management
provider.render(Container, {
  // Add configuration options here
})(document.getElementById('your-Quote Management-element'));
```

Test the Quote Management by viewing your Quote Management page in a browser, or running your local dev or build server. If you see the Quote Management rendered in the browser, congrats!, you have a successful installation. If not, check the console for any errors and verify that you have followed all the steps correctly.
...
The installation of all drop-in components follow the same pattern demonstrated by installing the Quote Management: Install, Map, Import, Connect, Register, and Render.
