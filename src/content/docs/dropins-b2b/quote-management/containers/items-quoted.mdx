...
|------|------|----------|-------------|
| `ProductListTable` | `function` | No | Customizes the quoted items table. Use this to replace or wrap the default table UI while keeping the built-in handlers for item selection, dropdown actions, quantity changes, and submission. |
| `QuotePricesSummary` | `SlotProps` | No | Customizes the pricing summary for quoted items. Use this to change how totals and price breakdowns are displayed. |
