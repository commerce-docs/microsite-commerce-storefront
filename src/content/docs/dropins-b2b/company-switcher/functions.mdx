...
```typescript
function getCompanyHeaderManager(): CompanyHeaderManager
```
...
The manager's `setCompanyHeaders()` method emits the [`companyContext/changed`](/dropins-b2b/company-switcher/events/#companycontextchanged) event after successfully setting or removing the company headers.
...
- Check current company header state
...
```typescript
function getGroupHeaderManager(): GroupHeaderManager
```
