...
| Model | Description |
|---|---|
| [`CustomerCompanyInfo`](#customercompanyinfo) | Transforms customer company context data from GraphQL including current company ID, company name, and list of available companies. Use this to add custom fields or modify company context data structures. |

</TableWrapper>

The following example shows how to customize the `CustomerCompanyInfo` model for the **Company Switcher** drop-in:
...
The **Company Switcher** drop-in provides additional configuration options beyond the standard `langDefinitions` and `models`. These options customize drop-in-specific behaviors and features.
