...
| Container | Slots |
|-----------|-------|
| [`Facets`](#facets-slots) | `Facet`, `FacetHeading`, `SelectedFacets`, `Facets`, `FacetBucket`, `FacetBucketLabel` |
...
interface FacetsProps {
  slots?: {
    ...
    FacetHeading?: SlotProps<{ data: SearchFacet }>;
...
The `FacetHeading` slot allows you to customize the heading of each facet group in the Facets container.
...
await provider.render(Facets, {
  slots: {
    FacetHeading: (ctx) => {
      // Your custom implementation
      const element = document.createElement('div');
      element.innerText = 'Custom FacetHeading';
      ctx.appendChild(element);
    }
  }
})(block);
