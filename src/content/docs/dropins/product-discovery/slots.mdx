...
The `Header` and `Footer` slot allow you to change content to the top and bottom of the `ProductList` container.
...
The context object passed to the callback function includes an `product` property that contains the product data with the following type:
