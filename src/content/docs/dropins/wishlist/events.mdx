...
| [wishlist/allItems](#wishlistallitems-emits) | Emits | Emitted when the full list of all wishlist items is available or changes. |
...
The drop-in functions that load or mutate a wishlist accept `{ scope }` and emit on that channel. See [Render an additional wishlist](/dropins/wishlist/tutorials/render-additional-wishlist/) for the full pattern.
