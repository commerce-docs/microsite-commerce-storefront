...
Storefronts with a single store view and the `default` store view continue to use the original unscoped keys. Existing wishlists are not invalidated when you upgrade the drop-in, as long as you do not change the code of your default store view.
</Aside>

<Aside type="cool" title="In short">
`storeCode` creates separate browser state per store view. It does not create separate Adobe Commerce wishlists. Authenticated customers share one wishlist across store views of the same website, while SCP filters which items are visible in each store view.
</Aside>
...
The wishlist remains website-scoped. SCP applies store-view-aware filtering to returned items and item counts, and to item removal, which requires **SCP 4.7.17** or later on the 4.7 release line, or **SCP 4.8.19** or later on the 4.8+ release line.
...
| Wishlist links drop the locale prefix | Links rendered outside of decorated content | Prefix links with the active root path or run `decorateLinks` |
