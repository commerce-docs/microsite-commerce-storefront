---
title: WishlistAlert container
description: Learn about the WishlistAlert container.
---

import Diagram from '@components/Diagram.astro';
import OptionsTable from '@components/OptionsTable.astro';

The WishlistAlert container displays notifications giving feedback about the different actions that can be done in a wishlist.

<Diagram caption="WishlistAlert container">
  ![Wishlist container](@images/dropins/wish-list/wishlist-alert.png)
</Diagram>

## Configurations

The `WishlistAlert` container provides the following configuration options:

<OptionsTable
compact
    options={[
        ['Options', 'Type', 'Req?', 'Description'],
        ['action', 'string', 'Yes', 'The action to notify the user about between "add", "remove" and "move".'],
        ['item', 'object', 'Yes', 'The wishlist item where the action is performed.'],
        ['routeToWishlist', 'string', 'No', 'Specifies the URL to show when the wishlist has products.'],
    ]}
/>

## Example

The following example demonstrates how to render the `WishlistAlert` container:

```javascript
provider.render(Wishlist, {
    action: 'add', // or 'remove', 'move'
    item: {product: {name: 'Sample Product'}},
    routeToWishlist: '/wishlist',
  });
```
