...
| `orderRef` | `string` | No | Reference identifier for the current order being viewed. Used to pre-populate order details when the page loads. |
| `returnRef` | `string` | No | Reference identifier for a return request. Used to pre-populate return details when viewing or managing product returns. |
| `orderData` | [`OrderDataModel`](#orderdatamodel) \| null | No | Pre-loaded order data to display immediately without fetching from the server. Useful for server-side rendering or caching scenarios. |
