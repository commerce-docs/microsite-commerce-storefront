...
The **Order initializer** configures how order details are displayed and managed, including order history, returns, and refunds. Use initialization to set order references, inject order data directly, and customize how order and return information is transformed for display.
...
| Model | Description |
|---|---|
| [`OrderDataModel`](#orderdatamodel) | Transforms order data including items, totals, shipping, payment, and status information. Use this to add custom fields or modify how order details are displayed. |
| [`CustomerOrdersReturnModel`](#customerordersreturnmodel) | Transforms the customer orders list data for display. Use this to add custom fields or modify how order summaries appear in the orders list. |
| [`RequestReturnModel`](#requestreturnmodel) | Transforms return request data for orders. Use this to add custom fields or modify the structure of return requests. |
...
- [Order Dictionary](/dropins/order/dictionary/) - Customize user-facing text and labels
- [Order overview](/dropins/order/) - Learn about Order features and capabilities
- [Installation guide](/dropins/order/installation/) - Install and set up the Order drop-in
