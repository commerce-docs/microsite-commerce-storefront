...
- the queries and mutations involved in the order cancellation.
...
- `Order cancellation through GraphQL`: it is a boolean and if set to `true`, the order cancellation will be available for the users.
...
There are different GraphQl queries and mutations related to order cancellation.
...
After requesting the cancellation, an email is sent to the user with a link to confirm the cancellation.
...
The following mutation is used to confirm the cancellation of the guest order based on the `Order ID` and the `Confirmation Key`:
...
mutation {
  confirmCancelOrder(
    input: {
      order_id: 9999999
      confirmation_key: "tkKMveHSSXelEoKJk7fgTH7yazKO5kng"
    }
  ){
    errorV2 {
        message
