---
title: User Account Containers
description: Overview of containers available in the User Account drop-in.
sidebar:
  label: Overview
  order: 1
---

import { Aside } from '@astrojs/starlight/components';

## Overview

The **User Account** drop-in provides **5** pre-built container components for integrating into your storefront.

<div style="background-color: var(--sl-color-blue-low); border-left: 4px solid var(--sl-color-blue); padding: 0.75rem 1rem; border-radius: 0.25rem; margin: 1rem 0;">
<strong>Version: 1.2.0</strong>
</div>

## What are Containers?

Containers are pre-built UI components that combine functionality, state management, and presentation. They provide a complete solution for specific features and can be customized through props, slots, and CSS.

## Available Containers

- [Address form](/dropins/user-account/containers/address-form/)
- [Address validation](/dropins/user-account/containers/address-validation/)
- [Addresses](/dropins/user-account/containers/addresses/)
- [Customer information](/dropins/user-account/containers/customer-information/)
- [Orders list](/dropins/user-account/containers/orders-list/)


<Aside type="tip">
Each container is designed to work independently but can be composed together to create comprehensive user experiences.
</Aside>
