...
compact
    options={[
        ...
        ['fieldIdPrefix', 'string', 'No', 'Prefix applied to every form field id rendered inside the container (for example, "shipping" produces ids like "shipping_firstname"). Use different values when rendering two Addresses instances on the same page, such as separate shipping and billing forms in checkout, to prevent duplicate HTML element ids.'],
