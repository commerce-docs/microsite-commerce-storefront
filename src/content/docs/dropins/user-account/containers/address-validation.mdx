...
Transform your address verification service output to `CustomerAddressesModel` format (with fields like `street`, `city`, `region`, `countryCode`, `postcode`) before passing to the container. Missing properties default to the original address values.
</Aside>
