...
description: Learn about the AddressForm container in the Adobe Commerce user account drop-in documentation, including configuration options, props, and practical integration guidance for your storefront.
---
...
The <Link href={"https://experienceleague.adobe.com/developer/commerce/storefront/dropins/user-account/containers/addresses/"} text={"Addresses"} /> container also uses this container.
...
compact
    options={[
        ...
        ['fieldIdPrefix', 'string', 'No', 'Prefixes every form field HTML `id` rendered inside the container. For example, `shipping` produces ids such as `shipping_firstname`. Use different values when rendering two address forms on the same page to prevent duplicate HTML element ids.'],
        ...
        ['onChange', 'function', 'No', 'Callback that runs on each form change, receiving form data as a parameter. Use for custom integrations.'],
    ]}
/>
