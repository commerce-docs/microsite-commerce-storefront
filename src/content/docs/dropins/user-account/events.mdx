...
When the list comes from the event bus, the shopper still confirms in the removal modal first; after confirmation, the row is removed from **local state only** and the **deletePaymentToken** mutation is **not** called.
