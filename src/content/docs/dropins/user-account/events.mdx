...
**Removing** a stored method is **not** handled through this event. When the shopper uses **Remove** in the PaymentMethods UI, they **confirm in the removal modal** first; the drop-in then calls the GraphQL **`deletePaymentToken`** mutation and updates the list in memory—whether the rows were first shown from this event or from **`getCustomerPaymentTokens`**. The event bus is **not** used to delete tokens. If another part of the page must stay in sync after removal, emit an updated **`account/customerPaymentTokens`** payload (optional).
