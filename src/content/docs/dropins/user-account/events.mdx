...
When the list comes from the event bus, the shopper still completes the remove confirmation dialog first. After that, the row is removed from **local state only** and the **deletePaymentToken** mutation is **not** called.
