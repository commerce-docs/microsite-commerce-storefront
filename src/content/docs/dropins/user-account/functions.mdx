...
Does not emit any drop-in events. To drive the UI from existing client data, emit **account/customerPaymentTokens** on the event bus (see [Events](/dropins/user-account/events/#accountcustomerpaymenttokens-listens)).
...
The `StoredPaymentMethodDisplay` object is returned as list items by [`getCustomerPaymentTokens`](#getcustomerpaymenttokens). It matches the **PaymentMethods** / **PaymentCard** view model (plus `publicHash` for keys and deletion).
