...
The built-in **[PaymentMethods](/dropins/user-account/containers/payment-methods/)** container calls this **only after** the shopper confirms in the removal modal. If you call `deletePaymentToken` from your own code, the mutation runs **immediately**—there is no confirmation UI unless you implement it.
