...
The `getCustomerPaymentTokens` function loads saved payment methods (vault tokens) for the logged-in customer. It calls the <Link href="https://developer.adobe.com/commerce/webapi/graphql/schema/checkout/queries/customer-payment-tokens/" text="customerPaymentTokens" /> query over the drop-in GraphQL client (GET, no cache). Results are transformed into display rows for the **PaymentMethods** container.
...
Does not emit any drop-in events. To **populate the PaymentMethods list** from client-side data without calling this query first, emit **account/customerPaymentTokens** on the event bus (see [Events](/dropins/user-account/events/#accountcustomerpaymenttokens-listens)). That event is for **display data only**; removing a token uses **`deletePaymentToken`**, not the bus.
...
The built-in **[PaymentMethods](/dropins/user-account/containers/payment-methods/)** container calls this **only after** the shopper confirms removal in the confirmation dialog. If you call `deletePaymentToken` from your own code, the mutation runs **immediately**—there is no confirmation UI unless you implement it.
...
The `StoredPaymentMethodDisplay` object is returned as list items by [`getCustomerPaymentTokens`](#getcustomerpaymenttokens). The same shape is the typed payload for the **`account/customerPaymentTokens`** event when you inject a list on the bus (see [Events](/dropins/user-account/events/#typescript-declaration)). It matches the **PaymentMethods** / **PaymentCard** view model (plus `publicHash` for keys and for **`deletePaymentToken`**).
