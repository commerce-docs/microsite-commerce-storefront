...
The Product Details drop-in exposes slots on each composed container so you can customize specific UI sections. Use slots to replace or extend those container components. A legacy monolithic single-container API from older releases is deprecated. Its slot list is not documented here. 
