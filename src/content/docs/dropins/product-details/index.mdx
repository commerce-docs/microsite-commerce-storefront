...
The tier pricing data is retrieved from the [Catalog Service GraphQL API](https://developer.adobe.com/commerce/services/graphql-api/merchandising-api/index.html#definition-ProductViewPrice), which provides the `tiers` field on the `ProductViewPrice` type. This includes the tier price amount, currency, and quantity threshold for each configured tier.
