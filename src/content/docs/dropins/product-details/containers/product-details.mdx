...
<Aside type="caution" title="Deprecated">
  The monolithic `ProductDetails` container is deprecated. It is not used in the Commerce boilerplate. Do not use it for new work. Compose the smaller containers from the [containers overview](/dropins/product-details/containers/) instead. For slot-by-slot examples, see [ProductDetails slots (deprecated)](/dropins/product-details/slots/#productdetails-slots-deprecated). For migration context, see [Product details page architectural evolution](/boilerplate/updates/#product-details-page-architectural-evolution).
</Aside>

The legacy `ProductDetails` (deprecated) container renders a full product details experience from one component. This topic lists configuration parameters and slot names for older integrations.
...
The `ProductDetails` (deprecated) container provides the following configuration options:
...
This container exposes the following slots for customization. For examples and TypeScript shapes, see [ProductDetails slots (deprecated)](/dropins/product-details/slots/#productdetails-slots-deprecated).
...
The following example shows how older code rendered the `ProductDetails` (deprecated) container:
