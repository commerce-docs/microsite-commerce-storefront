...
Starting with v1.0.0 the PDP drop-in can be configured to use multiple smaller containers for rendering the details page (instead of the single larger ProductDetails container). The boilerplate is configured to use this new approach, as it aligns with the other drop-ins.
