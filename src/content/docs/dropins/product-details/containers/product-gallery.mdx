...
The `videos` prop enables product video support in the gallery carousel. Videos are disabled by default.
...
The gallery supports multiple video sources, rendered differently depending on the URL type:

<OptionsTable
  compact
  options={[
    ['Video source', 'Render method'],
    ['Direct files (.mp4, .webm, .ogg, .mov, .avi, .mkv)', 'Native <video> element with playback controls'],
    ['Other URLs (YouTube, Vimeo, etc.)', '<iframe> with embedded player'],
  ]}
/>
