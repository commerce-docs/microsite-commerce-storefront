...
<OptionsTable
  options={[
    ...
    [
      ...
      'string',
      'No',
      'Unique identifier for the PDP context. Only containers rendered with this scope will respond to product events.',
    ],
  ]}
/>
...
The `scope` option enables **scoped contexts** for the PDP drop-in component. When a scope is provided, the component operates within an isolated context, allowing multiple independent PDP instances to run on the same page without conflicts.
