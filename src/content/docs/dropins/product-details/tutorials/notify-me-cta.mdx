...
"Notify Me" is a frequently requested customization. This tutorial shows how to wire the CTA using the event bus and CTA updates, and how to subscribe the shopper with the drop-in's [`subscribeStockAlert`](/dropins/product-details/functions/#subscribestockalert) function. The CTA wiring stays in your block; the back-in-stock subscription itself is handled by the drop-in against Adobe Commerce.
</Aside>
...
The Product Details drop-in exposes [`subscribeStockAlert`](/dropins/product-details/functions/#subscribestockalert) for exactly this. It subscribes the currently authenticated shopper to a back-in-stock alert for the given SKU.

```js
// Inside the onClick from Step 1:
if (isOutOfStock) {
  try {
    const { message } = await pdpApi.subscribeStockAlert(product.sku);
...
<Aside type="note">
`subscribeStockAlert` and [`isSubscribedStockAlert`](/dropins/product-details/functions/#issubscribedstockalert) are served by Commerce Core, so the drop-in must be pointed at the Commerce Core GraphQL endpoint, and the shopper must be authenticated. To reflect an existing subscription in the CTA on load, call `isSubscribedStockAlert(product.sku)` and adjust the button label accordingly.
</Aside>
