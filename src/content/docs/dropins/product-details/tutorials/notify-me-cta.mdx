...
<Aside type="note">
`subscribeProductAlertStock` is a customer-scoped Commerce Core mutation: the shopper must be authenticated and the product alerts feature must be enabled in the Commerce backend (`catalog/productalert/allow_stock`, exposed as `storeConfig.product_alert_allow_stock`). To reflect an existing subscription in the CTA on load, query `isSubscribedProductAlertStock(input: { sku })` the same way and adjust the button label accordingly.
</Aside>
