...
| `apiUrl` | `string` | No |  |
| `getCustomerToken` | `(() => string \| null) \| null` | No | Returns the customer's authentication token for API requests. For UI behavior like the save-card checkbox, see the [`authenticated`](/dropins/payment-services/events/#authenticated) event. |
| `storeViewCode` | `string` | No |  |

</TableWrapper>

<Aside type="note">
When the initializer runs, it configures the drop-in’s commerce API client: it sets the endpoint from `apiUrl` (default `/graphql`) and sends the `Store` header when `storeViewCode` is set. Vaulted card features (`getCustomerPaymentTokens`, `createPaymentOrderForVault`, and related helpers) use that client together with the bearer token from `getCustomerToken` when present.
</Aside>
