...
During asynchronous initialization, the Payment Services drop-in emits a `payment-services/method-available/${location}` event for each available payment method in each available location. The event payload is a single string containing the payment method code.

The following table contains the current supported locations per payment method and the corresponding event names:

<OptionsTable
  options={[
    ['Payment', 'Checkout', 'Product Detail Page'],
    ['Credit Card', '`payment-services/method-available/checkout`', 'Not supported'],
    ['Apple Pay', '`payment-services/method-available/checkout`', '`payment-services/method-available/product-detail`'],
  ]}
/>
