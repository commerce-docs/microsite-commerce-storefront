...
<Aside type="note">
When the initializer runs, it configures the drop-in’s Commerce API client: it sets the endpoint from `apiUrl` (default `/graphql`) and includes the Store header when `storeViewCode` is provided. Vaulted card features—such as `getCustomerPaymentTokens`, `createPaymentOrderForVault`, and related helpers—use this client along with the bearer token from `getCustomerToken`, when available.
</Aside>
