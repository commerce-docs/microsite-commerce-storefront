...
The following table contains the current available event names and their descriptions:
...
events.on(
  'payment-services/method-available/checkout',
  (code) => {
    switch (code) {
      case PaymentMethodCode.APPLE_PAY:
