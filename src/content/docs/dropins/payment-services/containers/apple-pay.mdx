...
| `onSuccess` | function | No | Called when the payment flow finishes successfully. The function receives a single argument containing a cartId property. If the function returns a promise, the promise will be awaited before marking the payment as "completed successfully" in the Apple Pay sheet. If the promise rejects, the payment will be marked as "failed" in the Apple Pay sheet and the error will be propagated to the `onError` callback, if provided. |
