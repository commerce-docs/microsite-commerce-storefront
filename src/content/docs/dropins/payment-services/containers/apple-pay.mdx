...
| `createCart` | `{ getCartItems: () => CartItem[]; }` | No | Provides cart items when `getCartId` is not used. |
...
| `hidden` | `boolean` | No | Whether the Apple Pay button is hidden from the layout. |
| `disabled` | `boolean` | No | Whether the Apple Pay button is disabled. Set this to `true` to disable the button (default: false). |
