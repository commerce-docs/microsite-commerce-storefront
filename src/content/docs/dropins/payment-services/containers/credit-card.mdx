...
<OptionsTable
  ...
  options={[
    ...
    ['getCustomerToken', 'function', 'No', 'The `CreditCard` container may send GraphQL requests on behalf of the shopper. This requires GraphQL authorization, which can be performed using authorization tokens or session cookies. For token-based authorization, the "getCustomerToken" function that should return a customer token as a string, or null for guest checkouts.'],
