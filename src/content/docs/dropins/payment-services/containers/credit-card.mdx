...
| `onError` | `function` | Yes | Called when the payment flow was aborted due to an error. The function receives an object with two properties, \{ name: string, message: string \}, containing the localized error name and message. Both properties are user-facing and can be translated using the "PaymentServices.CreditCard.errors" language definitions. **Note:** When omitted, the promise rejects properly so the calling code can catch errors directly. |
