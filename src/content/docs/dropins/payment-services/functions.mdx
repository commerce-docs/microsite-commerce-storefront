...
|----------|-------------|
| [submitCreditCard](#submitcreditcard) | Submits the payment order for the shopper's currently-selected credit card payment method, either a freshly-entered [CreditCard](/dropins/payment-services/containers/credit-card/) form or a selected [VaultedCreditCard](/dropins/payment-services/containers/vaulted-credit-card/). |
</TableWrapper>

## submitCreditCard

Submits the currently-selected credit card payment method — either a freshly-entered card or a stored (vaulted) card — placing the payment order for the cart identified by the latest [cart/data](/dropins/cart/events/#cartdata-emits-and-listens) event. That event must have fired at least once with the shopper's cart before calling this function.

- If the currently-selected payment method is `CREDIT_CARD`, this function validates and submits the currently-rendered [CreditCard](/dropins/payment-services/containers/credit-card/) form.
- If the currently-selected payment method is `VAULT`, this function submits the currently-selected [vaulted Credit Card](/dropins/payment-services/containers/vaulted-credit-card/), without requiring a [CreditCard](/dropins/payment-services/containers/credit-card/) form to be rendered at all.
...
|-----------|-----------------------------|
| The Payment Services drop-in has not finished initializing yet. | `"payment-services/dropin-not-initialized"` |
| The shopper's currently-selected payment method is neither `CREDIT_CARD` nor `VAULT`. | `"payment-services/unsupported-payment-method"` |
| Required cart data is missing or unavailable. | `"payment-services/missing-cart-data"` |
