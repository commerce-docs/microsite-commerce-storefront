...
|----------|-------------|
| [submitCreditCard](#submitcreditcard) | Submits the payment order for the shopper's currently-selected credit card payment method, either a freshly-entered [CreditCard](/dropins/payment-services/containers/credit-card/) form or a selected [VaultedCreditCard](/dropins/payment-services/containers/vaulted-credit-card/) payment method. |
</TableWrapper>

## submitCreditCard

Submits the payment order for the shopper's currently-selected credit card payment method — either a freshly-entered card or a stored (vaulted) card — placing the order for the cart identified by the latest [cart/data](/dropins/cart/events/#cartdata-emits-and-listens) event. That event must have fired at least once with the shopper's cart before calling this function.
...
- If the currently-selected payment method is `VAULT`, this function submits the currently-selected [vaulted Credit Card](/dropins/payment-services/containers/vaulted-credit-card/), without requiring a `CreditCard` form to be rendered at all.
