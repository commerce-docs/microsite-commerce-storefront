...
<TableWrapper nowrap={[0]}>
  ...
  | --- | --- |
  | [`submitCreditCard`](#submitcreditcard) | Validates and submits the currently-rendered `CreditCard` form. |

</TableWrapper>

## submitCreditCard

The `submitCreditCard` function validates and submits the currently-rendered `CreditCard` container's form.
