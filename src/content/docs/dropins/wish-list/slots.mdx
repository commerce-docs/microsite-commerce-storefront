...
description: Learn about the slots used in the DROPIN_NAME drop-in component.
---
...
The DROPIN_NAME drop-in component provides the slots described here. These slots are used to display the various components within the drop-in component.

## Big Picture

{/* Replace `placeholder.webp` in diagram with an illustrated image of the drop-in component identifying all its slots. */}

<Diagram caption="DROPIN_NAME slots illustration">
  ![DROPIN_NAME slots](@images/placeholder.webp)
</Diagram>

{/* Use `Callouts` control to name the slots in the drop-in component. */}
...
{/* Add a Note (if necessary) as a way to identify the slots that are not currently used in the drop-in component's default configuration. */}
:::note[Slots not in use]
The following slots are not currently in use within the default DROPIN_NAME drop-in:

- **SLOT_NAME_X**: BRIEF_DESCRIPTION.
- etc....
  :::

{/* Add the code definition for the drop-in component's `slots` interface */}

## DROPIN_NAME slots

The slots for the DROPIN_NAME are defined in the `DROPIN_NAMEProps` interface shown below.

{/* Add the slots interface for the drop-in. */}
