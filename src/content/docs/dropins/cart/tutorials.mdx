...
It doesn't matter if you want to customise the existing order summary lines, add new ones or skip some of them.
The `OrderSummary` container passes the `updateLineItems` to the `OrderSummary` component, which will be responsible for performing the appropriate actions.
...
There are default order summary lines in the `OrderSummary` component. This means, if no customisation is needed and, therefore, nothing is passed using the `updateLineItems` attribute, the default order summary lines will be rendered.
...
### Where to perform the customisations

Basically, to customise the order summary lines you need to render the `Cart` component passing the `OrderSummary` component as a slot.
When rendering the `OrderSummary` component, you can pass the `updateLineItems` attribute to customise the order summary lines as needed.
            ...
            OrderSummary: (ctx) => {
                ...
                provider.render(OrderSummary, {
                    updateLineItems: (lineItems) => {
                        // Customise the order summary lines here
                        return lineItems;
                    }
                }
            }
