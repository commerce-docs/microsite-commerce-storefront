...
You have the opportunity to group and sort the order summary lines into sections since the native [Accordion component](https://elsie.dev/?path=/docs/components-accordion--overview) from [Elsie](https://elsie.dev/) is used.

Therefore, you can easily configure the settings of the accordion component, such as:
- title
- expandable
...
updateLineItems: (lineItems) => {
  lineItems.map(lineItem => {
    if (lineItem.key === 'subTotalContent') {
      lineItem.sortOrder = 500;
    }
    return lineItem;
  });
