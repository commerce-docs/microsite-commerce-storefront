...
- A custom product attribute. Here, the product attribute is assigned the label `Shipping Notes`, and the **Catalog Input Type for Store Owner* is set to **Text Field**. You can optionally set the **Used for Sorting in Product Listing** option to **Yes** to make the product attribute easier to find applicable products in the Products grid. [Product attributes overview](https://experienceleague.adobe.com/en/docs/commerce-admin/catalog/product-attributes/product-attributes) describes how to create a custom product attribute.
...
1. Add a `Header` slot beneath the `ProductAttributes` slot.
...
1. Save the file. Add products that total at least $75 and apply the BOO24 coupon code to th cart. The page the rule name beneath each item in the cart.
