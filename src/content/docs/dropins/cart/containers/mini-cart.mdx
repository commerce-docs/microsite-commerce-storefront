...
| `confirmBeforeDelete` | `boolean` | No | When `true`, clicking the remove button shows an inline confirmation banner instead of immediately deleting the item. Forwarded to the internal `CartSummaryList`. Defaults to `false`. |
