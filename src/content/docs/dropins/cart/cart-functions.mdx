...
The `createGuestCart` function creates an empty shopping cart for a guest or logged in customer. It returns a cart ID. The function calls the [`createGuestCart`](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/create-guest-cart/) mutation.
...
The data returned is a new cart ID as a string:
...
```js
import { createGuestCart } from '@/cart/api/createEmptyCart';
