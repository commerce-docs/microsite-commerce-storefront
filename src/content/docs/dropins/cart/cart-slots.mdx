...
The following example demonstrates how to add a promotional cart rule to each product in the cart. To update the promotions in the footer on every change to the slot's context lifecycle, we append the discount within the `onChange` function.
