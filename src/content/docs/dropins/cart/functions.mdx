...
When passing an optionsUIDs array alongside the uid and quantity of the cart item, the function adds the item with the specified options and removes the previous item with the same uid but without the newly passed optionUIDs. In this scenario, the function calls both the [`addProductsToCart`](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/add-products/) mutation and [`updateCartItems`](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/update-items/) mutation.
...
To update the item from the cart by replacing the cart item. By passing optionUIDs, this function adds the product associated with the new optionsUIDs and removes the previous item associated with the given uid.
