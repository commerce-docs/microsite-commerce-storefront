...
Publishes a shopping cart view event to the Adobe Client Data Layer (ACDL). This function sets the shopping cart context and triggers a `SHOPPING_CART_VIEW` event on the ACDL, typically used when a cart page loads.
...
Publishes the `SHOPPING_CART_VIEW` event to the ACDL with the current cart context.
...
The `setGiftOptionsOnCart` function sets options such as gift receipts and wrapping. It calls the  <Link href="https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/set-gift-options/" text="setGiftOptionsOnCart" /> mutation.
