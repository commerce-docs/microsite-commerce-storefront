...
| [`setShippingMethodsOnCart`](#setshippingmethods) | Alias for `setShippingMethods`. Sets one or more shipping methods on the cart. |
...
The `setShippingMethods` function sets one or more shipping methods on the cart. The function calls the <Link href="https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/set-shipping-methods-on-cart/" text="setShippingMethodsOnCart" /> mutation. This function is also exported under the alias `setShippingMethodsOnCart`.
