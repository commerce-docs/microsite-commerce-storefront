...
The Checkout dropin provides a variety of fully-customizable controls to help you complete the purchase, including forms to introduce required information for contact details like email address, delivery and billing addresses, shipping options and payment methods. Also you have the ability to sign in in case you already have an account, recovering automatically default addresses and contact details.
...
Describes the API functions available in the Checkout dropin: `dispatchApiCall`, `enqueueRequest`, `estimateShippingMethods`, `fetchAddressFormFields`, `filterControlledErrors`, `getCart`, `getCountries`, `getCustomer`, `getRegions`, `getStockStatus`, `getStoreConfig`, `initialize`, `isEmailAvailable`, `placeOrder`, `redirect`, `setBillingAddress`, `setGuestEmailOnCart`, `setPaymentMethod`, `setShippingAddress` and `setShippingMethodsOnCart`. These functions allow developers to retrieve and display detailed checkout information dynamically. Visit the [Checkout Functions](/dropins/checkout/checkout-functions/) page to learn more.