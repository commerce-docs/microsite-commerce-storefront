...
After you install and configure the Adyen Payment Plugin on Adobe Commerce, you must add your Adyen client key to the Commerce boilerplate configuration.
...
1. Use the `PaymentMethod` [handlers slot](/dropins/checkout/slots/) to add a payment method handler for Adyen. This handler renders the Adyen payment method UI when the backend indicates that Adyen credit card is an allowed payment method.
