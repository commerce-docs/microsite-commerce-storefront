...
<Steps>
1. Open the `commerce-checkout.css` file and delete the contents.

1. Open the `commerce-checkout.js` file and remove everything inside the `decorate` function:
...
<Steps>
1. You've probably noticed that even after initializing the checkout the shipping method form still loads. To resolve this, create a new utility method called `continueToShipping` to remove the skeleton and render the shipping method form when the checkout is initialized.
