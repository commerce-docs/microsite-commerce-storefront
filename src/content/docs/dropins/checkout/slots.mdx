...
The slots for the [`Title`](/dropins/checkout/containers/overview/#title-and-heading-customization) component are defined in the `TitleProps` interface shown below. The slots allow you to customize the content to be displayed on the title for a specific container.
...
The `Title` property is a callback function used to render a custom title content. It receives the default context implementing the `DefaultSlotContext` interface:
...
<Aside type="note">
Some containers are extending their props interface to use the `TitleProps` slot in order to customize the title content. You will find some examples on those specific containers.
</Aside>

## LoginForm slots

The slots for the [`LoginForm`](/dropins/checkout/containers/login-form/) container are defined in the `LoginFormProps` interface shown below. The slots allow setting the heading content dynamically based on the user authentication status.
...
The `Heading` property is a callback function used to render a custom heading content based on the authentication status provided by the context. The `Heading` slot receives a boolean as a context indicating whether the user is authenticated or not.

<Aside type="note">
Please visit the [`LoginForm`](/dropins/checkout/containers/login-form/) container page to learn more about of the properties, and how to override the slots to configure and customize login form described on different examples.
</Aside>
...
The slots for the [`TermsAndConditions`](/dropins/checkout/containers/terms-and-conditions/) container are defined in the `TermsAndConditionsProps` interface shown below. The slots allow you to provide a list of agreements to be accepted by the user before placing the order.
...
The `Agreements` property is a callback function used to render and configure the list of agreements. It provides a context by including the method `appendAgreement()` to add a new agreement, based on the `SlotMethod` type:
...
<Aside type="note">
Please visit the [`TermsAndConditions`](/dropins/checkout/containers/terms-and-conditions/) container page to learn more about of the properties, and how to override the slots to configure and customize the list of agreements described on different examples.
</Aside>