...
- The `slots` property is an object containing the following properties:
  ...
  - The `PaymentMethodConfig` interface is composed by:
    ...
    - The `icon` configuration specifies the name of the icon to be shown beside of the label. The icon name must exist within the list of available icons defined on the <Link href="/sdk/components/icon/" text="Storefront SDK Icon component" />.
