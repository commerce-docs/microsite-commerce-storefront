...
<OptionsTable
  ...
  options={[
    ...
    ['onSelectionError', 'function', 'No', 'A function that takes a PaymentMethod object and an error as arguments. It is called when the setPaymentMethodOnCart() API throws an error when a payment method is selected and stored to the backend.'],
...
export interface PaymentMethodsProps extends HTMLAttributes<HTMLDivElement>, TitleProps {
  ...
  onSelectionError?: (error: unknown, method: PaymentMethod) => void;
  slots?: {
...
<li>The `onSelectionChange` property is a handler used to perform actions called when a payment method is selected.</li>

<li>The `onSelectionError` property is a handler used to perform actions called when a payment method is selected and the setPaymentMethodOnCart() API throws an error. It could be used in the integration layer by the merchant to show errors.</li>

<li>The `UIComponentType` property is a string containing the name of the UI component type to be used as a selector for each payment method. The available UI components are: `ToggleButton` or `RadioButton`.</li>

<li>The `slots` property is an object containing the following properties:</li>

#### Title property (*)

The `Title` property is inherited as a result of merging the `slots` from `LoginFormProps` and `TitleProps`. This is a handler that renders custom title content.
