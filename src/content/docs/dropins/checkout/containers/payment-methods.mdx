...
The `PaymentMethods` is a _conditional_ container. It is designed to manage and display the available payment methods during the checkout process. You can configure it to decide whether the container should not set the payment method automatically when it is selected, display or not a payment method, show an icon beside of the label, display or not the label and finally provide a specific handler to render the payment method.
...
<li>The `UIComponentType` property is a string containing the name of the UI component type to be used as selector for each payment method. By now, the UI components available are: 'ToggleButton' or 'RadioButton'.</li>

<li>The `slots` property is an object containing the following properties:</li>

#### Title property (**)

The `Title` property is inherited as a result of merging the `slots` from `PaymentMethodsProps` and `TitleProps`; this is a handler used to render a customized title content.
...
## Example 5: Render with customized title and radio button as selector

The following example renders the `PaymentMethods` container on a checkout page, displaying a customized title and showing radio buttons instead of toggle buttons as selector for each payment option:
