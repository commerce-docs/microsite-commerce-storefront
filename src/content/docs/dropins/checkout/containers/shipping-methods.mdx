...
<li>The `slots (*)` property is inherited from `TitleProps` interface; this is an object containing the following properties:</li>
...
The following example renders the `ShippingMethods` container on a checkout page. It includes configurations to hide the title, pre-select the free shipping in case no shipping method is selected, and showing a message in case the chosen shipping method is `Best Way` (Table Rate).
