...
<OptionsTable
  ...
  options={[
    ...
    ['onCartSyncError', 'function', 'No', 'A function that takes a ShippingMethod object and an error as arguments. It is called when the setShippingMethods() API throws an error when a shipping method is selected to be stored to the backend.'],
...
- The `onCartSyncError` property is a handler used to perform actions called when a shipping method is selected and the setShippingMethods() API throws an error. It could be used in the integration layer by the merchant to show errors.
