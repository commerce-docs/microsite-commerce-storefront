...
<OptionsTable
  ...
  options={[
    ...
    ['handleValidation', 'function', 'No', 'A function that performs validation checks and returns a boolean or Promise&lt;boolean&gt; indicating whether the validation passed. Supports asynchronous validation such as server-side fraud checks. If it returns false (or a promise that resolves to false), the order will not be placed.'],
...
- The `handleValidation` property is a handler used to perform validation checks before submitting the checkout forms and placing the order. It can return a `boolean` synchronously or a `Promise<boolean>` for asynchronous validation (for example, server-side fraud checks).
