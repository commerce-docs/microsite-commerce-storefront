...
<li>The `appendAgreement` configuration is a callback function which accepts the following attributes to configure an agreement:</li>
  ...
  - **`mode`**
    It defines how the checkbox should be shown:
    - 'manual' for unchecked; the user should manually check it
    - 'auto' for checked; the checkbox will appear checked by default
  - **`name`**
...
The following example renders the `TermsAndConditions` container on a checkout page, displaying a custom agreement providing directly the text to show with the link to the EDS page, in the element with the class `.checkout__terms-and-conditions`:
  ...
  slots: {
    Agreements: (ctx) => {
      ctx.appendAgreement(() => ({
        name: 'custom',
        mode: 'auto',
        text: 'Custom terms and conditions <b><a href="/terms-and-conditions">Terms & Conditions</a></b>.',
      }));
    },
  },
})($termsAndConditions),
...
## Example 2: Render the available agreements labels configured in EDS

The following example renders the `TermsAndConditions` container on a checkout page, displaying the available agreements configured into the **`placeholders.xlsx`** EDS file, in the element with the class `.checkout__terms-and-conditions`:
