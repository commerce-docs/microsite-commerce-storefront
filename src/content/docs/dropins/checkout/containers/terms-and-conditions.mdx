...
The following example renders the `TermsAndConditions` container on the checkout page, displaying the configured agreements and taking the labels from the translations in the **`placeholders.xlsx`** EDS file, within the element with the class `.checkout__terms-and-conditions`:
