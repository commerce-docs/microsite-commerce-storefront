...
- `handleSelectedAddress` - Called when the shopper selects an address. Use to persist the selection or continue checkout.
...
The container listens to checkout events and automatically maps the current shipping address to `CartAddressInput`. Only pass `suggestedAddress` when available.
</Aside>
