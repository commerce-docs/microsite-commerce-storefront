...
The `autoSync` property determines whether the container automatically synchronizes its state changes with the backend via API calls. When set to _false_, the container does not synchronize its state, but still maintains local updates.
