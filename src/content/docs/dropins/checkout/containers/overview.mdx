...
This section describes generic aspects common to all Checkout components that can be applied and used at the same way by all Checkout components that require them.
...
Each container provides a title and heading by default, but you may not want to display the title, or you may even want to provide a different customized title.
For this reason, an interface has been created to extend this functionality.
...
<OptionsTable
  ...
  options={[
    ['Option', 'Type', 'Req?', 'Description'],
    ['displayTitle', 'boolean', 'No', 'Hides the title of the container.'],
    ['slots', 'object', 'No', 'Object with the content to be displayed on the title.'],
  ]}
/>

The containers receives an object as parameter which implements the `TitleProps` interface with the following properties:
...
The `Title` property is a handler used to render the customized title content.
