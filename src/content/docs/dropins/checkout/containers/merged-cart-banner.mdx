...
The `MergedCartBanner` container is designed to display a notification banner when items from an old cart are merged into the current cart. You can apply styles to the banner by passing a `className` prop to the container.
