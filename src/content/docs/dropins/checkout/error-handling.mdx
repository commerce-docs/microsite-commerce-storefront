...
The system should work using "Optimistic" UI updates with rollback concept. It is a technique used to improve the user experience by making the application feel like it is more responsive.
...
In this scenario, when the user performs an action that changes the state, that action sends information to the backend, and the user interface (UI) is optimistically updated to reflect the change. We refer to this process as "optimistic" because we are making the UI change with the belief that the backend will accept the state change. If we waited for confirmation from the backend that the user action was successful before updating the UI, it would take too long, negatively impacting the user experience.
