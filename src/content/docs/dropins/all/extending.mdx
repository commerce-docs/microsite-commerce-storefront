...
In this how-to, we'll walk through the steps needed to integrate a third-party ratings component on the product-details commerce page. We will add the ratings component under the product's title and the detailed reviews to show below the gallery. However, how you fetch and display these reviews can impact performance, especially CLS. In this guide, we'll show you how to:
...
After the page loads, your code needs to fetch ratings data from Yotpo (or the relevant third-party). Once the API key is set, you can trigger a call to the Yotpo endpoint.
