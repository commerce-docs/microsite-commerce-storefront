...
This tutorial describes the steps needed to integrate a third-party ratings component on the product detail page. We will add the ratings component under the product's title, and the detailed reviews will be displayed below the gallery. However, how you fetch and display these reviews can impact performance, especially Cumulative Layout Shift (CLS).
...
- Yotpo (or other third-party) API key — you'll need this to fetch rating data.
- Config Excel — a spreadsheet or config file that stores your API key for easy reference.
- Event Emitter — a mechanism (like the events object) to emit a ratings event and listen for it in the code.
