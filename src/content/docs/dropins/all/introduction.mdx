...
| [Compose a block](/dropins/all/compose-a-block/) | Combine an existing drop-in's API function, shared SDK components, and the event bus in a plain block, without creating a new drop-in. |
| [Extend, compose, substitute, or create?](/dropins/all/extend-or-create/) | Decide when to extend an existing drop-in, compose a plain block from existing pieces, substitute with a third-party solution, or create a new one. |
