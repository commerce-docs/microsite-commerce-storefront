...
Use <Term>Compose</Term> when the feature doesn't fit inside any single drop-in container, but the pieces already exist: an existing drop-in's API function, shared SDK components, and the event bus. Combine those in a plain block instead of creating a new drop-in.
...
Yes, the SDK is generally available. Building and maintaining a new drop-in is still a long-term commitment, so contact Adobe to discuss your use case before you start.
