...
1. **Can you achieve your goals by extending?** 
   ...
   - Use slots to add custom content (including third-party payment methods)
   - Listen to events for custom behavior
   - Use transformers to change how data is processed
   
   **If yes** → **Extend** (recommended path)
   
   **Note**: You can integrate third-party services using slots without replacing the entire drop-in. For example, you canintegrate Stripe or PayPal payment methods into the existing Checkout drop-in rather than replacing the whole checkout flow.

2. **Do you need to completely replace core functionality?**
   - Adobe's drop-in doesn't fit your business model
   - You want an all-in-one experience from a payment provider
   - Third-party service provides their own complete UI/logic
...
3. **Do you need something entirely new that doesn't exist?**
   ...
   **If yes** → **Create** (early access)
</Steps>

<Aside type="tip" title="Default recommendation">
Start with extending. Most customization needs can be met through slots, styling, and events without the complexity and maintenance burden of substitution or creation.
</Aside>
