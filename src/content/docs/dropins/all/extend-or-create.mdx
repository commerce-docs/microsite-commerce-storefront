...
sidebar:
  label: Choose a Drop-in Approach
---
...
<Aside type="tip" title="Don't need a drop-in at all?">
If the feature doesn't fit inside any single drop-in container, you can combine an existing drop-in's API function, shared SDK components, and the event bus in a plain block instead. See [Build custom features](/blocks/build-custom-features/).
</Aside>
