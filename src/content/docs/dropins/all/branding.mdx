...
The following diagram shows a small branding change. When we override the default value of a single shape token, we override the default border-radius of the `Button` library component, which changes the look and feel of drop-ins that use it.

<Diagram caption="How to override the drop-in-design tokens.">
  ![Diagram for how to brand drop-ins](@images/brand/howtobrand.svg)
</Diagram>
...
To change existing drop-in CSS classes or add new CSS classes to the drop-in elements. Styling provides a deeper level of customization than branding.
...
- scripts/
- **styles/** _CSS files for drop-in design tokens, fonts, deferred styles_
...
The process of branding drop-ins is typically fast and easy. Focus on one brand category at a time and work with your designers to solve the less obvious brand-to-token overrides.
