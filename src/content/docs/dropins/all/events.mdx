...
Components subscribe to events when they need to listen for and respond to changes happening elsewhere in the application.
...
Components emit events when they want to share information with other components, drop-ins, or external systems in the application.
...
Many drop-ins listen for or emit common events that enable cross-component communication, authentication state management, localization, and error handling.
...
`} caption="External events (thin dashed blue arrows) flow from outside sources through the Event Bus to the drop-in. Internal events (thick solid green arrows) coordinate between containers within the same drop-in.">
</Diagram>
