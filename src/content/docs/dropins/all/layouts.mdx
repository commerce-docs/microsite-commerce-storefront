...
<<<<<<< HEAD
Within the `eds/lcp` lifecycle event, query the EDS block's class selector from the rendered block and append it to the right element in the layout. In this example, we select the Product Recommendations block using the `.product-recommendations` class, then move it to the element you want in the layout (`$prex`).
=======
...
>>>>>>> 4d37cf0f (fix: spelled out two EDS abbreviations)
