...
Within the `decorate` function, override the drop-in component's default dictionary with the `placeholders` object. The documentation for each drop-in component includes a "dictionary" topic where you can find `JSON` examples.
