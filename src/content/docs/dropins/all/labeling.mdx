...
A (`placeholders`) content sheet where you change and maintain your storefront's labels, which includes localization. See the [Placeholders file](/resources/placeholders/) for more information.
...
To add or change UI text labels within Commerce drop-in components. Merchants can change the UI labels in their storefronts by changing the values in the `placeholders` file within their content directories.
...
Within the `decorate` function, use the `fetchPlaceholders` function to retrieve the `placeholders` language object from the content folder. The following diagram and code snippet shows how to fetch the placeholders.
