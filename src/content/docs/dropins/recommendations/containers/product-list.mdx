...
- When `currentProduct.price` is present (**Adobe Commerce Optimizer and Adobe Commerce as a Cloud Service only**): uses `GetRecommendationsByUnitIdsWithCurrentProduct`, which supports dynamic price filter operators.
- When only `currentProduct.sku` is present, or `currentProduct` is absent: uses `GetRecommendationsByUnitIds`, which is compatible with all backend types including PaaS.
