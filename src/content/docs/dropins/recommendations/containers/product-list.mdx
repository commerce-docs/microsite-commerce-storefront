...
<OptionsTable
  ...
  options={[
    ...
    ['currentProduct', 'object', 'No', 'Anchor product context (`{ sku: string; price?: number }`). Required fields depend on unit type and price filter operator. See [Product context](#product-context).'],
...
- **Non-PDP pages**: There is no product context on PLP, cart, home, and similar pages. For **anchor-based** unit types, set `currentsku` in the [product-recommendations block configuration](/merchants/content-customizations/product-recommendations/#configuration). Add `currentprice` only when the unit uses dynamic or relative price filter operators.
- **Context-free unit types**: Omit `currentProduct`. The dropin renders with `recId` alone on any page type.

<Aside type="caution" title="Do not omit seed SKU on anchor-based units">

On non-PDP pages, anchor-based unit types return no products when the seed SKU is missing—even if the unit uses a static price filter. Only **context-free** unit types can run with `recId` alone.
...
The dropin sends `currentProduct.price` only when a dynamic or relative price operator is active. Static price filters use the seed SKU only.
