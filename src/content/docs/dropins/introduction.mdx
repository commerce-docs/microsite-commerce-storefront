...
| [Arranging block layouts](/dropins/all/layouts/) | Configure where drop-in containers appear on the page via HTML fragments and block layout. |
...
You now know which drop-ins are available and what every drop-in is made of. For more on the container piece specifically, see [Containers](/dropins/all/containers/). Or jump straight to [Using drop-ins](/dropins/all/quick-start/), which shows the three-line pattern — import the initializer, import a container, render it — with a complete working example.
