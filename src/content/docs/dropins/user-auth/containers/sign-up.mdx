...
By default, upon a successful registration, the SignUp container renders the [SuccessNotification](./success-notification.mdx) container. 

If the `isAutoSignInEnabled` property is passed, the user is logged in. 
If the `routeRedirectOnSignIn` property is passed, the user is redirected to the specified URL after a successful login. Neither the SuccessNotification container nor slot willbe rendered.
