...
<OptionsTable
  ...
  options={[
    ...
    [
      ...
      'function',
      'No',
      'Callback executed when the user successfully signs up, receiving userName, userEmail, and status as parameters.',
    ],
  ]}
/>
...
The following example redirects the user to the account page if they are already authenticated. If not, the user is redirected first to the login page, then to the account page. The example also includes an onSuccessCallback that handles newsletter subscription when the user successfully signs up.
    ...
    await authRenderer.render(SignUp, {
      ...
      slots: {
        PrivacyPolicyConsent: async (ctx) => {
          ...
          newsletterInput.name = 'newsletterSubscription';
          newsletterInput.checked = true;

          wrapper.appendChild(newsletterInput);
        }
      },
      onSuccessCallback: (data) => {
        // If newsletter checkbox is not checked, do not subscribe to newsletter
        // The newsletterInput refers to a checkbox element in the sign-up form for newsletter subscription
        ...
        mesh.fetchGraphQl(query, { method: 'POST', variables: { email }});
      }
    })(block);
  }
}
