...
description: Learn about the UpdatePassword container in the Adobe Commerce user auth drop-in documentation, including configuration options, props, and practical integration guidance for your storefront.
---
...
The UpdatePassword container continues the process of resetting the user's password that began in the <Link href={"https://experienceleague.adobe.com/developer/commerce/storefront/dropins/user-auth/containers/reset-password/"} text={"ResetPassword container"} />. The user clicks a link in an email to navigate to the UpdatePassword container, where they can enter a new password.

Upon a successful password update, the UpdatePassword container renders the <Link href={"https://experienceleague.adobe.com/developer/commerce/storefront/dropins/user-auth/containers/success-notification/"} text={"SuccessNotification"} /> container. 
