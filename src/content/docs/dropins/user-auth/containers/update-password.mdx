...
The UpdatePassword container continues the process of resetting the user's password that began in the [ResetPassword container](./reset-password.mdx). The user clicks a link in an email to navigate to the UpdatePassword container, where they can enter a new password.

Upon a successful password update, the UpdatePassword container renders the [SuccessNotification](./success-notification.mdx) container. 
