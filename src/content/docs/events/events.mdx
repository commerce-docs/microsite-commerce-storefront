---
title: Event bus
description: Learn how drop-ins and your own custom blocks use events for communication, including the event bus architecture and common events shared across all drop-ins.
slug: dropins/all/events
sidebar:
  label: Event bus
---
...
Drop-ins and your own custom blocks can share a single event-driven architecture using the `@dropins/tools/event-bus.js` module to communicate. This event system lets any block respond to application state changes, stay loosely coupled from other blocks, and keep state synchronized across your storefront.
