...
For `npm install`, `npm start`, and local preview, use [Running locally](/boilerplate/#running-locally). If accounts and tools are not ready, read [Prerequisites](/get-started/before-you-start/) first.

You can return later for optional steps such as <Term>Sidekick</Term> and <Term>Experience Workspace</Term>.
...
1. Experience Workspace — Edit content in context on the rendered page.
1. Sidekick — Preview, publish, and open source documents from the live site.
...
The [Site Creator or this walkthrough](#site-creator-or-this-walkthrough) section explains when to use this long-form path. The tasks below assume you create the GitHub repository from the <Link href="https://github.com/hlxsites/aem-boilerplate-commerce" text="Commerce boilerplate template" />, install the <Link href="https://github.com/apps/aem-code-sync" text="AEM Code Sync app" />, and use Site Creator for content (including attaching an existing repository), as in [Prerequisites](/get-started/before-you-start/). If your organization only allows Site Creator for brand-new sites, work with your administrator, then pick up at the Add content task with the repo Site Creator already created.

## Step-by-step

The tasks below follow the same order most teams use: GitHub repository, Code Sync, Commerce data, Document Authoring content, optional tooling, local clone, then security. Run them in order unless a note tells you to wait or skip. Open [Architecture](/get-started/architecture/) or [Prerequisites](/get-started/before-you-start/) when a label or step does not make sense.
...
### Use Experience Workspace for content editing

Experience Workspace lets authors edit content directly on the rendered page as well as in the document editor. It's off by default, and enabling it may require you to update your storefront's code. See [Using Experience Workspace](/merchants/quick-start/experience-workspace/) to enable it and to share with your authors for day-to-day use.
</Task>
...
To use Sidekick, install the browser extension from the Chrome Web Store, or follow Edge steps on <Link href="https://www.aem.live/docs/sidekick" text="Using AEM Sidekick" />. Sidekick detects your project's preview and live URLs automatically. If it doesn't, add your project manually from the Sidekick's menu. That page covers install steps, adding your project, preview, and publish. For prerequisites that line up with this page, see [Prerequisites](/get-started/before-you-start/#sidekick-browser-extension).
...
For the full command reference and troubleshooting, such as a failed install or port `3000` already in use, see [Running locally](/boilerplate/#running-locally) on the boilerplate overview page.
...
<Aside type="tip" title="What you have now">
  You have a Commerce storefront project on GitHub, starter content in Document Authoring, a local clone, and a working local preview showing your own Commerce data. Next, use [Boilerplate overview](/boilerplate/) to explore the code, then customize the boilerplate and <Term term="Drop-in components">drop-in components</Term> to match your project.
</Aside>
...
- [Boilerplate overview](/boilerplate/) — Install paths, `npm` scripts, and a typical local loop after your repository exists.
...
- [Drop-ins overview](/dropins/all/introduction/) and [Connect a drop-in](/dropins/all/quick-start/) — Initializer flow, slots, and styling when you are ready to change Commerce UI.
