...
This walkthrough uses **Document Authoring (DA.live)** as the content source — the default for new sites created with EDS. If your project uses **AEM Sites** as the content source (for example, you are adding Commerce drop-ins to an existing AEM as a Cloud Service site), the content setup steps and Universal Editor instrumentation differ. See <Link href="https://github.com/adobe-rnd/aem-boilerplate-xcom" text="the XCOM boilerplate"/> and <Link href="https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/edge-delivery/overview" text="Edge Delivery Services with AEM Sites" /> on Experience League for that path.
</Aside>
