...
1. **Add Content** to the Document Author environment hosted at `https://da.live/` using the [Adobe Commerce Site Creator app](https://da.live/app/adobe-commerce/storefront-tools/tools/site-creator/site-creator) in DA to create a new storefront based on the Commerce boilerplate.
...
Storefront supports three distinct customer scenarios. Each has different prerequisites and setup steps—**do not mix guidance** between them. See the [Storefront Architecture](/get-started/architecture/#backend-options) page for details.

<TableWrapper nowrap={[0]}>

| Scenario | Description | Prerequisites |
|----------|-------------|---------------|
| **Commerce PaaS** | Existing Adobe Commerce Platform-as-a-Service customers who previously accessed Storefront without Adobe Commerce as a Cloud Service or Adobe Commerce Optimizer licenses | Manual installation: Storefront Compatibility package, Storefront services (Data Connection, Services Connector, Catalog Service, Live Search, Product Recommendations). See [PaaS prerequisites](/get-started/architecture/#commerce-platform-as-a-service-paas). |
| **Adobe Commerce as a Cloud Service** | New Adobe Commerce as a Cloud Service customers whose license includes Storefront | All requirements and services are automatically managed. See [Adobe Commerce as a Cloud Service prerequisites](/get-started/architecture/#adobe-commerce-as-a-cloud-service). |
| **Adobe Commerce Optimizer** | New Adobe Commerce Optimizer customers whose license includes Storefront, or existing Commerce customers who add Adobe Commerce Optimizer | Fully managed SaaS. See [Adobe Commerce Optimizer prerequisites](/get-started/architecture/#adobe-commerce-optimizer) for supported backend combinations. |

</TableWrapper>

<Aside type="note" title="Before you start">
- **Commerce PaaS**: Complete the [PaaS prerequisites](/get-started/architecture/#commerce-platform-as-a-service-paas) (Storefront Compatibility package, Data Connection service, Services Connector, Catalog Service, Live Search, Product Recommendations) before starting. Setup requires manual installation—see the [Storefront Architecture](/get-started/architecture/#commerce-platform-as-a-service-paas) page.
- **Adobe Commerce as a Cloud Service**: No additional prerequisites. All backend requirements and services are automatically managed by Adobe—see the [Storefront Architecture](/get-started/architecture/#adobe-commerce-as-a-cloud-service) page.
- **Adobe Commerce Optimizer**: No additional prerequisites. All backend requirements and services are automatically managed—see the [Storefront Architecture](/get-started/architecture/#adobe-commerce-optimizer) page.
</Aside>
...
<LinkCard
  ...
  link="/get-started/architecture/#backend-options"
...
ACCS and ACO customers follow the same repo creation steps above. Your backend services are already provisioned — the difference comes in the next step when you configure `config.json` to point to your managed environment.
</Aside>
...
The Code Sync app redeploys your storefront site whenever you push or merge changes to the `main` branch of your repo. This step is the same for all backends (PaaS, ACCS, and ACO).
...
To link your storefront repo to your commerce backend, use the [config generator tool](https://da.live/app/adobe-commerce/storefront-tools/tools/config-generator/config-generator) to automatically generate a `config.json` file for your site based on your Commerce backend. The configuration differs significantly by backend — follow only the tab for your backend.
...
ACCS uses standard Adobe Commerce header naming conventions but does **not** require API keys or environment IDs. Your managed environment provides these automatically.
...
Adobe Commerce Optimizer uses **different header naming conventions** from PaaS and ACCS. Do not use standard Commerce headers.

**Required values:**
- Commerce GraphQL endpoint URL (provided by Adobe during provisioning)
- `AC-View-ID` (your Optimizer view identifier)
- `AC-Source-Locale`
- `AC-Price-Book-ID`

See the [Merchandising Services API documentation](https://developer.adobe.com/commerce/services/optimizer/merchandising-services/using-the-api/#headers) for header details and the [Storefront configuration](/setup/configuration/commerce-configuration/) reference for the full schema.
...
1. Open the [config generator tool](https://da.live/app/adobe-commerce/storefront-tools/tools/config-generator/config-generator) and select your backend type to generate the correct `config.json` structure.
...
<Callouts>
1. Open the [Site Creator app](https://da.live/app/adobe-commerce/storefront-tools/tools/site-creator/site-creator) in DA, and select **Use existing repository**.
...
<Aside type="tip" title="Adobe Commerce as a Cloud Service / Adobe Commerce Optimizer">
ACCS and ACO customers use the same Site Creator and DA.live content workflow as PaaS. The difference is in your backend configuration (`config.json`), which you already set up in the previous step. Your demo content will connect to your managed environment automatically.
</Aside>
...
The [Universal Editor](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/sites/authoring/universal-editor/authoring) enables visual, in-context editing of your storefront content directly on the rendered page. Two editing options are available depending on your authoring setup:

<Tabs>

<TabItem label="Visual Editor (Document Authoring)">

If your content is managed in DA.live (the default for storefronts created with the Commerce boilerplate), use the **Visual Editor** — a subset of Universal Editor features built for DA.live-hosted content.

<Steps>

1. Open your storefront page in DA.live.
2. Select the **Visual Editor** option from the editing toolbar to switch from document mode to visual mode.
3. Click any element on the rendered page to edit text, images, or blocks in context.
4. Changes save back to your DA.live content and can be previewed and published using Sidekick.

</Steps>

See [Using the Visual Editor](/merchants/quick-start/visual-editor/) for the full guide.

</TabItem>

<TabItem label="Universal Editor (AEM Sites authoring)">

If your storefront is configured for [AEM Sites authoring](https://www.aem.live/docs/publishing-from-authoring), you can use the full Universal Editor for richer in-context editing.

<Steps>

1. Configure your storefront for AEM Sites authoring per the [publishing from authoring](https://www.aem.live/docs/publishing-from-authoring) guide.
2. Open your storefront page URL in the Universal Editor at `https://experience.adobe.com/#/aem/editor`.
3. Edit content blocks, commerce blocks, text, and images directly on the rendered page.
4. Publish changes through the AEM Sites workflow.
...
<Aside type="caution" title="Block support limitations">
The Commerce boilerplate is still being instrumented for Universal Editor support. Not all blocks may render or be editable in the Universal Editor yet. Content blocks (Cards, Columns, Headers) have the broadest support. Some Commerce blocks (cart, checkout) may have limited visual editing capabilities. Check the [changelog](/releases/changelog/) for the latest UE instrumentation updates.
</Aside>
...
The [Universal Editor](/merchants/quick-start/universal-editor/) enables visual, in-context editing of your storefront content directly on the rendered page. You can edit text, images, blocks, and commerce components with immediate feedback.

See [Using the Universal Editor](/merchants/quick-start/universal-editor/) for setup instructions and usage details.
...
Sidekick is automatically installed on new EDS sites. Configuration of Sidekick is now managed via the site config. Visit the [Sidekick documentation](https://www.aem.live/docs/sidekick) for more information on how to use Sidekick to manage your storefront content.
</Aside>
