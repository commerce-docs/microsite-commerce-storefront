...
Use **one** row below—the scenario that matches how you run Commerce and Storefront. Full prerequisite lists, service names, and install guidance stay on the [Backend options](/get-started/backends/) page so this tutorial stays short.

<TableWrapper nowrap={[0]}>

| Scenario | What to read next |
|----------|-------------------|
| **Commerce PaaS** | [Commerce PaaS prerequisites and storefront services](/get-started/backends/#commerce-platform-as-a-service-paas) |
| **Adobe Commerce as a Cloud Service** | [Adobe Commerce as a Cloud Service prerequisites](/get-started/backends/#adobe-commerce-as-a-cloud-service) |
| **Adobe Commerce Optimizer** | [Adobe Commerce Optimizer prerequisites](/get-started/backends/#adobe-commerce-optimizer) |
...
An app in Document Author (DA.live) that creates and initializes the content side of your storefront. It creates your content folder, connects your code repository to that content, and lets you choose content only or code and content with a base theme for B2C (default) or B2B. The app generates the [storefront configuration](/setup/configuration/commerce-configuration/) and fills in values from the API where possible.
