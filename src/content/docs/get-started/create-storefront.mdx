...
This topic is the main place to create a new Adobe Commerce storefront on Edge Delivery Services from the Commerce boilerplate. Many developers open the boilerplate section of the docs only after they already have a repository. If you still need a site, start here and pick one path.

<Steps>

1. **Site Creator** in Document Author (DA.live)
   Use the <Link href="https://da.live/app/adobe-commerce/storefront-tools/tools/site-creator/site-creator" text="Site Creator tool" /> to create boilerplate code and starter content in one guided flow. This is the fastest way to a working starter when you want the tool to drive setup.

1. **Full walkthrough** on this page
   Continue with Big picture and Which backend, then follow Step-by-step from the GitHub template through Code Sync, Commerce configuration, content in Document Author, and local development.

</Steps>

If you already used Site Creator, imported an existing Edge Delivery Services site, or cloned a team repo, skim Big picture so you see how the pieces connect. Jump to [Set up local environment](#set-up-local-environment) when you are ready to clone and run the project on your machine.

We still start with a big-picture overview and vocabulary before the hands-on tasks so you know why each step exists.
...
4. **Add Content** to the Document Author environment hosted at `https://da.live/` using the <Link href={"https://da.live/app/adobe-commerce/storefront-tools/tools/site-creator/site-creator"} text={"Site Creator tool"} />.
