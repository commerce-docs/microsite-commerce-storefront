...
Plan about 30 minutes for five short topics. Together they give you a working map of documents, delivery, blocks, drop-ins, events, and Commerce services. You will also see which layer to check first when a block will not render, a drop-in shows no data, or Commerce calls fail.
...
Read in order the first time through. Each topic builds on the previous one. After you have run the boilerplate once, you can jump ahead for lookup. If you open [Events](/dropins/all/events/) on day one with no context, it can feel out of order; the architecture pages above prepare you for that topic.
