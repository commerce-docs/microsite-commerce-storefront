...
You've seen what an individual drop-in does. This page explains what happens when more than one drop-in is on the same page — for example, when both a Cart drop-in and an Auth drop-in need to know whether the shopper is signed in. Drop-in components communicate through a shared <Term>event bus</Term> (`@dropins/tools/event-bus.js`) using a publish-subscribe pattern. This keeps them loosely coupled so no drop-in component depends directly on another.
