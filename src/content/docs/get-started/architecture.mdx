...
Edge Delivery hosts your site. **Commerce blocks** use the boilerplate and **drop-in components** to reach your Commerce backend and services. **Content blocks** do not. This topic shows the overall architecture, then explains how each part fits together.
...
The figure below shows the high-level composable story: content and commerce both publish through Edge Delivery.
...
When you understand how the pieces fit together, use [Commerce configuration](/setup/configuration/commerce-configuration/) to align your storefront with your Commerce backend. See [Backend options](/get-started/backends/) for how backend types differ.

## How a page loads

The [system stack diagram](#how-the-pieces-connect) is structural. This diagram adds the time dimension: what happens from document authoring to a Commerce API call when the page loads. The sequence reads left to right (author → Edge Delivery → browser → decorator → drop-in → Commerce API).
...
Drop-in components are domain-driven Commerce UI packages (for example, `npm install @dropins/storefront-cart`). Commerce blocks and initializers mount them and call Commerce APIs. Customize them using the [documented extension patterns](/dropins/all/extending/).

The [Drop-ins introduction](/dropins/all/introduction/) lists the current B2C and shared packages. For B2B flows (for example, company, quote, purchase order, requisition lists), see [Drop-ins B2B](/dropins-b2b/).

Representative B2C `@dropins/storefront-*` packages include:
