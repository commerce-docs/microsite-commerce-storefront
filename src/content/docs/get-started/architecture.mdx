...
flowchart TB
    Table["<b>Document table</b><br/>Authored in DA.live or Universal Editor"]
    Table -->|"EDS transforms to HTML"| Block["<b>HTML block</b><br/>&lt;div class='block-name'&gt;"]
    Block --> ContentPath["<b>Content block</b><br/>Hero, card, column<br/>Stays presentational"]
    Block --> CommercePath["<b>Commerce block</b><br/>Cart, checkout, PDP<br/>Decorator runs in browser"]
    CommercePath -->|"mounts"| DropIn["<b>Drop-in component</b><br/>@dropins/storefront-* or @dropins/b2b-*<br/>UI + Commerce logic"]
    DropIn -->|"API calls from browser"| API["<b>Adobe Commerce APIs</b><br/>GraphQL · REST · Services"]
    Boilerplate["<b>Your repository</b><br/>decorators · initializers · config.json"]
    CommercePath -.->|"decorator loaded from"| Boilerplate
    Boilerplate -.->|"initializes"| DropIn
    classDef eds fill:#e8f5e9,stroke:#4caf50
    classDef commerce fill:#e3f2fd,stroke:#2196f3
    classDef api fill:#fff3e0,stroke:#ff9800
    class Table,Block,ContentPath eds
    class CommercePath,DropIn,Boilerplate commerce
    class API api
`} caption="A document table authors both content and commerce blocks through the same EDS pipeline. Content blocks stay presentational. Commerce blocks load drop-in components through the boilerplate, which calls Adobe Commerce APIs directly from the browser." />

### Documents, blocks, and your repository

Storefront **pages start as documents**. Authors build them in **[Document Authoring](/merchants/quick-start/document-authoring/)** (DA.live), **Google Docs**, **SharePoint**, or the **[Universal Editor](/merchants/quick-start/universal-editor/)**, structuring the page with **tables**.
...
Although not shown in this diagram, <Link href="https://developer.adobe.com/graphql-mesh-gateway/" text="API Mesh for Adobe Developer App Builder" /> is an option for integrating multiple APIs into a single endpoint. It can aggregate data from multiple sources and expose a single GraphQL endpoint for the storefront to consume.
</Aside>
...
Understanding how content transforms into rendered commerce experiences helps you know where to customize and integrate. The [system stack diagram](#how-the-pieces-connect) is structural. This diagram adds the time dimension: what happens from document authoring to a Commerce API call when the page loads. The sequence reads left to right (author → Edge Delivery Services → browser → decorator → drop-in → Commerce API).
...
Blocks are the fundamental parts of a page delivered by Edge Delivery Services. Each block encapsulates the styling and code that drives a logical component of the page. Edge Delivery Services comes with a comprehensive library of predefined content and commerce blocks that can be customized to meet your project needs. Both block types are authored as document tables. Content blocks stay presentational; commerce blocks load drop-in components through the boilerplate.

#### Content blocks

Content blocks provide layout and marketing UI for **non-commerce** pages on the storefront. They include Cards, Columns, Headers, Footers, and more from the <Link href="https://www.aem.live/developer/block-collection" text="Block Collection" />. Edge Delivery renders them as HTML and optional decoration. Developers can enrich them with semantic HTML, accessibility attributes, and dynamic DOM transformations. They do not mount drop-in components or call Adobe Commerce GraphQL.
...
Commerce blocks that need complex client-side state and multiple render states can follow the buildless <Link href="https://preactjs.com/" text="Preact" /> and <Link href="https://github.com/developit/htm" text="HTM" /> patterns used in the boilerplate. Otherwise, prefer plain JavaScript. React is possible but not recommended—its bundle size and execution overhead make it difficult to achieve a 100 Lighthouse score. For a short rule of thumb, see [Libraries](/troubleshooting/faq/#libraries) in the storefront FAQ. Use the existing blocks in the Commerce boilerplate as a reference—for example, the teaser and product recommendations blocks.
...
The boilerplate is organized into four main layers that build on each other to deliver a modular, maintainable storefront architecture. The table maps the [boilerplate `scripts/` folder](https://github.com/hlxsites/aem-boilerplate-commerce/tree/main/scripts) and `blocks/` in the [Adobe Commerce boilerplate](https://github.com/hlxsites/aem-boilerplate-commerce) repository.
...
| `scripts/commerce.js` | Commerce-specific loading, templates, page type detection, Adobe Data Layer initialization, and storefront configuration from `config.json`. Centralizes all commerce features and keeps them distinct from core AEM logic. |
...
Storefront services are a set of multi-tenant services (shared application, many users, separate data) that provide access to storefront data via GraphQL APIs. These APIs are available in addition to the core Adobe Commerce GraphQL APIs—they are not a replacement for them. They are a hard requirement for building a storefront on Edge Delivery Services: they are the only APIs that deliver the performance needed to achieve a 100 Lighthouse score. Unlike the Commerce application itself, they are not bound by its scaling constraints. They provide fast, read-only access to catalog data, and their synchronized data is stored in a SaaS data space available through Adobe IMS.
...
| **Storefront Compatibility package** | Extends the GraphQL schema by adding new mutations and missing fields required for cart, checkout, account, and order drop-in components. PaaS: manual install required. Adobe Commerce as a Cloud Service and Adobe Commerce Optimizer: managed automatically. | [Install guide](/setup/configuration/storefront-compatibility/install/) |
