...
1. Use [AEM's Configuration Service](https://www.aem.live/docs/config-service-setup) and the `PublicConfig` schema from the [Site Config API docs(https://www.aem.live/docs/admin.html#tag/siteConfig).

1. Use the `config.json` in your storefront repo (templated from the boilerplate). This JSON object overrides the key-values found in the `PublicConfig` schema. You can use [demo-config.json](https://github.com/hlxsites/aem-boilerplate-commerce/blob/main/demo-config.json) as a reference.
