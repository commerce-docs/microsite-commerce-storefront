...
1. Install Sidekick from the Chrome Web Store and pin the Sidekick extension to your browser's toolbar.
...
1. After you are signed in, select the **Options** icon (three horizontal lines ⫢) from the Sidekick menu and select **Add this project** from the options.
1. In your content folder, select all content files, then select the **Preview** button from the Sidekick overlay at the bottom of your browser page.  Repeat this for all files in each folder and each subfolder. Select all the files again, but select the **Publish** button to publish these files to the CDN. These actions add your content to the internal (preview) and production (publish) Edge Delivery CDNs, where they can be downloaded and displayed in your browser for local development.
