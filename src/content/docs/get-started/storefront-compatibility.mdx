...
The Storefront Compatibility Package contains changes to the Adobe Commerce 2.4.7 codebase that enable dropin functionality. Most of these changes enhance the Adobe Commerce GraphQL schema. However, miscellaneous bugfixes are also included. Cart, checkout, user account, and user authentication dropins require the package.

Partners can request access to the Storefront Compatibility Package by sending an email to `commerce-storefront-compatibility@adobe.com`.
...
* Corrected an error that occurred when using the `customer` to retrieve returns with the "Processed and Closed" status.
...
* The `updateCustomerEmail` mutation allowa the email to be changed when a custom attribute is not defined for the customer.
