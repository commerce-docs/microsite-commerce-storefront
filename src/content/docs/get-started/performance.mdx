...
If your storefront feels slow or your mobile scores look low, tune `delayed.js`, CSS load order, and `head.html` to match the <Term>Edge Delivery Services</Term> eager, lazy, and delayed loading pattern.
...
Don't load the scripts, fragments, and styles that render a piece of UI for shoppers who may never need them.

Treat any UI that stays hidden until the shopper interacts with it as lazy: load what it needs only on the interaction that reveals it, such as a click or tap, not before.

The Mini Cart panel is a good example. Most visitors never open it, so the scripts, fragment, and styles it needs to render shouldn't load until they do. Load the fragment on the click that opens the panel, and update the item-count badge, the piece shoppers always see, straight from event data instead.

The same rule applies to any drop-in container, fragment, or block that's hidden by default: request what it needs when the shopper's action reveals it, not on every page load.
