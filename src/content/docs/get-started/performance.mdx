...
You'll get the most from this page if you already set up a storefront using [Create a storefront](/get-started/create-storefront/). Then you can open the files this page mentions in your own project folder while you read.

The Commerce boilerplate uses the same <Term>Edge Delivery Services</Term> performance pattern as other storefronts. That pattern covers eager loading, lazy loading, and delayed loading for scripts and styles, plus fonts and LCP (Largest Contentful Paint, a measure of how fast the main content appears). The sections below map those ideas to boilerplate files.

Keep <Link href="https://www.aem.live/developer/keeping-it-100" text="Keeping it 100" /> open while you edit `head.html` or split CSS so you stay inside the limits it describes. After you change how scripts or styles load, run [Lighthouse audits](#run-lighthouse-audits) and review [Performance and monitoring](/setup/launch/#performance-and-monitoring) on the launch checklist.
