---
title: Overview
description: Learn about Adobe Commerce on Edge Delivery Services project requirements, data validation, drop-ins, and Luma Bridge.
---
...
1. **Drop-in components**: Develop and integrate drop-ins to enhance the storefront experience.
...
Document your use cases and requirements and create a plan for how you will implement them. Adobe offers pre-built components that accelerates development (drop-ins and widgets).

Drop-ins are re-useable components that define the storefront shopping experience. They are framework agnostic and can be used in any context (Edge Delivery Services, AEM, Luma). However, this documentation focuses on the use of drop-ins in Edge Delivery Services projects using the [boilerplate template](https://github.com/hlxsites/aem-boilerplate-commerce). The drop-in development roadmap is synchronised with Adobe Commerce APIs, so new API features are automatically available in drop-ins and widgets.
...
:::note
See the [drop-ins overview](/dropins/all/introduction/) for a list of all available drop-ins.
:::
...
- Identify if drop-ins can solve your use cases and requirements
