...
The [Storefront Compatibility package](/setup/storefront-compatibility/) extends the GraphQL schema provided with Adobe Commerce. It provides new mutations and adds missing fields that are needed to implement low-funnel drop-ins, such as the cart, checkout, user account, and order drop-ins.
