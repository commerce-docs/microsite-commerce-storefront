...
slug: setup/configuration/multistore-setup
---
...
1. **Multiple Domains**: Each domain is configured as its own Edge Delivery Services site, pointing to the same code and content repository. See [Multiple Edge Delivery Services sites on one Fastly service](/setup/configuration/content-delivery-network/#multiple-edge-delivery-services-sites-on-one-fastly-service) for how to route and configure push invalidation when these sites share one Fastly service.
2. **Subfolders (this page)**: A single domain with subfolders (such as `/en/`, `/fr/`) for each locale or store. Each subfolder can have its own localized content and Commerce configuration.
...
Adobe Commerce uses a <Link href={"https://experienceleague.adobe.com/en/docs/commerce-admin/start/setup/websites-stores-views"} text={"hierarchical structure"} /> to manage multiple stores within a single instance. This structure consists of three levels: websites, stores, and store views. Localization is managed at the store view level, allowing merchants to present the store in different languages and apply the proper currency.

Each store or locale is defined in configuration files. These files specify which headers to send to the commerce backend. They also define which content folders to use and how to structure localized content. The content hierarchy follows <Link href={"https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/edge-delivery/overview"} text={"Edge Delivery Services"} /> best practices, emphasizing a single-tier structure.
...
For translating placeholder files, see [Commerce localization tasks](/dropins/all/commerce-tasks/). For technical implementation details, see [Labeling and Localizing Drop-In Components](/dropins/all/labeling/).
...
- Access to Adobe Commerce Admin for <Link href={"https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/site-store/store-views"} text={"store view configuration"} />
- Your [Storefront Configuration](/setup/configuration/commerce-configuration/) set up and pointing to the Adobe Commerce instance
- Familiarity with <Link href={"https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/edge-delivery/document-authoring/authoring"} text={"Authoring and Publishing Content"} />
- Understanding of [Storefront Configuration](/setup/configuration/commerce-configuration)
- Knowledge of <Link href={"https://experienceleague.adobe.com/en/docs/commerce-admin/config/scope-change"} text={"Adobe Commerce Configuration Scope"} />

---

## Implementation walkthrough

This walkthrough demonstrates the process of expanding the Acme brand from the United States to the Canadian market with two new locales: Canadian English (`en-ca`) and Canadian French (`fr-ca`).
...
To support the new store views, set up the folder structure in your Document Authoring environment.
...
To match your content folder structure, create the corresponding store views in Adobe Commerce Admin.
...
If you don't receive a response, you may not have folder mapping yet. To handle this case, create the JSON object manually in step 3.
...
To include configuration overrides for the new store view, update your [storefront config](/setup/configuration/commerce-configuration/). In this file, you'll define store-view specific settings, such as service headers and analytics values.

The `key` of each overridden object must match the root folder path of the corresponding store view. These values merge into the default configuration.
...
If you are using the Config Service, read the <Link href={"https://www.aem.live/docs/admin.html#tag/siteConfig/operation/updateConfigSite"} text={"API documentation"} /> to learn how to publish your updates to the Config Service for your site.
</Aside>
...
**Important:** The boilerplate automatically localizes internal links to keep users within their current locale (see [Localizing links](/dropins/all/linking/) for technical details). To prevent automatic localization for links that point to a specific locale (such as store switcher links), add the hash `#nolocal`:
...
To ensure proper functionality before publishing, test your new store views.
...
<Steps>
1. **Translate content files** in the `/en-ca/` and `/fr-ca/` folders using the [Localization (Document Authoring Tool)](/merchants/quick-start/content-localization/) workflow.

2. **Complete Commerce-specific tasks**: Follow [Commerce localization tasks](/dropins/all/commerce-tasks/) to:
...
2. **Preview all files** in the updated folders (`/en-ca/`, `/fr-ca/`) with <Link href={"https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/edge-delivery/resources/sidekick/sidekick"} text={"AEM Sidekick"} />.
...
- **Check the folder mapping**: Verify that the folder mapping is correct by querying the Config Service: `GET https://admin.hlx.page/config/{ORG}/sites/{SITE}/folders.json`. All folder mapping changes must go through the Config Service API.
...
- [Wishlist multistore support](/dropins/wishlist/multistore/) — Per-store-view wishlist data isolation
...
- <Link href={"https://www.aem.live/developer/folder-mapping"} text={"Folder Mapping"} /> - Edge Delivery Services documentation
- <Link href={"https://www.aem.live/docs/config-service-setup"} text={"Config Service Setup"} /> - Configuration Service documentation
