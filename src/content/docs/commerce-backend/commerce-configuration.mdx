...
Most Commerce blocks read configuration through a drop-in's initializer in `scripts/initializers/`, rather than calling `getConfigValue` directly. The initializer wires the resolved endpoint into the drop-in before the block ever renders.
