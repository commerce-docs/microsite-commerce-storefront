...
slug: setup/configuration/commerce-configuration
---
...
To generate a `config.json` file for your Commerce backend, use the config generator tool to set up your storefront using the boilerplate code. This file contains the environment values for a Commerce backend including GraphQL endpoints and headers.
...
This page covers your <Term>storefront configuration</Term>, when to replace sample <Term>default values</Term>, and how helper APIs such as <Term>getConfigValue function</Term> and <Term>getHeaders function</Term> read resolved values.

## Configuration Caching

The storefront configuration is cached at multiple levels to optimize performance:

- **CDN Caching**: The EDS CDN caches the configuration file with a `max-age=7200` (2 hours) cache control header. This means configuration changes can take up to 2 hours to be reflected on the CDN after deployment.
...
- Or use the <Link href="https://da.live/app/adobe-commerce/storefront-tools/tools/config-generator/config-generator" text="Config Generator tool" /> to generate a configuration with your backend values (contains placeholders to fill in).
...
The multistore section manages configurations for multiple store views and internationalization. This allows different configurations for different locales or store views. Each non-"default" configuration merges with the default at runtime and applies to the corresponding locale or store view. For example, the below serves as a foundation for configuration for the site at aemshop.net/fr.
...
- **`commerce-b2b-enabled`** - Boolean flag to enable or disable B2B-related global and shared logic within the storefront. When enabled, this flag is passed to the Auth initializer as `customerPermissionRoles: true` to enable customer role permissions API. This flag controls B2B functionality at the authentication and authorization level, enabling the drop-in to fetch B2B customer role permissions automatically on initialization and re-fetch when authentication state changes. See the [User Auth initialization documentation](/dropins/user-auth/initialization/) for details.
...
<Aside type="note" title="B2B Configuration Flags">
**Understanding the difference:**

- **`commerce-b2b-enabled`** - Controls B2B functionality at the authentication and authorization level. When set to `true`, this flag enables the customer role permissions API in the Auth drop-in (via the `customerPermissionRoles` parameter), which automatically fetches B2B customer role permissions including purchase order and company management capabilities. This configuration is required for B2B features to work properly. See the [User Authentication changelog](/releases/changelog/user-authentication/) for implementation details.
...
**Relationship:** `commerce-b2b-enabled` is required for B2B features as it enables the authentication system. `commerce-companies-enabled` controls company features, which require `commerce-b2b-enabled` to be `true`.
...
1. **Config Service (recommended for production)**: Use the Config Service to store and manage your production configuration. To get started, see the <Link href="https://www.aem.live/docs/config-service-setup" text="Config Service setup guide" />. Your configuration is stored at `https://admin.hlx.page/config/{ORG}/sites/{SITE}/public.json` and served automatically — no `config.json` in the repository is needed.

2. **Generate a local starter config**: Use the <Link href="https://da.live/app/adobe-commerce/storefront-tools/tools/config-generator/config-generator" text="config generator tool" /> to generate a configuration for your Commerce backend automatically. Use this as the basis for your Config Service upload or for local development. Note that the generated config contains placeholders that you need to replace with actual values.
...
- When deployed, `https://[branch]--[site]--[org].aem.page/config.json` serves the configuration for your branch.
- The `main` branch and other branches are unaffected.
- Public config is NOT used for this branch.

**Committing to main branch:**
- If you commit `config.json` to `main`, it is ALWAYS served.
- Public config from EDS site config will NEVER be used (unless you delete `config.json` from main).
- All branches without their own `config.json` inherit the configuration from main.
...
- Any branch with a committed `config.json` uses it (on that branch) instead of this public config.

### Testing Configuration Changes

To verify the changes after creating or updating your configuration (either in public config or `config.json`), follow these steps:
...
3. **Reload the page:**
   ...
   - Or reload after clearing session storage
...
Beyond `config.json`, the boilerplate includes template configuration files with placeholders for the rest of your production site setup.
...
A mock PDP / Catalog Service block demonstrates how to utilize the config utilities and values. This example shows how to access the configuration sections detailed above.
...
To make authenticated API requests to your Commerce backend, combine the endpoint and header configuration.
...
### Configure a drop-in initializer.

Most Commerce blocks read configuration through a drop-in's initializer in `scripts/initializers/`, rather than calling `getConfigValue` directly. The initializer connects the resolved endpoint into the drop-in before the block renders.
...
The Storefront configuration provides a structured approach to connecting your storefront to Adobe Commerce backends. By organizing settings into clear sections for endpoints, headers, analytics, plugins, and multistore configurations, you can easily customize your storefront for different Commerce environments. The configuration system supports Adobe Commerce (PaaS), Adobe Commerce as a Cloud Service (SaaS), and Adobe Commerce Optimizer, each with specific header requirements and authentication methods. For local development and testing, use a `config.json` file in your repository or the Configuration Service public config, as repository files take precedence. Use the `getConfigValue` and `getHeaders` helper functions to access these settings in your Commerce blocks, ensuring consistent and maintainable integration with your Commerce backend.
