...
The boilerplate requires configuration at two levels:

- **Commerce backend connection:** `demo-config.json` (local) or Configuration Service (production) provides Adobe Commerce GraphQL endpoints, API credentials, store settings, and analytics configuration.

- **Drop-in initialization:** Files in `scripts/initializers/` set up each drop-in by specifying which GraphQL client to use, loading language labels from placeholder files, and passing runtime options to the drop-in.
...
1. **Configuration Service (Recommended)** - Use the `.aem.live` Configuration Service, which stores configuration at `https://admin.hlx.page/config/{ORG}/sites/{SITE}/public.json`. This approach stores configuration in the same format as the `public` section in `default-site.json` and allows configuration updates without code deployments. Note that a local `config.json` in your repository will override the Configuration Service.
