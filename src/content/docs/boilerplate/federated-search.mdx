...
- Familiarity with [customizing blocks](/boilerplate/customizing-blocks/) in the your code repository
...
This federated search approach involves:

1. Creating a content index for your site's pages and content
...
4. Handling search queries in parallel for optimal performance

This approach keeps your content and commerce searches separate but presents them as a unified experience to the user.

<Aside type="tip">
**Note:** This approach is ideal when you want quick implementation, have straightforward search requirements, or want to maintain separate control over content and commerce search logic.
</Aside>
...
First, you need to configure a content index that will crawl and index your site's content pages. This is done by creating a `query.yaml` configuration file.

   Create the configuration by making a POST request to your site's admin config endpoint:

   ```bash
   POST https://admin.hlx.page/config/[owner]/[repo]/[branch]/query.yaml
   ```

   Replace `[owner]`, `[repo]`, and `[branch]` with your site's GitHub details.
   ...
   **Configuration details:**
   ...
   - `properties`: Metadata fields to extract from each page
   </Aside>

   After creating this configuration, the system will automatically generate a content index at `https://[your-domain]/content-index.json` whenever you publish pages.

**Expected result:** You should be able to access your content index at the configured target URL and see a JSON array of indexed pages (after you publish the pages).

### Step 2: Create Sample Content Pages

To test your federated search, create and publish some content pages that will be indexed. For example, create and publish blog posts in a `/blog` directory:
...
<Aside type="tip">
**Content tip:** Include relevant keywords in your content that shoppers might search for. The content index contains the full text content of your pages, so descriptive content improves search results.
</Aside>

### Step 3: Create the Combined Search Block

Now create a custom block that will perform federated search. This block will handle both the content and commerce searches.
...
<Aside type="note">
**Full implementation:** The code below shows the key concepts. For a complete working example with full rendering logic, see the [reference implementation](https://gist.github.com/sirugh/6690b7ded67868f3b17d3c1b5a29dc76).
</Aside>
  ...
  block.innerHTML = `
    <div class="combined-search-container">
      <div class="search-input-wrapper">
        <input type="text" class="search-input" value="${query}" />
...
4. **Parallel execution** (`performSearch`): Both searches run simultaneously using `Promise.all()`, which means the slower of the two doesn't hold up the faster one.

5. **Tabbed UI**: Results are organized in separate tabs (Products and Content) for a clean user experience.

<Aside type="tip">
**Customization:** You can add additional filters (like category or price range), adjust pagination with `pageSize`, add sorting options, customize the content search algorithm in `searchContent()`, or modify the UI structure to display results differently (e.g., in a single list instead of tabs).
</Aside>
...
Create a new page in your project to host the combined search block. In your content authoring tool (e.g., DA.live, Google Docs, SharePoint), create a page at `/search`:

![Example search page](images/federated-search-search-example.png)

This creates a page that uses your `combined-search` block.
...
Now let's verify everything is working correctly:
   ...
   1. **Test the content index:**
      - Navigate to `https://[your-domain]/content-index.json`
...
**Technical Flow:**
```javascript
// 1. User searches for "winter"
performSearch('winter', block)

// 2. Parallel requests
Promise.all([
  getContentIndex(),      // Fetch/use cached content-index.json
  searchProducts('winter') // GraphQL query to commerce
])

// 3. Filter content index client-side
searchContent('winter', contentIndex) // Returns matching content

// 4. Render both result sets in tabs
renderContentResults(contentResults)
renderProductResults(productResults)
```
...
Now that you've implemented federated search, you might want to:

<LinkCard
  title="Customize the Product Discovery Dropin"
  description="Learn how to further customize product search and filtering"
  href="/dropins/product-discovery/"
/>

<LinkCard
  title="Content Authoring Best Practices"
  description="Optimize your content for better search results"
  href="/merchants/content-customizations/"
/>

<LinkCard
  title="SEO Configuration"
  description="Ensure your content and products are discoverable"
  href="/setup/seo/"
/>
...
**Issue:** Content index returns 404 error
**Solution:** Verify that you've correctly configured the `query.yaml` file and that you've published pages that match your include patterns. The index is only generated after the configuration is in place and pages are published.

**Issue:** No products appear in search results
**Solution:** Check that your GraphQL endpoint is correctly configured. Verify in browser console that the GraphQL query is executing without errors. Ensure your Adobe Commerce instance has products with the searched terms.

**Issue:** Content search returns too many irrelevant results
**Solution:** Improve the search algorithm in the `searchContent()` function. Consider implementing:
...
Instead of tabs, you could display content and products in a single view with section headers:

```javascript
function renderUnifiedResults(contentResults, productResults, container) {
  container.innerHTML = `
    <section class="results-section">
      <h2>Products</h2>
      <div class="product-results-list">
        ${renderProductItems(productResults)}
      </div>
    </section>
    <section class="results-section">
      <h2>Content & Resources</h2>
      <div class="content-results-list">
        ${renderContentItems(contentResults)}
      </div>
    </section>
  `;
}
...
This provides a single scrolling experience but may be less organized for large result sets.
