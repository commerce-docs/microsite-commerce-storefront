...
indices:
  content:
    target: /product-content-index.json
    exclude:
      - '/blog/index'
    include:
      - /blog/**
...
- Generates a searchable JSON index at `/product-content-index.json`
...
**Verify:** After publishing pages, access `https://[your-domain]/product-content-index.json` to see the indexed pages as JSON.
...
async function getContentIndex() {
  ...
  const response = await fetch('/product-content-index.json');
...
1. **Content index**: Visit `https://[your-domain]/product-content-index.json` - verify pages are indexed with metadata
2. **Product search**: Navigate to `/search?q=shirt` - check Products tab shows results with images, titles, prices
...
{/* TODO: Add screenshot
![Federated search results showing search input, Products and Content tabs, with product cards displayed in the Products tab](images/federated-search-results.png)
*/}
