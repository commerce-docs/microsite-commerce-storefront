...
On Adobe Commerce as a Cloud Service or Adobe Commerce Optimizer, Adobe manages Storefront Compatibility packages on Commerce for you, so upgrades on this page are mainly npm packages and boilerplate changes—not a separate compatibility install checklist. For B2B GraphQL on Commerce PaaS, see [Storefront Compatibility B2B Package](/setup/configuration/storefront-compatibility/b2b/)

<Aside type="tip" title="Commerce PaaS or on-premises">
If you deploy Adobe Commerce on cloud infrastructure or on-premises—Commerce PaaS—and your team installs or upgrades Storefront Compatibility packages on Commerce, align those backend upgrades with storefront work.
</Aside>
