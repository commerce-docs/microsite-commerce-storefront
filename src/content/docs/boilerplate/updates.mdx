...
You can keep your storefront current in two ways:

- **Updating drop-in npm packages** (`@dropins/*`, `@adobe/*`) — This is the primary update path for every forked implementation. Drop-in packages follow semantic versioning: minor and patch releases stay non-breaking by contract. Run `npm outdated` to see available updates, or enable the `update-dropins.yml` GitHub Actions workflow included in the boilerplate to receive automatic weekly pull requests when newer stable versions are available.

- **Merging upstream boilerplate code into your fork** — This path is optional and advanced. Once you fork or clone the boilerplate, that code is yours, and you are under no obligation to pull upstream boilerplate changes into your project. Merging upstream can bring in new integration patterns or bug fixes, but expect manual conflict resolution and a careful review of your customizations. Reserve this path when a specific upstream improvement is worth the effort.
