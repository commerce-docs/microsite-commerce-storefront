...
<Aside type="note" title="Two distinct update paths">
There are two separate ways to keep your storefront current, and it is important to understand the difference:

- **Updating drop-in npm packages** (`@dropins/*`, `@adobe/*`) — This is the primary update path and applies to every forked implementation. Drop-in packages follow semantic versioning: minor and patch releases are non-breaking by contract. Run `npm outdated` to see available updates, or enable the `update-dropins.yml` GitHub Actions workflow included in the boilerplate to receive automatic weekly pull requests whenever newer stable versions are available.

- **Merging upstream boilerplate code into your fork** — This is optional and advanced. Once you fork or clone the boilerplate, that code is yours. You are under no obligation to pull upstream boilerplate changes into your project. Doing so can bring in new integration patterns or bug fixes, but it may require manual conflict resolution and careful review of your customizations. Reserve this path for situations where a specific upstream improvement is worth the effort.

Suite releases are tagged snapshots that validate a specific combination of drop-in versions and boilerplate code. They serve as starting points for new implementations, not as upgrade targets for existing ones.
</Aside>
...
**<Term>Drop-in components</Term>:** npm packages containing the core Commerce functionality, including the following:
...
The `*` in `npm list @dropins/storefront-*` and `npm update @dropins/storefront-*` is a wildcard: npm matches every installed package whose name starts with `@dropins/storefront-`, including these B2B packages when they are in your project. For what each drop-in does and how to connect it in your storefront, see [B2B drop-ins overview](/dropins-b2b/).
...
<Aside type="tip" title="Commerce PaaS or on-premises">
If you deploy Adobe Commerce on cloud infrastructure or on-premises—Commerce PaaS—and your team installs or upgrades Storefront Compatibility packages on Commerce, align those backend upgrades with storefront work. For B2B GraphQL on Commerce PaaS, see [Storefront Compatibility B2B Package](/setup/configuration/storefront-compatibility-b2b/).
</Aside>

**Boilerplate <Term>integration layer</Term>:** storefront-level files that integrate drop-ins
...
When upgrading drop-in packages across major versions (for example, from 1.x to 2.x), perform more thorough testing as these updates may include <Term>breaking changes</Term>. Pay particular attention to:
...
When you need integration layer improvements or new features that require changes to your storefront's integration code, use <Term>selective updates</Term> to merge changes from the <Term>upstream</Term> boilerplate repository.
...
### <Term>Post-install scripts</Term> failures
...
- [Storefront Developer Tutorial](/get-started/create-storefront/): Comprehensive storefront setup page
- [Storefront Architecture](/get-started/architecture/): Understanding the technical foundation
- [Drop-in Components Documentation](/dropins/all/introduction/): Detailed component reference pages
- [B2B drop-ins overview](/dropins-b2b/): B2B packages, containers, and initialization
