...
Older `@dropins/storefront-pdp` releases used a single monolithic container with predefined slots. That pattern is deprecated, is not part of the Commerce boilerplate, and should not be used for new work. The official name and deprecation notice for that entry point are on the [Product Details containers overview](/dropins/product-details/containers/). In that legacy model, teams could only:
...
1. Update dependencies
   ...
   ```javascript
   // Before: one deprecated monolithic entry point (not shown; do not reuse)
...
1. Update block structure
   ...
   ```javascript
   // Before: one deprecated render with a large slots map (not shown; do not reuse)
