---
title: Instrument commerce events without drop-ins
description: Learn how to publish Adobe Client Data Layer (ACDL) commerce events, such as product page views and add-to-cart, when your storefront bypasses or replaces the AEM Commerce drop-ins.
sidebar:
  label: Custom commerce events
prerequisites:
  html: true
  js: true
  commerce: true
...
<Aside type="tip">
For the full list of events drop-ins emit and the contexts each one requires, see the [Custom events reference](/dropins/all/custom-events/).
</Aside>
...
### Step 2: Publish a product page view event
...
### Step 3: Publish an add-to-cart event
...
4. If AEP forwarding is configured, use the AEP Debugger Events view to confirm the events arrive — see [Analytics instrumentation](/setup/analytics/instrumentation/#manual-testing-steps).
...
* **Events push successfully but `eventInfo` is missing storefront or page data**
  ...
  > Confirm `storefrontInstanceContext` and `pageContext` are pushed to the ACDL before any `publish*` call runs. See [page-view](/dropins/all/custom-events/#page-view) in the Custom events reference for when these are normally seeded.
...
- Review the [Custom events reference](/dropins/all/custom-events/) for the full list of events and required contexts
