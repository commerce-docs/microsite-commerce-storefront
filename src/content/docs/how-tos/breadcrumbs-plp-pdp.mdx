...
| Lands on the PDP, authored as `Home / Adobe pattern hoodie` | Check whether the stored `path` matches the URL. It matches, so swap in the stored trail — the shopper now sees `Home / Apparel / Adobe pattern hoodie` |
...
![The commerce-breadcrumbs table placed above the product-list-page table in a DA.live document](images/commerce-breadcrumbs.png)

Set its first row to `commerce-breadcrumbs` — this [block name row](/merchants/blocks/block-tables/#add-a-block-to-your-document) tells Edge Delivery Services which block to render. Populate the row below it with an ordered (`<ol>`) or unordered (`<ul>`) list, using one list item per breadcrumb in the hierarchy. Link every breadcrumb except the last, which represents the current page and remains plain text.

<table style="width: 100%; min-width: 470px; max-width: 100%; table-layout: fixed; border-collapse: collapse;">
<tbody>
<tr>
<td colspan="2" style="text-align: center; border: 1px solid var(--sl-color-gray-5); background-color: var(--sl-color-gray-6); font-weight: 600;">commerce-breadcrumbs</td>
</tr>
<tr>
<td colspan="2" style="padding: 0.75rem; border: 1px solid var(--sl-color-gray-5);">
...
Author the PDP the same way. For example, the published HTML for `/products/adobe-pattern-hoodie/ADB127` looks like this:
...
On a direct visit, it renders as:

```text
Home / Adobe pattern hoodie
```

On a PDP, the last `<li>` doesn't need to match the product name because the `commerce-breadcrumbs` block automatically replaces it with the product's catalog name after the Product Details drop-in loads.
...
<Aside type="note" title="PLPs still need their own breadcrumb">
  Adopting the prerender doesn't change how PLPs get their breadcrumb. Continue authoring the PLP's `<ol>` by hand (see [Step 2](#step-2-add-breadcrumbs-to-a-plp-and-a-pdp) above), or generate it with your own tooling.
</Aside>
