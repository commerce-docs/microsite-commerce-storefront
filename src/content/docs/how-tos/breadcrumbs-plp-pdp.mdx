...
<Aside type="note" title="PLP breadcrumbs and prerendering">
  If your prerender deployment also renders category listing pages, the generated category HTML already includes a breadcrumb `<nav>` and `BreadcrumbList` structured data built from the category's ancestors, so you don't need to author it by hand for those pages. If your prerender deployment doesn't render category listing pages, or your storefront doesn't use the prerender solution at all, continue authoring the PLP's `<ol>` by hand (see [Step 2](#step-2-add-breadcrumbs-to-a-plp-and-a-pdp) above), or generate it with your own tooling.
</Aside>
