...
This tutorial covers the manual path: create a page that represents one category, give it SEO metadata, and keep its URL working when the category's URL key changes. You author everything in Document Authoring (DA.live), so no code changes are required.
