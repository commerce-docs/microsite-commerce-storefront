...
This tutorial covers the manual path: create a page that represents one category, give it SEO metadata, and keep existing links working when you change the page's URL. You author everything in Document Authoring (DA.live), so no code changes are required. To produce many category pages this way with a script instead of by hand, see [Generate category pages programmatically](/how-tos/automatic-category-page/).
