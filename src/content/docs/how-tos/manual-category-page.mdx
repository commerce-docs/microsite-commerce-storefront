...
description: Learn how to author a single category (product listing) page by hand in Document Authoring, attach SEO metadata, and handle a category URL key change.
sidebar:
...
This tutorial covers the manual path: create a page that represents one category, give it SEO metadata, and keep its URL working when the category's URL key changes. You author everything in Document Authoring (DA.live), so no code changes are required. To produce many category pages this way with a script instead of by hand, see [Generate category pages programmatically](/how-tos/automatic-category-page/).
...
- A plan for handling a category URL key change without breaking existing links
...
<Aside type="note" title="When to prefer prerendering">
  Author category pages by hand only for the categories that exceed the automated limits. For
...
<Aside type="tip" title="Add breadcrumbs and structured data">
  ...
  block, and validate structured data with the{' '}
  <Link href="https://search.google.com/test/rich-results" text="Rich Results Test" />.
</Aside>
...
## Handle a category URL key change

"The category's URL key changed" can mean two different things, and each needs a different fix. Identify which one applies before you make changes.

### Case A: The category's URL key changed in Commerce

The category's URL key (its `urlPath`/`urlKey` in Commerce) changed, but you want the page to stay at the same storefront URL. Because the `product-list-page` block loads the category by its URL key, the page will stop resolving the category until you update it.

<Steps>

1. Open the category document in Document Authoring.
2. In the `product-list-page` block, change the `urlPath` value to the category's new URL key.
3. Republish the document.

</Steps>

The page URL is unchanged, so existing links keep working. If you _also_ want the storefront path to match the new key (for example, move the page from `/apparel` to `/clothing`), follow Case B as well.

### Case B: The page's URL changed

The path shoppers use to reach the category changed — you're moving the page from one URL to another (for example, `/apparel` to `/clothing`). Create the page at the new path, remove the old one, and redirect so existing links and search rankings follow.

<Steps>

1. Create (or move) the document to the new path, following Steps 1–5 above. Publish it.
2. Unpublish and delete the old category document so it's no longer served at the old URL.
3. Add a redirect from the old path to the new one in your `redirects` sheet, so shoppers and crawlers that hit the old URL land on the new page.

</Steps>

Use a redirect rather than only deleting the old page: a redirect preserves inbound links and search equity, while a deleted page with no redirect returns a 404. Edge Delivery Services redirects are permanent (301) redirects managed in a spreadsheet — see [Redirects](/merchants/edge-delivery-services/redirects/) for how the sheet works in this storefront, and <Link href="https://www.aem.live/docs/redirects" text="Redirects on AEM.live" /> for the sheet columns and behavior. After the redirect is live, update your [sitemap](/merchants/edge-delivery-services/sitemaps/) so search engines discover the new URL.
...
| `redirects` sheet entry                     | Keeps old links working after a page URL change (Case B)                 |
