...
3. Uploads and publishes the file to da.live via the DA Admin API
...
**Without credentials**, the script still runs — it fetches and flattens the categories, writes local output files, but skips the DA upload. This is useful for testing the category tree locally.
...
**Example — sync the default store:**
...
**Example — sync a localized store:**
...
[default] Fetching navigation (family: default)...
[default] Flattened 7 categories
✅ [default] Written tools/nav-sync/default/nav.json
✅ [default] DA: saved /adobe-commerce/thunderbolts-aco/nav.json
