...
<Aside type="tip" title="Faster with an AI coding agent">
Install [Boilerplate skills](/ai/boilerplate-skills/) before you start. Your agent will already know these block and drop-in customization conventions instead of inventing its own.
</Aside>

## Where to customize

Most files in the boilerplate can be modified. The table below lists the usual layers you change for branding, blocks, Commerce wiring, page scripts, and build-time queries. Order follows a common path: global styles and blocks first, then initializer and script layers, then `build.mjs` for query shaping.
...
| `scripts/initializers/` | Configure the API endpoint and UI labels for each drop-in. Each drop-in has one initializer file, such as `cart.js` or `checkout.js`. If you remove a drop-in, also remove its initializer file and all imports of it in the codebase. See [Connect a drop-in](/dropins/all/quick-start/). |
...
Listen to and respond to drop-in events using the event bus. See [How drop-ins coordinate](/get-started/architecture/drop-ins-on-a-page/) if you're not yet familiar with how the event bus works:
...
graph LR
  T1["Row 1<br/><b>Commerce Cart</b>"]
  T2["Row 2<br/>custom-option | premium"]

  subgraph blockDiv["<strong>div.commerce-cart</strong><br/>data-block-name"]
    subgraph rowDiv["row div"]
      direction LR
      C1["cell div<br/>custom-option"]
      C2["cell div<br/>premium"]
    end
  end
  ...
  style T1 fill:#FEF3E2,stroke:#E8A33D,stroke-width:1.5px
  style T2 fill:#E3F2FD,stroke:#4C9AD4,stroke-width:1.5px
  style blockDiv fill:#FFFDF8,stroke:#E8A33D,stroke-width:1.5px
  style rowDiv fill:#F1F8F2,stroke:#5CB85C,stroke-width:1.5px
  style C1 fill:#ffffff,stroke:#5CB85C,stroke-width:1px
  style C2 fill:#ffffff,stroke:#5CB85C,stroke-width:1px
`} caption="How a document table maps to a block's DOM. Row 1 names the block and becomes its class and data-block-name attribute rather than a row div. Row 2, and every row after it, becomes a row div, with each cell becoming a nested cell div." />
...
1. Check the [Blocks reference](/blocks/) for the full list of blocks, their drop-ins, and page-type groupings.
