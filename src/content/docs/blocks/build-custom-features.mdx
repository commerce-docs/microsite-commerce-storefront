...
You can still use the configuration patterns covered in [Customize blocks](/blocks/customize-blocks/). A content author places each block by adding a table with its name to a document — see [Block tables](/merchants/blocks/block-tables/) — so placing or moving a block never requires a code change.
</Aside>
...
- Read [Customize blocks](/blocks/customize-blocks/) for `readBlockConfig()` and other block-authoring patterns.
