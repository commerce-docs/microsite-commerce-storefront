...
slug: dropins/all/build-custom-features
sidebar:
...
<Aside type="note">
This page assumes you already know how Edge Delivery Services loads a block, and that you've already tried wiring in a drop-in's pre-built container. If you haven't created a custom commerce block before, start with [Create commerce blocks](/boilerplate/create-commerce-blocks/) for the file conventions and `decorate()` function this pattern builds on. For the standard way to add a drop-in to a block, see [Connect a drop-in](/dropins/all/quick-start/) first — come back here only if no container fits your UI.
</Aside>
...
You can still use the configuration patterns covered in [Customize blocks](/boilerplate/customizing-blocks/). A content author places each block by adding a table with its name to a document — see [Block tables](/merchants/blocks/block-tables/) — so placing or moving a block never requires a code change.
</Aside>
...
- Read [Customize blocks](/boilerplate/customizing-blocks/) for `readBlockConfig()` and other block-authoring patterns.
