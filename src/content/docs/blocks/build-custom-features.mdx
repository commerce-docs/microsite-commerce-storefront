...
<Aside type="note">
This page assumes you already know how Edge Delivery Services loads a block, and that you've already tried wiring in a drop-in's pre-built container. If you haven't created a custom commerce block before, start with [Create commerce blocks](/blocks/create-commerce-blocks/) for the file conventions and `decorate()` function this pattern builds on. The examples use the shared `CS_FETCH_GRAPHQL` instance and the `@dropins/tools` component provider, which your storefront includes by default.
</Aside>
...
| How much you own | Get data | Render UI |
| --- | --- | --- |
...
The trade-off is that you own the raw response shape: results come back nested under `items[].productView`, with prices split across `SimpleProductView` and `ComplexProductView`. Drop to this level only when a drop-in API doesn't cover your case.
...
## Check what your feature needs

The rows below run from least work to most, so take the first that matches your case.

<TableWrapper>

| If this is true | Do this |
| --- | --- |
| **A container already renders it** | Use it as-is: [Connect a drop-in](/dropins/all/quick-start/) |
| **An API has the data**, no container fits | Call the API function: [Get data](#get-data) above |
| **The field's in the schema**, no API exposes it | Write your own query: [Get data](#get-data) above |
| **A payload is missing a field** | [Extend the data payload](/dropins/all/extending/#extend-the-data-payload-for-the-drop-in) |
| **The field isn't in the schema** | Extend the schema with <Link href="https://developer.adobe.com/commerce/extensibility/" text="App Builder" /> |
| **The feature needs its own state or model** | [Extend or create a drop-in](/dropins/all/extend-or-create/) |
