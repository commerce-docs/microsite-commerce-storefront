...
For the configuration patterns behind these options, see [Blocks customization](/boilerplate/customizing-blocks/). A content author places each block by adding a table with its name to a document, so placing or moving a block never requires a code change. See [Block tables](/merchants/blocks/block-tables/) for how those tables work.
