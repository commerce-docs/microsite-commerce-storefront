...
The Product Details drop-in (PDP) is a JavaScript module that renders product details on a Product Details Page (PDP) in a storefront. The PDP drop-in is a collection of functions, components, and utilities that work together to fetch product data from a Commerce backend service and render it on a PDP page.
