...
description: Learn about the slots used in the DROPIN_NAME drop-in.
---
...
The DROPIN_NAME drop-in provides the slots described here. These slots are used to display the various components within the drop-in.

## Big Picture

{/* Replace `placeholder.webp` in diagram with an illustrated image of the drop-in identifying all its slots. */}
...
{/* Add a Note (if necessary) as a way to identify the slots that are not currently used in the drop-in's default configuration. */}
...
{/* Add the code definition for the drop-in's `slots` interface */}
